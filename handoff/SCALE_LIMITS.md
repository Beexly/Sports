...
## 9. CORRECTIONS (2026-08-16, Opus adversarial fact-check — MATERIAL ERRORS found)

The route counts, the unbounded-`findMany` scan, and the Prisma-singleton/no-connection-limit
findings (§1-§3) are independently re-verified and solid. **§4's Vercel platform numbers and §5's
rate-limit ranking are wrong, not just stale**, and both feed the report's headline "what breaks
first under 10k req/hour" conclusion — do not use the uncorrected numbers for capacity planning.

**Wrong Vercel platform defaults (§4):**
| Claim | This report said | Verified reality |
|---|---|---|
| Execution timeout default | 10s Hobby / 15s Pro | **300s on every current plan** (Fluid Compute is now the default; Pro/Enterprise max is 800s) |
| Memory default | 1024 MB | **2048 MB (2GB)** |
| Response size cap + mechanism | ~1MB, via Next.js `api.bodyParser` | **4.5MB hard platform cap** — the cited mechanism (Pages Router `bodyParser`) doesn't even apply; this repo is 100% App Router |
| Routes explicitly setting `runtime="nodejs"` | "only 2" | **40 files**, including public routes (`waitlist`, `contests/week`) |

Effect: items ranked #4/#5 in the original failure-ordering (timeout, response cap) have 20-30x and
4.5x more headroom than claimed — at 10k req/hour they are very likely not real near-term failure
points. Drop or heavily de-prioritize them; #1-#3 (rate limiter, DB connections, unbounded findMany)
remain the real risks.

**§5's rate-limiter claim overstates its own case — this is good news the report buried:**
A durable, cross-instance **Postgres-backed** rate limiter (`PostgresDurableRateLimiter`) already
exists and is **live in production** for `waitlist`, `contests/enter`, `cipher/verify`,
`v1/signals`, `v1/probabilities`, and both B2B routes — not "no durable store exists anywhere," as
originally written. It has its own pruning cron. This does not change the 105-unprotected-route
count, but it means 5-6 of the 71 "protected" routes are NOT subject to the per-instance-multiplies
risk the report describes for all 71.

**`daily-slate`'s stated bound is factually wrong.** Claimed a date filter in `baseWhere`; no such
filter exists. The real bound is `result: "PENDING"` tied to the hourly settle-picks cron — a
different, arguably more fragile bound (no orphan-cleanup job found for stuck PENDING rows).

**Independent corroboration worth surfacing:** DB connection exhaustion (§ranked #2) is not
hypothetical — production logged 113 real "can't reach database" errors Jun 3-30 per the go-live
doc, stronger evidence than this report itself cited. Partial mitigant already in prod and not
credited here: the Neon connection string uses the `-pooler` hostname (server-side pooling).

Also: 8+ file:line citations in §1-§7 point to the wrong line number (code correctly described,
citation off — e.g. `schema.prisma:5`→14, `index.ts:194`→209). Cosmetic individually, but re-check
before using this doc as a file:line reference.
