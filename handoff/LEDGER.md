...
||| H-1 | CLV + pick-universe data census (read-only) | BLOCKED | 2026-08-18 22:45 | Database unreachable: Can't reach database server at `localhost:5433` (DATABASE_URL/DIRECT_URL not set in environment). Per handoff: stop entire run. ||
||| H-2 | modelProb provenance trace (read-only) | TODO | — | — ||
||| H-3 | fix isotonic PAV (unblocks P1-15 and CORP) | TODO | — | — ||
||| H-4 | Bernoulli e-process + null-simulation tests | TODO | — | — ||
||| H-5 | wire markClosingSnapshots behind existing flag | TODO | — | — ||