...
| #94 | claude/hotfix-proof-count-utc-bounds | main | M-F12: root+count commitment contract codified (fold stable — frozen prod roots), ambiguity pinned by test, commitmentDigest additive primitive; T-board-utc: board todayBounds → Date.UTC; Codex round (1/1 real, fixed 8d856a18, resolved): /board co-renders passes.ts which still had local midnight — extracted shared lib/time/utc-day.ts, rewired state.ts + passes.ts + admin picks-today counter (sweep), boundary pinned. Remaining adversarial queue: T-perf-summary (before any summary writer), O-3.x remainder, G-10..G-18 (on #77/#78 branches), M-F11 (OWNER) |
| #95 | claude/hotfix-vacuous-stub-tests | main | O-6: 8 populated-fixture pins for board state/passes + daily-slate (suppression proven against DATA, zero-query short-circuit pinned; where-clause/mapping/aggregation no longer vacuously green). Codex round (5/5 real, fixed dc1e3a3c, resolved): mocks now APPLY args.where against pools seeded with must-be-excluded rows (bootstrap + stale-48h decisions can't leak); BASE_WHERE call-shape asserted on both pickCount calls + findMany (isPublished/PENDING/isBootstrap:false/dataQualityScore gte); fallback lanes get distinct mockResolvedValueOnce fixtures with both lanes asserted by gameId; modelVersion sentinel v9.9.9 ≠ engine constant v5.1.0 (proves copy, not fallback); AND one LIVE ROUTE BUG — demo daily-slate published totalPicks 0 beside freePickCount 1 (breakdown didn't sum to total); fixed at root: demo branch now sets totalPicks = samples.length, one source of truth per mode. No longer test-only: 1-line runtime diff in daily-slate route |
