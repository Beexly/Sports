...
| M-F5/F6 | `subscription.updated` trusts embedded snapshot w/o freshness ordering; refresh/settle TOCTOU can rewrite a settled pick | MED/CONF | FIXED: M-F5 → PR #91 (retrieve-fresh on created/updated + Codex round 3a0cdd31: superseded-sub guard, CANCELED sync converges on delete handler's terminal state); M-F6 → PR #92 (refresh rewrite = updateMany scoped result:PENDING, P2002 winner adoption on create race) |
| M-F7 | No staleness bound on "the close"; `take:80` can truncate a closing batch | MED/CONF | FIXED: PR #92 — MAX_CLOSE_AGE_MS 6h bound in deriveClosingSnapshotFromOdds (over-age batch → empty snapshot, CLV verdict stays null); take 80→240 (27+ books x 3 markets exceeds 80 in ONE batch) |
...
| T-picks-outage | `/api/picks` DB outage returns bootstrapGateResponse → outage dressed as deliberate gating | MED/CONF | **FIXED — PR #87** (claude/hotfix-picks-outage-state): outageGateResponse (reason:"backend_outage", bootstrapMode:false) on /api/picks + /api/clv catch paths; three public dark states mutually distinguishable. **Verify-workflow round (16 agents, all confirmed, fixed 413dc11e):** /picks page guard admitted only 2 of 3 states → outage rendered raw "Failed to fetch picks: 503" (now a designed amber state); prod-probe validators were shape-blind → shared classifyDarkState passes deliberate states, fails backend_outage BY NAME; launch-runbook 503 row now documents all three discriminators. **Codex round (d8e76090, real, resolved):** stale_data 503 accepted ONLY on the picks probe (allowStale opt-in; misrouted stale bodies fail by name) |
| T-outage-sweep | Same dressing anti-pattern CONFIRMED at 5 more public surfaces (verify workflow, each independently verified): calibration report DB-fail → 200 isCollecting:true; daily-slate count fallbacks → healthy empty slate; promotions DB-fail → 200 success + 5-min CDN cache; game-room load null-collapse → outage becomes 404 "game-not-found"; proof-of-record `.catch(()=>[])` → empty ledger with sha256("") root (outage shown as "no record") | MED/CONF | **FIXED — PR #89** (claude/hotfix-outage-sweep, stacked on #87): calibration loader carries readFailed discriminator (pages stay render-safe, API 503s); daily-slate primary reads → outage 503 (page already tolerates non-OK); promotions → outage 503 + no-store (was CDN-cacheable fabricated empty); game-room loader throws (null = missing only; model-court 503s, room page → error boundary); proof committed-set read throws into the page's EXISTING ledgerUnreachable state (display-enrichment catch stays — "—" placeholders are honest). prod-probe calibration moved to status-aware gate probes, outage fails BY NAME. 13 pins |
...
| M-F10 | `fantasy-upsell.tsx` hardcodes "$49/yr" — drifts from pricing-phases on phase advance | LOW/CONF | **FIXED — PR #88** (claude/hotfix-fantasy-upsell-price): price resolved SERVER-side from pricing-phases (client-side resolution would silently pin FOUNDING forever — PRICING_PHASE is server-only env) and threaded through DraftAssistant/BestBallBoard/OptimizerWorkspace as a REQUIRED prop (compiler-enforced, M-F13 lesson). pricing-honesty pin |
| M-F12 | Merkle count not folded into root (`[A,B,C]` and `[A,B,C,C]` share a root) | LOW/CONF | FIXED: PR #94 — fold kept stable (slate roots frozen in prod DB; algorithm change would read as tampering); contract codified root+count = commitment, ambiguity documented AND pinned by test, count-check pinned (verify/slate receiptIndexComplete + /proof totalSettled already enforce); additive commitmentDigest(hash,root,count) for external publications |
...
| M-F11 / T-board-utc | vercel.json Root Directory check; board todayBounds uses LOCAL time vs engine UTC | LOW/CONF | M-F11 still OWNER (dashboard check). T-board-utc FIXED: PR #94 — todayBounds computed via Date.UTC (was server-local midnight; identical on Vercel, wrong day on non-UTC hosts) |
...
| G-3/G-4 | radar `validateSnapshot` never called at runtime → policy fails OPEN on unknown POSTURE (risk is fixed-closed, posture is not); REJECT/posture typos → OBSERVE | FIXED — #76 (7cfdb285): validateSnapshot runs at module load, getObservations throws on problems (loud error, never a mislabeled feed); effectiveDisposition guards unknown postures → quarantine (mirrors risk guard); unknown labels rank most-restrictive in merges; normalizePosture REJECT matches by prefix, unrecognized fallback → OWNER_REVIEW (importer mirrored; committed snapshot proven byte-stable by re-import); source-scan pin keeps the wiring alive |
| G-10 | assurance council-autonomy tripwire checks "ACTIVE", outside the CouncilSeatStatus union → dead guard behind an `as` cast | FIXED — #77 (ed2911de): closed-set membership over GOVERNED_SEAT_STATUSES (any future autonomy-shaped union expansion fires it); the cast shape banned by pin |
| G-11 | assurance security + outcome_quality have no findings generator → vacuous health 1.0 shown as "70% inspected, 100% healthy" | FIXED — #77 (ed2911de): explicit MEDIUM "no checks implemented" findings for both categories — health capped through normal scoring; behavior pin asserts health < 1.0 |
| G-6 | foundry authority rule fires only for tier-0 seats; 2 of 3 seed owners are tier≥1 | FIXED — #77 (ed2911de): HIGH/CRITICAL risk requires humanApprovalRequired under ANY tier; pinned across all seed owners |
| G-15 | routing rule order: HIGH+structured takes cheap EXTRACT lane before the HIGH-risk rule | FIXED — #78 (4cdd2a38): HIGH-risk check hoisted above the extraction shortcut (taskType precedence unchanged); ROUTING_POLICY_VERSION → 1.1.0 (shadow records carry it); 4-case order pin |
| G-14 | assurance coverage rounded to 2dp before threshold compare (boundary disagree) | FIXED — #77 (ed2911de): grade gate compares unrounded weightedCoverage; rounding display-only; source pin bans the rounded-compare shape |
| G-2 | repo-root partial-tree spoof on Vercel (`packages/db` traced for Prisma) → null-guard may not fire | FIXED — #77 (ed2911de): findRepoRoot additionally requires source-only markers (apps/web/__tests__ + .github/workflows) that bundler tracing never ships |
| G-12/G-13 | assurance evidence strings hard-coded vs existsSync-only; `foundry-unused` hardcoded ACKNOWLEDGED; flag name `AI_MODEL_ROUTER_LIVE_ENABLED` exists nowhere | FIXED — #77 (ed2911de): observations content-verified (files read, symbols found); telemetry finding derives from the actual ClaudeApiCallRecord block and self-extinguishes when a lane column ships; live-flag name EXTRACTED from router source; manifest counts derived; fabricated ACKNOWLEDGED → OPEN |
| G-7/G-8/G-9/G-16/G-17/G-18 | scanner verb/field coverage gaps; nested-key canonical hash; page executable tile hardcoded 0; SENSITIVE-block reason text; grep pins scoped too narrow; install-affordance regex misses pnpm/yarn/bun | FIXED — G-7/8/9/16/17 on #77 (ed2911de): verb list widened + whole-manifest injection scan; deep-sorted canonical JSON (seed hashes unchanged — single-key nested objects); executable tile derived via canExecute; sensitive-block names classes, no phantom exception; cwd pin widened to tree-walk. G-18 on #76 (411d7f5e): no-install pin covers pnpm/yarn/bun/cargo/uv/pipx/brew |

### Guardrail/CI/secret-scan holes (independent → guardrail-hardening PR)

| # | Finding | Note |
|---|---|---|
| O-3.x | trust-gate: line-based matching defeats multi-word bans; string-splitting; homoglyphs/entities/soft-hyphen; markdown headings skipped as comments; "the lock" blanking; commercial-scan negation+import exemption; draft-only comment-suffix exemption | HIGH-within-scope/CONF. **FULLY FIXED — PR #81** — CORE (4eb7d475): NFKC + zero-width/soft-hyphen strip + apostrophe folding on all 3 copy gates; markdown headings/lists now scanned; 6 planted-evasion pins. REMAINDER (793901cf): shared scan-normalize.mjs adds a cross-script CONFUSABLES fold (Cyrillic/Greek→Latin, which NFKC won't do) + a join-collapse view ("guaran"+"teed" / `risk${x}-free` re-form) + adjacent-line-pair matching for multi-word bans; "the lock" blanks ONLY behind a temporal preposition (bare "the lock" is slang again); commercial tout-slang exemption is CLAUSE-scoped (a distant "not hype." no longer excuses a later-sentence tout) while descriptive EVIDENCE_REQUIRED terms keep line-wide; only STRUCTURAL import/export shapes are exempt (`export const tagline="…"` scanned); draft-only matches comment-STRIPPED code (strings preserved) so a real publish call can't hide behind "// INTENTIONALLY". brand.ts + draft-review-fixtures.ts moved to explicit whitelist (held banned phrases as detection material, previously hidden via concat). 10 more planted-evasion pins; all 4 gates exit 0 on the real tree; guardrails suite 42/42. |
...
| O-6 | vacuous stub-DB tests: board-phase2/daily-slate/dev-state assert empty results that hold whether or not the where-clause is correct | FIXED: PR #95 — 8 populated-fixture pins (data flows through lanes/counts/breakdowns; suppression empties a POPULATED board with ZERO queries — a decision, not stub coincidence). dev-state assessed: env-flag reporting only, nothing to de-vacuate. Stub tests kept (they pin the stub contract; new ones pin the logic). Codex round (5/5 real, fixed dc1e3a3c, resolved) — the review of the TESTS surfaced a LIVE ROUTE BUG: demo daily-slate published totalPicks 0 beside freePickCount 1; demo branch now sets totalPicks = samples.length (one source of truth per mode). Plus: filtering mocks (applyDecisionWhere over pools seeded with excluded rows), BASE_WHERE call-shape assertions, distinct per-lane fallback fixtures, v9.9.9 modelVersion sentinel distinct from the v5.1.0 engine constant. Lesson: a vacuous test isn't just risk of future regressions — it was already hiding a shipped inconsistency |
