...
| T-outage-sweep | Same dressing anti-pattern CONFIRMED at 5 more public surfaces (verify workflow, each independently verified): calibration report DB-fail → 200 isCollecting:true; daily-slate count fallbacks → healthy empty slate; promotions DB-fail → 200 success + 5-min CDN cache; game-room load null-collapse → outage becomes 404 "game-not-found"; proof-of-record `.catch(()=>[])` → empty ledger with sha256("") root (outage shown as "no record") | MED/CONF | QUEUED: per-surface honest degraded state — each needs its own design decision (200-shape surfaces can't just 503; proof ledger must distinguish "empty record" from "couldn't read record"). Pattern: outage-gate.ts doctrine table |
