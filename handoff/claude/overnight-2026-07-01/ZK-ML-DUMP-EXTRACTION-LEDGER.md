...
## WAVE 7 (2026-07-02, sixth dump): Bulletproofs/IPA/Pedersen — the first BUILDABLE crypto primitive

Most of this wave is echo (KZG/Plonk/IPA restated) plus our own wave-6
"Verification + Improvement" paragraph handed back. But it surfaced the one
crypto idea across all seven waves that is **real, buildable TODAY, and adds a
capability GSE genuinely lacks** — so it got built, not just ledgered.

**BUILT — `pedersen-ledger.ts` (+ 13-test suite), `[BUILT]` primitive /
`[BUILDABLE-future]` consumer.** Pedersen commitments are additively
homomorphic: `Commit(v1,r1)·Commit(v2,r2) = Commit(v1+v2, r1+r2)`. That gives
GSE the middle rung between Merkle (tamper-evidence, but you must OPEN a leaf to
check it) and Nova ZK (full generality, unbuilt): **verify a published aggregate
against per-pick commitments without opening any individual pick.** The concrete
consumer is the pre-reveal sealed slate — prove a numeric property of tomorrow's
committed slate before kickoff, reveal picks later — a strictly stronger
anti-cherry-pick story than Merkle alone. Proven by execution: hiding
(same value, different blinding → different commitment), computational binding
shape (opens only to the true (v,r)), exact homomorphism, and the payoff —
`verifyLedgerAggregate` accepts the true total and REJECTS a total doctored by
one, a wrong blinding, or a swapped-in commitment to a different value.

**Honesty scope carried in the module doc (crypto claims get data-and-method
discipline too):** classical DLOG only — **NOT post-quantum** — so it is strictly
ADDITIVE to the SHA-256 Merkle layer, never a replacement (SHA-256 stays the
primary, hash-based, publicly-re-verifiable tamper-evidence). Perfectly *hiding*
+ computationally *binding* (stated precisely, not the reverse). Pure-BigInt
reference, not hardened (no constant-time modexp — acceptable because the verify
path has no secrets; blindings live at a CSPRNG loader boundary the caller owns).
The group is a demonstration group **generated by reproducible execution** (safe
prime ≥ 2^255+951 by Miller-Rabin search; g=4 order-q; h nothing-up-my-sleeve
from `sha256("GSE-pedersen-h-v1")`) with a runtime `verifyGroup()` that
RE-CERTIFIES it, and a doc note that production should swap to a standardized
group.

**Hostile-cryptographer pass (independent execution) — CLEAN on all crypto,
2 prose fixes applied:** the agent re-derived `h` from the seed byte-for-byte
(NUMS holds), independently re-primality-tested the group (16 disjoint MR
witnesses), and found no cross-value forgery in 100k attempts — binding under
DLOG, hiding honestly conditional on uniform blinding, homomorphism correctly
scoped, "not post-quantum" accurate. The real finding was **a hallucinated
constant in my own doc**: it recited the provenance offset as "2^255 + 951"
(the search's *starting* point) when the true offset is **115095** — precisely
the recited-constant trap the paragraph brags about avoiding. Fixed, plus a
pinned regression test (`PEDERSEN_Q − 2^255 === 115095n`) so it can never
silently return, an understated no-wrap bound corrected (10^60 → 10^69), and
the timing-scope sentence tightened (the commit path DOES exponentiate the
secret blinding; only the verify path is secret-free). 14 Pedersen tests green.

**Reference rows filed:** Bulletproofs (transparent log-size range/IP proofs, no
setup), Inner-Product Arguments (the recursive-folding core of Bulletproofs +
Halo2 IPA), KZG (correctly flagged trusted-SRS — the transparency-preferred
ledger keeps it final-compression-only). All accurate; all `[REFERENCE]` /
`[ROADMAP]` with no present GSE surface beyond the Pedersen build.

**The stub (`foldPick` + "pedersenCommit: homomorphicAdd") — refuted by
execution:** parses, but running it throws `ReferenceError: F is not defined`
(the `// tested` claim is false; same family as every prior stub). The idea it
