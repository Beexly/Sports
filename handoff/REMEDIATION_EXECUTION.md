...
**CORRECTION 2026-08-16 (verified live):** The claimed commit `d4da1265` does NOT implement a refund handler. `git show d4da1265 --stat` shows it touches only `board-gate-decisions.test.ts`, `route.ts` (board state), `app/board/page.tsx`, `app/preview/[sport]/[slug]/page.tsx`, and `lib/board/state.ts` — all tier-gating for the board/preview paywall (GSE-SEC-025), zero refund-related code. `git show d4da1265 | grep -ic "refund\|charge"` returns `0`. Additionally, `grep -n "charge.refunded\|refund" apps/web/app/api/webhooks/stripe/route.ts` returns ZERO hits — the refund-revocation handler is NOT present in the webhook route. This finding remains **OPEN**, not FIXED. `handoff/LAUNCH_BLOCKERS.md` §1.2 correctly lists GSE-SEC-021 as BLOCKING (the refund gap). Status corrected to OPEN.
...
**CORRECTION 2026-08-17 (verified live by P8-08-RESUME):** This finding is RESOLVED, not OPEN. Re-verification of the current tree found every Stripe **mutation** in `apps/web/lib/stripe.ts` is already gated by `requireDurableWriteStore` — `getOrCreateStripeCustomer` (guard("stripe-checkout"), line 209), `createCheckoutSession` (guard("stripe-checkout"), line 290), and `createPortalSession` (requireDurableWriteStore("stripe-portal"), line 451) — all introduced by commit `a56fe1dc` (`git log -S 'requireDurableWriteStore("stripe-portal")'` confirms). A repo-wide mutation scan (`grep -rnE "stripe\.[a-zA-Z_]+\.(create|update|del|cancel)\("` over apps/web + packages, excl tests + node_modules + the two stale worktrees at `.claude/worktrees/phase3` and `Sports/`) returns exactly three mutation sites, all guarded. The webhook path is separately gated at `app/api/webhooks/stripe/route.ts:62` (capability `stripe-webhook-entitlement`) and the reconcile path at `lib/billing/reconcile-entitlements.ts:494,579` (`stripe-reconcile`). The original "only two caps" description at line 393 is stale — that line is now the checkout-session *listing* loop (a READ), not an unguarded write. Status corrected to FIXED. New invariant test `apps/web/__tests__/stripe-mutation-guard-invariant.test.ts` (4 tests, all passing) pins the property so a future 4th unguarded mutation fails. |
...
**CORRECTION 2026-08-16 (verified live):** The original FIXED status was false. The `checkClearance` at `free-score-persist.ts:103` gates `henrygd-ncaa`, not `fpl-api` (confirmed via `sed -n '95,110p'`). Commit `b992f1c3` is titled "fix: GSE-SEC-050" and only touches henrygd-ncaa/MLB/BallDontLie/NHL gates. The fpl-api adapter at `free-adapters/fpl.ts:150` remains dormant (INFO, not FIXED).
