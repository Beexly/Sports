...
- Stripe payments.
- Separate Python service "odds-engine" with api_server.py: odds sync has a "fast lane" + "full sweep" (~50 leagues), process-local mutex, WatermarkBroadcast via Turbo Streams; see RAILS_TRIGGER.md (internal doc referenced in JS comments, not public).
...
- odds-engine (Python, api_server.py): "Sync odds now" fast lane + "Full sweep" ~50 leagues, one process-local mutex, 90s client backstop. Live board: Postgres trigger -> Turbo Streams WatermarkBroadcast, min interval 5s, price flash on >=0.05 relative implied-probability move (green=better for bettor, red=worse).
...
- @NerdingonNFL — timeline still sticks at "Loading…" (standing obstruction, unchanged).
...
- @NerdingonNFL: timeline never renders (standing obstruction); one bounded retry failed.
- @FTNData: protected account (no follow requested); one retry failed (standing obstruction).
- @NFLResearcher: timeline never renders; one bounded retry failed (standing obstruction).
...
Window: posts after ~10:30 PM CDT 2026-09-21 through ~10:00 AM CDT 2026-09-22 (overnight + morning). Read-only; 21 primary + 6 secondary accounts checked sequentially, home feed scanned, keyword searches (EPA / TPRR / aggressiveness / pass rush win rate / CPOE). No rate-limiting; no CAPTCHAs; no interactions. Standing obstructions: @NerdingonNFL and @NFLResearcher timelines never render; @FTNData protected (no follow requested).
