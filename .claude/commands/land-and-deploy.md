...
allowed-tools: Read, Grep, Glob, Bash(git status*), Bash(git log*), Bash(git diff*), Bash(git fetch*), Bash(npm run qa*), Bash(npm run guardrails*)
---
Land branch: $ARGUMENTS
1. Check preconditions before proposing a merge: CI is green on the branch head (inspect, don't assume), `/qa` is green, `/review-pr` has been run on this branch with no unresolved Critical/High findings, and the branch has no merge conflict with `origin/main` (`git fetch origin` then diff/status check).
