...
3. **`db:push` (`prisma db push`) is dev-only.** It's fine against a local/disposable database for quick iteration, but it must never run against a shared or production URL — it has no migration history and can silently drop data. CI no longer runs it at all (removed 2026-09-02): the test database is built by `migrate deploy`, exactly like production.

4. **Production applies migrations via `prisma migrate deploy`**, run by `scripts/deploy/migrate-if-configured.mjs` during the Vercel build. On the normal path that script is fail-closed: if migration application is misconfigured, the build fails rather than deploying app code against a schema it doesn't match. There is one documented break-glass: setting `MIGRATE_GATE_ALLOW_UNVERIFIED=true` in the Vercel env makes the gate skip `prisma migrate deploy` entirely, logging a loud warning. It is an explicit, deliberate operator override — never automatic — scoped to the migration-ledger reconciliation window (`docs/ops/MIGRATION_LEDGER_RECONCILIATION_RUNBOOK.md`), and must be removed immediately after that deploy.

5. **CI replays the migration history, blocking.** `.github/workflows/ci.yml` runs `prisma migrate deploy` against the empty test database and then `prisma migrate diff --from-url "$DATABASE_URL" --to-schema-datamodel … --exit-code`. Since the 2026-09-02 squash the replay passes: the baseline applies from empty and is a no-op on a database that already carries the schema (verified on a disposable Postgres 16 against both). A `schema.prisma` change without a migration, or a migration that drifts from the schema, fails CI. Do not "fix" a red replay by editing or reordering existing migrations — write a forward migration.
