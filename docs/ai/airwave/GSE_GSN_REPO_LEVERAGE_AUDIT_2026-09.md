...
## Round 14 — turning inward with a dead-code scanner, five dormant-subsystem investigations, one shipped fix (2026-09-07)

**Why this round exists**: a second explicit founder correction — Rounds 11-13 were still repo-browsing, and were told directly to "go outside the box" while also being told to stop producing isolated verdicts. Rather than another external GitHub sweep, this round ran `npx knip` (v6.34.0, one-off via `npx`, not installed as a dependency — Law 7 unaffected) against the whole monorepo, a different discovery method than nine prior grep-based passes. Result: 275 unused files, 282 unused exports, 380 unused exported types — an order of magnitude more than manual grepping had ever surfaced. Five apparent dormant subsystems were dispatched for full investigation (real code read, test coverage checked, git history checked, three-lens verdict required): `packages/prediction-engine/src/metrics/core/` and siblings, `packages/stats-api/src/catalog-expand.ts`, `apps/web/lib/decision-genome/`, `apps/web/lib/fable/`, and the `apps/web/lib/{revenue,media-revenue,studio/templates,pre-mortem/templates,newsletter}` cluster.

**One false positive, corrected and fixed in code this round.** `catalog-expand.ts`'s 16 "zero-importer" functions are not dead — `expandAll()` feeds `catalog.ts`'s `getMetricCatalog()`, which backs the live `/api/gse/v1/catalog` and `/metrics` routes (verified by running the real catalog build: 881 metrics, 826 `publicApi`, by sport `NFL 400, MLB 112, SOCCER 80, NHL 39, NBA 32, NCAAF 23, NCAAB 23, MULTI 172`). Knip flagged them only because no *other module* imports the function names directly. But the investigation found a real, narrow bug underneath the false positive: `SportCode` (`packages/stats-api/src/catalog-types.ts`) never had `WNBA`/`F1`/`TENNIS`/`MMA` members, so `expandWnba`/`expandF1`/`expandTennisOptical`/`expandMmaResearch` stamped ~50 real, already-live catalog rows as `sport: "MULTI"` — invisible to `GET /api/gse/v1/metrics?sport=WNBA` (or `F1`/`TENNIS`/`MMA`) because no such code existed to filter against. **Fixed in this round** (commit `ed39253`): added the four `SportCode` members and retagged the four functions' rows to their real sport; confirmed no exhaustive switch anywhere depends on the union (`bySport` is a generic `Record<string, number>`; `rights-boundary.test.ts`'s sport-filter test iterates a hardcoded literal list, unaffected). `expandOpticalResearch` (generic CV tracking-benchmark metrics, not tied to one sport) and `expandPredictionMarkets` (cross-league Kalshi quotes) were deliberately left `MULTI` — genuinely cross-sport, not mistagged. Validated: `npx tsc --noEmit -p packages/stats-api/tsconfig.json` (0 errors), `npx vitest run packages/stats-api/src/__tests__/` (100/100 passed), a direct re-run of the four expand functions confirming the new tags, `npm run guardrails` (26/26 PASS). Purely additive — no schema change, no new data, no gate touched, 2 files.

**Two real, well-built parallel-implementation problems, neither fixed (product decisions, not bugs).** `packages/prediction-engine/src/metrics/{core,passing,receiving,rushing,role,calibration,decision,market,source}` (89 files, 14,439 lines, 296 passing tests) is a genuine metric-governance framework — a "birth certificate" registry (`GseMetricBirthCertificate`: target question, historical precedent, allowed/forbidden inputs, formula class, validation method, lifecycle `DESIGN → SHADOW → BACKTESTING → REVIEW_READY → APPROVED`) gating ~20 real statistical metrics (QB burden index, expected/created YAC, rush-over-expected), every one still sitting at `SHADOW` by its own rules — not hidden, correctly un-promoted. The problem: `packages/prediction-engine/src/nfl/` independently implements the *same* metric family (its own birth-certificate registry, its own `gseQbBurden` with 6 terms at different weights than `metrics/`'s 10-term version) — both barrels are re-exported from the package's `index.ts`, and **neither QB-burden implementation is called anywhere outside its own test file**. Activating either means picking one formula over the other, a product call, then running it through the already-built, already-tested `metric-graduation.ts` promotion gate into one `feature-store` slot — not a bulk activation and not a merge of the two dialects. Similarly, `apps/web/lib/decision-genome/` (17 modules, 65 passing tests) is a real pre-decision governance layer — an `evaluateAperture()` state machine (Signal/Shadow/Wait/Pass/Quarantine), a point-in-time leakage kernel, an anti-survivorship-bias candidate ledger — but two of its pieces duplicate already-live GSE systems rather than complementing them: `conformal.ts` reinvents the "should we abstain" question `packages/prediction-engine/src/certificate/selective-abstention.ts` already answers with real production authority (that file's own comment: "does NOT replace `selective-gate.ts` — production authority stays in the gate"), and `agent-court.ts`'s Brier-scored credibility court uses the exact same agent roster (`scout`/`tal`/`jarvis`/etc.) as the already-live `apps/web/lib/jarvis/agent-council.ts`, with zero type relationship keeping the two rosters in sync. Recommendation, not executed: archive `decision-genome/conformal.ts` as superseded, and treat `agent-court.ts` as a spec to fold into the live council rather than a parallel system. The rest of decision-genome (`aperture.ts`, `knowability.ts`, `candidate-ledger.ts`, `proof-card.ts`, `epistemic-alpha.ts`, `claim-lang.ts`, `rumor-quarantine.ts`) is novel and a real candidate to activate as the pre-result complement to the already-live post-result `SettlementObservation`/`SettlementAnomaly`/`SettlementDecisionEvent` system (Round 13) — a founder/product-scoping decision on how much of a 17-module subsystem to turn on at once, not a small safe edit.

**One non-finding, worth recording so it stops getting re-discovered.** `apps/web/lib/fable/` (24 files) plus `docs/fable/`/`docs/personal/aws/` (~40 files) is not sports-related, not AI-related, and not abandoned — it's the repo owner's own AWS re/Start career-credentialing evidence tracker, deliberately embedded in this repo with fail-closed gates (`FABLE_AWS_ALLOW_EXPERIMENTS/DEPLOY/PAID_RESOURCES` all default `"false"`, zero real AWS SDK calls anywhere in it), served at a real public `/fable` route, 43 passing tests. "Fable" here shares nothing but a name with the Claude model tier — confirmed by reading all ~65 files, no Anthropic/Claude/LLM reference anywhere. Working as designed; the only real gap is that CLAUDE.md/AGENTS.md never mention it, which is exactly why nine prior audits missed it and had to reconstruct its purpose from scratch each time. Not touched this round (personal-content boundary, not a code decision); flagging so round 15 doesn't re-investigate it.

**One cluster, three distinct failure modes wearing one costume.** The `revenue`/`media-revenue`/`studio/templates`/`pre-mortem/templates`/`newsletter` group is not one dormant-drafts problem, on closer read: (1) `media-revenue`'s first-month content queue (30 days, fully scripted, claim-safety-scanned, `DRAFT_ONLY`) has a working review-queue/persistence system with zero UI ever built to look at it — the same "built-drafts-never-reviewed" shape Round 9 found in the weekly transparency recap, and the one piece here worth activating (an admin `/cockpit/content-queue` page shaped like the already-live `/cockpit/studio`, wiring existing logic, not new logic); (2) `revenue/`'s partner-risk/eligibility/disclosure engine has nothing to govern — no seeded partner data exists anywhere, so no review UI fixes it, only a first real outreach message would; (3) `pre-mortem/templates` (hyphenated) is a fully-built, tested, orphaned duplicate of the *different*, already-live `premortem/` (no hyphen) module that actually feeds `/api/picks/[id]/audit` and the public `/room/[gameId]` panel — a handoff failure, not a missing review step, and a founder call on which one to keep. `newsletter/issues.ts` and `studio/templates`'s 8 flagged exports were confirmed knip false positives (both genuinely wired and live) during this investigation, same pattern as `catalog-expand.ts`.

**What this round changes**: one real, small, safe bug fixed and shipped (`ed39253`, the `SportCode` gap) rather than filed as a founder-decision item — consistent with the standing instruction to ship real code over cataloging more deferrals. Everything else this round found is genuinely a product/architecture decision (which QB-burden formula, whether to fold agent-court into the live council, how much of decision-genome to activate at once, which pre-mortem module survives) and stays a named, evidenced proposal rather than an autonomous edit, per Week-Saver Mode's own rule to stop at multiple-viable-implementation-paths and architecture-level ambiguity.

## Round 15 — the widest sweep yet: 10 parallel domains, real verification required, two shipped fixes (2026-09-07)

**Why this round exists.** Founder instruction, verbatim in spirit: cast the wide net, no cap on scope, look outside the box, find what hasn't been researched or what's regressed, "coded claims then polished," and put everything where every agent can see it. Ten agents ran in parallel, each read-only, each required to verify externally (real fetches, real license/commit checks — no memory, no star-count trust) and check GSE's own code first before claiming a gap, following the standard this audit has held since Round 5. Two real, safe, scoped fixes were built and shipped directly from this round's findings (below); everything else is a named, evidenced proposal — separated clearly into buildable-now-free-small vs. founder/legal-only vs. honest-nothing-better-exists, per domain.

### 1. Player health, biometric, and injury-risk data

GSE already has more here than the prompt assumed: `apps/web/lib/human-performance/availability.ts` (NFL injury designation + weather + surface + rest → a band-widening-only availability multiplier), `readiness.ts` (a prior R&D pass already evaluated and correctly filed Catapult/Zone7/Kitman Labs/VALD as `ADMIN-ONLY`), `nflverse/snap-share.ts` and `next-gen-stats.ts` (live workload/tracking signals). This round checked seven wearable vendors total (adding STATSports, Kinexon, Firstbeat to the four already filed) and confirmed the same verdict across all of them: closed B2B platforms, club-licensed, no marketplace API. Whoop's own fetched Terms of Use go further than a pricing wall — they explicitly forbid reselling or exposing API data to any third party even with end-user consent, closing the door contractually, not just commercially. Academic injury-risk models: nothing beats GSE's existing "cite methodology only, don't reproduce" posture; one repo found along the way (`sahandb/football-injurylens`, advertised "96% recall") admits in its own README that the shipped dataset is a synthetic placeholder — flagging this so nobody cites that number later, the same class of trap Round 6 caught in ParlayAPI's sandbox-vs-real endpoints.

**Shipped this round:** `packages/data-ingestion/src/mlb-statsapi-client.ts` already calls `statsapi.mlb.com` for standings and scores but never called its free, keyless `/transactions` endpoint — the exact host, verified live before writing any parsing code (real Aug-2026 sample fetched and inspected, not guessed), confirming the stable "placed/activated/transferred ... injured list" phrasing. Added `fetchMlbInjuredListMoves()` (commit `b4a5a5d`), mirroring the file's existing soft-fail pattern; not yet wired into any scoring/availability path — that wiring, mirroring `availability.ts`'s existing NFL shape, is real follow-on work, not done here.

**Buildable now, not yet done:** wire the already-built, already-cleared (`approved_public_logged_off`), sport-agnostic `apps/web/lib/data-sources/free-adapters/espn-boxscore.ts` injuries parser into NBA/NHL — it has zero callers today despite being fully generic; an NBA back-to-back workload factor mirroring `availability.ts`'s existing pattern. **Founder/legal:** NBA's official injury report is PDF-only (a real, MIT-licensed parser exists, `mxufc29/nbainjuries`, but the underlying NBA data-reuse terms are unverified); Kaggle's NFL 1st & Future injury-tracking dataset is real and high-quality but its commercial-redistribution terms are unverified, not assumed free.

### 2. Weather, circadian rhythm, and schedule fatigue

Confirmed live and correctly scoped already: NWS weather for the 19 outdoor NFL venues (`apps/web/lib/weather/game-weather.ts`, feeding `availability.ts`'s confidence math, explicitly `canPublishPicks: false`). Confirmed built-but-fully-unwired (zero callers outside their own tests): `packages/prediction-engine/src/edge-lab/features/nfl-weather.ts` and `nfl-body-clock.ts` (a timezone-shift proxy) — the whole chain sits in edge-lab, tested, never reaching `game-context.ts`, the module that actually feeds published picks. `game-context.ts` itself has day-count rest and game-density features only — no weather, no travel-distance, no timezone field, board-wide (not just NFL). External check: NWS remains free/unlimited/correctly chosen for US venues; Open-Meteo's free tier really is non-commercial-only (GSE's existing gate is correct); Visual Crossing (1,000 free records/day) is a real, small gap-filler for GSE's Canadian MLS venues NWS doesn't cover. Real, peer-reviewed circadian-travel research exists (NBA eastward jet lag: −6% home win rate, Roy et al. 2022; MLB jet-lag offense/defense effects, Song et al. 2017 — though a published PNAS rebuttal letter disputes the causal claim, a genuinely contested finding, not settled science) and a real, MIT-licensed open dataset/methodology (`josedv82/airball`) computes exactly the missing travel-distance feature from public schedule data alone.

**Buildable now, not yet done:** add a travel-distance-in-miles feature (haversine on team home coordinates — GSE already stores venue lat/lon for the 19 outdoor stadiums, so this is ~10 lines, zero new dependency) alongside `nfl-body-clock.ts`'s existing timezone-shift feature. **Founder decision:** whether/when to promote the weather/body-clock/distance chain out of edge-lab into live `game-context.ts` scoring (MODEL_VERSION-frozen territory); whether to apply the contested east/west directional asymmetry at all.

### 3. Referee and officiating tendency data

GSE already partially ingests the raw ingredient: nflverse's `officials` dataset (CC-BY-4.0, already registered) plus `expandOfficialRows()` produce a crew-identity join key — but no foul-rate, penalty-rate, or bias computation exists anywhere; `evidence-readiness-matrix.ts`'s `"official.tendencies"` factor is explicitly unmet, and `schema.prisma`'s `hadOfficialsSignal` field is tagged `// (future)`, always false today. External check: MLB's own Terms of Use (fetched directly) explicitly ban automated collection for commercial use — UmpScorecards is `excluded`, not just ungated. Three NBA referee-data repos were checked at the source level; two have no license and unverified data provenance (unsafe to ingest commercially), one (`atlhawksfanatic/L2M`, MIT) is real but scoped only to final-two-minutes-of-close-games, too narrow for a season-wide signal. Honest literature read: referee/crew effects are real but small relative to team-strength factors, and documented as *shrinking* over time as markets price them in (NFL home-underdog bias: ~58% in the 1970s toward ~52-53% today) — this is a legitimate but marginal factor, not a headline one.

**Buildable now, not yet done:** the one real, free, zero-new-rights-call piece — join nflverse's already-licensed `officials` crew data to its already-ingested play-by-play penalty events by `game_id`, producing an NFL crew-level penalty-rate tiebreaker signal. Small scope, matches a real, corroborated public finding (crew-level penalty-rate variance up to 55% between crews is independently documented). **Not worth pursuing:** MLB/NBA officiating data — rights-blocked or too narrow for the size of edge involved.

### 4. Alternative leading-indicator signals — the round's single highest-value finding

**"Steam" / line-velocity is fully built, tested, and switched off — not a data gap at all.** `packages/ingestion-pipeline/src/line-archive.ts` and `pinnacle-line-archive.ts` persist timestamped OPEN/INTERIM/CLOSE line snapshots (Pinnacle explicitly as the program's primary CLV benchmark) but are hard-gated off by default (`LINE_ARCHIVE_ENABLED="false"`, `LINE_ARCHIVE_EU_PINNACLE="false"`) — meaning this history is not even being collected in production today. On top of that (would-be) data sit three independent, pure, tested compute libraries that already implement exactly the "steam"/informed-bettor-movement-proxy concept: `apps/web/lib/truthmetrics/market-memory.ts` (velocity, CLV, book disagreement — and a `sharpSplitSourced` gate that hard-blocks any informed-bettor-movement framing unless a real sourced split is ever supplied, which grep confirms nothing in the codebase ever does — GSE has already correctly drawn this honesty line in code, not just policy), `line-dna.ts` (a movement fingerprint), and `consensus-clock.ts` (an exponential dispersion-decay fit measuring how fast books converge). **Verified by grep: all three have zero callers anywhere outside their own tests.** This is the same "raw material and the math both exist, only the switch and the wiring are missing" pattern this audit keeps finding (Round 13's nflverse/scoring-zone, Round 14's calibration-regression detector) — here it happens to be gated behind a real Law-3 environment flag, so flipping `LINE_ARCHIVE_ENABLED` is explicitly a founder call, not something built this round, but wiring the three already-built compute functions into a real caller (an API route or cockpit view) ahead of that flip is safe, low-risk, zero-new-dependency follow-on work — the honest-empty design already handles the "flag still off, no data yet" case cleanly.

Everything else in this domain was checked and correctly ruled out with real evidence, not assumption: Google's official Trends API is alpha/allowlist-gated (not generally available), and `pytrends` (the long-standing workaround) was archived in April 2025 with unpatched rate-limit failures — no credible free path exists. StubHub/SeatGeek/Vivid Seats ticket-resale data is partner- or terms-gated (SeatGeek's fetched API terms explicitly bar using derived fields the way GSE would want to). Two candidate free public-betting-percentage sites (WiseGuyTeam, Covers.com) both explicitly prohibit redistribution in their own fetched Terms of Use — `permission_required`, not free, matching the NCAA.com precedent from Round 7. GSE's existing Reddit narrative client (`reddit-narrative-source.ts`) is real, Tier-B, and already correctly scoped — extend it, don't duplicate it, if a social-buzz signal is ever built.

### 5. Advanced math for parlay correlation and calibration

The formula-class registry (`GseFormulaClass`: linear/logistic/poisson/tweedie/hurdle/hierarchical_bayes/kalman/hidden_markov/gam_spline/ensemble/conformal) defines 12 categories; grep confirms only 4 are ever actually used in the metric registry — `kalman`, `hidden_markov`, `hierarchical_bayes`, `tweedie`, `hurdle` are placeholders. But real implementations of several of those classes exist elsewhere, unregistered: **the strongest reassurance finding of this round** is that GSE's Elo is fixed-K (a non-adaptive Kalman-gain analogue), but a full, tested, documented bootstrap particle filter (`packages/prediction-engine/src/team-strength-filter.ts`) already implements the state-space model a Kalman filter targets — and does it more correctly, since the observation here is Bernoulli through a sigmoid link, which a textbook linear-Gaussian Kalman filter can't handle directly without an Extended/Unscented approximation. It's already wired into a live shadow-signal comparison pipeline (`shadow-signal-store.ts`), tracking real posterior variance, snapshot-restorable for serverless rehydration. A second, more exotic hierarchical-Bayes particle filter (`research/nb-rbpf.ts`) exists too, research-only. **This is a graduation decision, not a math gap.** Extreme value theory for blowout/upset tails is genuinely absent, but GSE's existing `margin-mixture-model.ts` (Stern-normal + Laplace-smoothed key-number masses, real citations) already targets the same "how much mass sits away from a naive normal" question more directly and with far less data than fitting a GEV/GPD tail would need — building EVT separately would be duplicative over-engineering for a bounded, moderate-variance quantity that isn't actually heavy-tailed.

The real, scoped gap sits in the live (shadow, `priced: false`) Parlay MRI tool: `apps/web/lib/projections/correlation.ts` only computes a normal approximation to the variance of a leg-probability sum, never the textbook per-leg Φ⁻¹ transform + multivariate-normal joint-survival calculation (verified against the standard worked methodology, which shows a 33% relative gap between the naive-independence and true-copula answer on a realistic same-game stack) — and its pairwise correlation constants (0.25 same-game, 0.35 QB-to-pass-catcher) are hardcoded, never fit from GSE's own settled multi-leg history. **Buildable now, low risk (the tool is explicitly unpriced/shadow, doesn't touch MODEL_VERSION):** swap the variance approximation for the real joint-tail calculation (Monte Carlo past 2 legs — GSE already runs seeded deterministic Monte Carlo elsewhere, so the pattern is in-house), and build a real correlation estimator from settled-picks history once volume supports it, replacing the fixed constants. A separate, real, tested bivariate-Poisson same-game-parlay model already exists (`correlationAdjuster.ts`, ported from a verified MIT-licensed reference) but is scoped to soccer goals only — correct as shipped, just the wrong sport for GSE's actual NFL/NBA prop-correlation need; no action needed there.

### 6. Player-prop data sources

Confirmed: GSE's production scoring/settlement pipeline (`scoring.ts`, `settlement.ts`) has zero prop-market awareness today, and the live `/fantasy/props` "Pick'em Edge" tool runs entirely on a hardcoded, explicitly-labeled-illustrative array of invented players and lines — no real prop data reaches any user right now. But this is not greenfield: a real, tested hierarchical-Bayes statistical core (`packages/prediction-engine/src/edge-lab/props-hb.ts` + ~40 sibling files, Gamma-Poisson conjugate shrinkage) and a Shin-devig pricer (`props-priced-edge.ts`) already exist as R&D scaffolding, and — the one piece that's actually wired into the production ingestion cycle — `packages/ingestion-pipeline/src/event-odds-ingest.ts` already fetches real player-prop odds from GSE's existing paid Odds API plan (same vendor, same books, confirmed by the code's own comments, not a new contract) every cycle, gated off by default (`EVENT_ODDS_INGEST_ENABLED` unset — another Law-3 gate, founder-only, and a real budget tradeoff against the same credit pool C-109 already flagged as tight). Grading data readiness varies sharply by sport: NFL is sufficient today (nflverse player-week data already live); MLB has only season-aggregate player sources, no per-game box score; NBA/NHL have no player-level source wired at all, though the generic, already-cleared `espn-boxscore.ts` adapter is untested against their real payload schemas (unverified, not confirmed broken).

**Closest to buildable, still real work even once the flag is flipped:** NFL `player_receptions`/`player_pass_tds` — the exact market the ingestion code already defaults to, the exact count-shaped stat `props-hb.ts`'s math is built for, the one sport with sufficient grading data. Even so, nothing today connects `props-hb.ts`'s posterior to an actual published pick type — a real prop pick type in `scoring.ts`/`settlement.ts` is still new work, flag or no flag.

### 7. Web automation and scraping tooling landscape — a clear, valuable "do not adopt" result

All six tools checked (Firecrawl, Browserbase, Stagehand, browser-use, ScrapingBee, Apify/Crawlee) were fetched and read at the license/docs level, not assumed. Five of six have anti-bot/CAPTCHA evasion as a first-class, vendor-documented feature (Firecrawl's `stealth`/`enhanced`/`auto` proxy modes explicitly named for defeating DataDome/PerimeterX/Cloudflare; Browserbase's own marketing sells stealth mode and CAPTCHA solving; ScrapingBee's entire blog-as-documentation is a how-to library for bypassing exactly those same three systems; Crawlee ships fingerprint-spoofing *by default*, not as an opt-in). Stagehand's open-source core is genuinely clean (verified by direct fetch — no stealth/proxy code anywhere in it), but its natural cloud pairing is the same evasion-oriented Browserbase. None of this is disqualified on a technicality — GSE's own scraping rule bans exactly this class of tool (no proxy rotation to circumvent IP blocks, no CAPTCHA bypass), and this round's fetches confirm the ban is aimed at real, current products, not a hypothetical. Checked and reconfirmed with fresh evidence rather than re-litigated: `@playwright/test` is already installed and already running real visual-regression tests (`apps/web/e2e/visual-regression.spec.ts`, `toHaveScreenshot`, honestly documenting its own WebKit-baseline gap) — nothing in this round's list offers anything relevant to that use case. And a full read of every currently-`approved_*` entry in `source-rights-registry.ts` found that every one of them is a plain JSON/REST/file endpoint already reachable by a normal `fetch()` — there is no source in the registry today that a browser-automation tool would legitimately unlock. **Verdict: nothing to adopt, and this list should not be re-investigated later without new evidence that one of these vendors has changed its core design.**

### 8. Data visualization on public trust surfaces — shipped

`apps/web/lib/calibration/compute.ts` has always computed an exact 95% Clopper-Pearson interval per confidence bucket, but the site's flagship reliability chart (`CalibrationCurve`, rendered on `/calibration` via `ProofExplorer`) plotted only the point estimate — a 30-sample bucket and a 500-sample bucket rendered with identical visual weight, which matters directly for a platform whose whole position is "math you can read." Confirmed GSE uses no charting library at all (every chart on the site, including this one, is hand-rolled inline SVG) — so the fix needed zero new dependency. **Shipped (commit `5d94003`):** wired the already-computed interval through `page.tsx` → `ProofExplorer` → `CalibrationCurve` and drew a whisker (vertical line + end caps) per bucket, reusing the exact y-axis mapping already used for the point marker; 3 new tests cover a bucket with a real interval, one with none yet (null bounds, no crash), and two-bucket scaling. **Not yet fixed, found in the same pass:** `/performance`'s `ReliabilityRow` renders its 95% CI as a text column with `hidden ... sm:inline-block` — the one thing that makes the row's percentage claim honest disappears entirely on mobile, while the percentage itself does not; a real, small, still-open fix (drop the breakpoint class, or better, put the whisker on the bar itself instead of text-only).

### 9. New MCP servers and connectors

Checked the official MCP registry live and found nothing that changes prior rounds' verdicts on sports-data servers, but surfaced two genuinely new, real, zero-production-footprint dev-tool candidates for whoever works on `packages/prediction-engine` R&D: `posit-dev/mcptools` (an R-language MCP server, MIT, maintained directly by Posit/RStudio — the strongest possible provenance) and `finite-sample/rmcp` (54 statistical tools across regression/time-series/Bayesian methods, MIT, active), plus a Jupyter MCP server (`datalayer/jupyter-mcp-server`, BSD-3) for reproducible calibration notebooks — all local machine-config additions, never touching `package-lock.json` or any production path, same class as the already-filed `@playwright/mcp` item. A weather-MCP wrapper and a Yahoo-Fantasy MCP wrapper were also found, real but lower-value. **One new repo needs recording as a hard exclusion, not a governance caution:** `DanielTomaro13/sportsdata-mcp` is real and actively maintained, but since v0.31.0 ships tools (`sportsbet_place_bet`, `tab_place_bet`, `entain_place_bet`, `unibet_place_bet`) that place real-money wagers using supplied credentials — this must never be added to any `.mcp.json`, even scoped to a "safe" tool group, because a misconfigured toggle is the only thing standing between an agent and placing an actual bet. Honest negative: no real referee/officiating-data MCP server exists anywhere — confirmed absent, not missed.

### 10. Internal regression self-audit — confirms the founder's instinct, with one important correction already handled

Spot-checking recent "DONE" claims against actual code: C-106 (zero-sit lane) is confirmed genuinely live (`overduePending 0 of 2627`, `stalePendingPicks 0` in production) — this branch's own AGENTS.md text had understated it as merely "code-complete." C-107 is confirmed exactly as claimed (still `CLAIMED`, `MODEL_VERSION` unbumped at `v5.2.7`, correctly sequenced to the first clean NFL Sunday). The calibration-regression detector wiring from Round 14 is confirmed still live, not reverted by any later commit. `workers/content-publishing` and the weekly transparency-recap draft are confirmed genuinely unchanged since Round 14 — a real admin review UI now exists but its own header comment says it "never sets `publishedAt` ... no publish, no send, no post," so the gap remains fully open across two rounds, not fixed and not new. **The one real regression this round found, and the reason the founder's "somewhat regressing" instinct was correct:** this branch was two commits behind `origin/main`, and those two commits (`21f60ff`, `f533ff4`) *correct* an earlier optimistic read this same file carried — "PROVEN is days away... all four floors pass today" (2026-09-05) does not survive a fuller production sample: calibration eligibility now reads RED, ECE 0.0524 against a 0.05 floor, and the pooled figure was flattering every individual model-version stratum (`v5.2.7`, the version actually serving traffic, measures ECE 0.1089 on its own 245 rows — more than double the floor). This branch has since merged `origin/main` (commit `dabc428`) specifically to bring that correction in rather than let a stale, superseded status keep circulating.

### What actually got built this round

Two real, tested, safe fixes, both zero-new-dependency and outside any frozen/gated territory: the Clopper-Pearson whisker on the public reliability chart (`5d94003`), and the MLB injured-list transaction fetcher (`b4a5a5d`). Everything else is a named, evidenced, scoped proposal — separated explicitly per domain into buildable-now-free-small, founder/legal-only, and honest-nothing-better-exists — for the founder or the owning domain agent to act on next. The single highest-leverage item in the whole round is the "steam"/line-velocity finding in §4: three tested libraries and a full archival pipeline already exist for exactly the signal a factor-model platform would want most, gated behind one environment flag no agent may flip.
