...
Run the local contract validator before changing any AI-output surface:

```bash
npm run evals:contracts
```

This validates eval frontmatter, required sections, and numbered pass criteria. It does not call Claude or score generated outputs yet; the model-output runner will build on top of this contract layer.
