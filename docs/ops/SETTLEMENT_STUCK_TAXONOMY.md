# Settlement stuck taxonomy

**Live (2026-08-06 re-probe):** `overduePending: 0` · settlement **HEALTHY** · free-path date-target + SNAPSHOT on main.  
This note is the **RCA playbook** if overdue climbs again — not a claim that 139 still burn.
...
|-------|--------------|------|
| `deployment.sha` | probe ops truth | **If SHA lags main HEAD, redeploy before matching work** |
| CRON unauth | 401 | Secret present |
| Overdue | probe `settlement.overduePending` | Moat oxygen — 0 = healthy |
...
Redeploy if SHA lags → settle → read RCA Pareto. Matching code only if overdue stuck after healthy deploy.
