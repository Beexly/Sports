...
- Full proprietary metric backlog remains future work, with owner-approved live-route promotion packet, API payload-envelope coverage for composed decision metrics, generated draft card markdown reports, and Market Mirage Score next.
...
Continue the Sunday frontier implementation with API payload-envelope fixture coverage for composed decision metrics:

1. Add synthetic/local payload-envelope fixtures for PWS, GSS, SLRS, QBI, and RVI outputs without route exposure.
2. Prove payload filtering preserves public drivers, blocks protected weights/raw fields, and reports blocked fields with safe reasons.
3. Keep payload-envelope fixtures separate from live `app/api/v1` route implementation.
