...
- draft fence workflow harness for content/API drafts with source-rights, commercial-copy, disclosure, responsible-gaming, API payload-rights, restricted-tracking-data, manual-review gates, and local review packet serialization
...
- Fence plugin path family under `apps/web/lib/fences`, the draft workflow harness, and local review packet serialization exist as pure manual-review gates. Packet rendering and local packet ledger storage remain future work.
...
1. Add local review packet markdown rendering and in-memory packet ledger tests.
...
Continue the Sunday frontier implementation by adding local packet rendering and review-ledger tests:

1. Render `createDraftFenceReviewPacket()` output to markdown.
2. Add an in-memory append-only packet ledger.
3. Add tests proving packet rendering omits protected payloads and ledger append does not approve or publish.
