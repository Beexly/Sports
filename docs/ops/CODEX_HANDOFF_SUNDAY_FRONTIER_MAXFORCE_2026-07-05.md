...
- Full proprietary metric backlog remains future work, with owner-approved live-route promotion packet, deeper role-stability fixtures, deeper decision-window split fixtures, API payload-envelope coverage for composed decision metrics, and Market Mirage Score next.
...
Continue the Sunday frontier implementation with deeper local validation fixtures for the role-stability and decision-window seams:

1. Add synthetic/local split fixtures that stress RVI role-stability and PWS decision-window stability without public/API exposure.
2. Prove split fixtures preserve source-policy posture, lifecycle locks, API locks, and no-bet/calibration/staleness veto semantics.
3. Add API payload-envelope coverage for composed decision metrics if the split fixtures expose new payload shapes.
