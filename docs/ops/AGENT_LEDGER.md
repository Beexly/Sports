...
| C-9 | Genuinely CI-only failures (pass locally, fail in CI): gse-waitlist 6, backfill-independent-trueprob 2, espn-odds-client 1 — almost certainly env divergence between the CI runner and dev containers | — | OPEN | split out of C-7 once the local/CI distinction was established; needs a CI-log diff of the env each suite reads |
...
| L-6 | CLV analysis on the census (the measurement that should have been L-4) | hermes | UNPUSHED | hermes/l6-clv-analysis 830ff6df local; 909/1161 graded pairs; beat-rate 24.86% Wilson [22.16,27.77] vs 52.4%; push grant issued 2026-08-19 |
