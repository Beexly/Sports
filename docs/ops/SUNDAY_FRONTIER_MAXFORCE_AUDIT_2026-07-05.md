...
- `packages/prediction-engine/src/metrics/__tests__/metric-historical-distribution-adapter.test.ts`
  - proves CIG/PFS/DPI historical distribution fixtures adapt only after source and payload rights pass, block raw input leakage, and preserve lifecycle/API/licensing locks
...
1. Continue guarded metric backlog with Source Trust Score or CUW historical adapter coverage after CUW, DPI, no-bet, payload-envelope, source-rights, and historical-adapter veto tests stay green.
