...
- `packages/prediction-engine/src/metrics/__tests__/metric-historical-distribution-adapter.test.ts`
  - proves CIG/PFS historical distribution fixtures adapt only after source and payload rights pass, block raw input leakage, and preserve lifecycle/API/licensing locks
...
- Source/payload-reviewed historical distribution adapters now exist for CIG and PFS. They are local adapters only: source rights must clear first, payload rights must clear second, raw input leakage blocks before metric execution, and public API exposure remains closed.
