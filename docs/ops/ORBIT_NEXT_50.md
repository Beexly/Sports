...
59. arXiv:2606.18805 — reference-dependent emotions → risky post-game driving near stadiums — content/responsible-gaming — skill-doc (bias-mirror + RG content angle). **Content CORRECTED by full-text read:** the robust effect is *predicted-close losses* (+speeding/+crashes in the first hour; NBA null), NOT upset losses (upset coefficients ≈ 0, n=5 — underpowered). Ports: RG "cool-down hour after a close loss" framing (phrased as driving evidence, never bet-chasing proof) + a leak-safe prev-game-close-loss expectation covariate from the line archive. Full extraction: `edge/extraction/2026-08-26-group-sports-domain.md`
60. arXiv:2603.17866 — Bayesian multilevel step-and-turn models on NFL tracking data; simulated counterfactual movement metrics — model — **pattern, SPLIT by full-text read:** portable NOW = opponent (defensive-team) random effects into the Gamma-Poisson props binds + success-rate/tail-exceedance distributional metrics into the CPAE-GAM spec; the step-and-turn core itself is GATED on a cleared tracking-data source (Big Data Bowl licenses are research-only — same gate as 2603.25901). Un-annotated, the old row invited building on uncleared data. Full extraction: `edge/extraction/2026-08-26-group-sports-domain.md`
...
**Full-text deep-read pass (2026-08-26, same session):** every wave-4/5 item is being re-read in FULL (ar5iv/PDF), not just abs pages — outputs in `docs/ops/edge/extraction/2026-08-26-group-*.md` and `docs/ops/edge/2026-08-26-paper-spec-*.md`. Rows corrected by the deep read are marked inline; unmarked verdicts survived full-text scrutiny. The triage's biggest miss: two "NFL-collision ignores" (rows 63, 69) hid update-governance methods that map directly onto the C6 calibration apply/rollback decision.
...
63. arXiv:2403.04146 — FL-GUARD: A Holistic Framework for Run-Time Detection and Recovery of Negative Federated Learni… — **pattern (HIGH) — CORRECTED by full-text read (was ignore)** — Negative-Update Guard for the C6 calibration apply/rollback: per-cohort gain β̂ = incumbent-loss − candidate-loss on the newest settled window (their Eq. 3 cheap surrogate), median across cohorts (Eq. 4), rolling mean over c windows (Eq. 5), rollback alert after NR consecutive negative windows, auto-cancel on recovery — extends `calibration-monitor.ts` (today absolute-Brier-only) + `agent:eval` fleet-regression detection. Their Table 4 (unconditional remedies harm healthy systems; run-time-gated ones cost nothing) is written evidence for GSE's apply-default-OFF posture. Full extraction: `edge/extraction/2026-08-26-group-learning-theory.md`
...
65. arXiv:2303.05774 — NFL Career Success as Predicted by NFL Scouting Combine — skill-doc (medium) — Reference for fantasy projections: cautions agents against weighting Combine/athletic-testing features for long-horizon player value; useful prior on which rookie signals carry projection information. Full-text nuance: production regression fails outright (best R² 0.17); the 0.83 matriculation accuracy has no base rate/AUC reported and is complete-case-only — don't over-trust it either direction.
66. arXiv:2510.07297 — Agentic generative AI for media content discovery at the National Football League — **pattern (medium, ops) — RAISED by full-text read (was skill-doc low)** — the nine-stage blueprint for a read-only query lane over the picks/odds DB: intent gate → decomposition → semantic cache that caches the QUERY PLAN with entity redaction and replays it against fresh data (freshness-law compatible — never the answer) → schema router → bounded auto-correct retries → cite-only summaries; their ≥95%-on-held-out-QA-pairs bar becomes the admission gate. Merge with row 85's eval-harness lane. Full extraction: `edge/extraction/2026-08-26-group-sports-domain.md`
...
69. arXiv:2503.04638 — No Forgetting Learning: Buffer-free Continual Learning Classification — **skill-doc (medium) — CORRECTED by full-text read (was ignore)** — the Plasticity–Stability ratio (their Eq. 29) is the missing regression-guard statistic for the ~250-settled re-fit cadence: a candidate calibration map must not buy recent-cohort gains with early-season-cohort forgetting beyond a bound. Buffer machinery itself does not port (GSE keeps all data). Full extraction: `edge/extraction/2026-08-26-group-learning-theory.md`
...
74. arXiv:1805.01271 — NFL Injuries Before and After the 2011 Collective Bargaining Agreement (CBA) — **skill-doc (medium) — RAISED by full-text read (was low; "no portable method" was wrong)** — the Poisson interrupted-time-series design with exposure offset + player random intercepts ports directly (availability priors; refit on current nflverse — their 2007–2016 rates are stale), and their codified failure modes for injury causal claims (rule-change confounds, reporting drift — 51% of listed injuries excluded) become a check-claims checklist for any injury content. Full extraction: `edge/extraction/2026-08-26-group-sports-domain.md`
75. arXiv:2206.13222 — ML-Based Approach for NFL Defensive Pass Interference Prediction Using GPS Tracking Data — **skill-doc (medium) — sharpened by full-text read** — the rare-event conventions port even though the GPS data doesn't: domain prefilter before modeling, inverse-frequency class weights over synthetic resampling (they reject SMOTE for time series), precision-at-recall operating points in the edge-lab harness; their AUC-0.82-but-precision-0.075 outcome is the canonical base-rate lesson for why props use count models + LCB gates, not classifier scores. Full extraction: `edge/extraction/2026-08-26-group-sports-domain.md`
...
78. arXiv:2606.24443 — Verifiable Auto-Formalization of Mathematics Using a Relaxed Natural Formal Language — **skill-doc (low) — CORRECTED by full-text read (was ignore)** — "NFL" here = Natural Formal Language; its §3 harnessed-elaboration recipe (LLM only picks among rule-generated finite candidates, gated by deterministic checkers) is the documented v2 path for relation-level claim verification in `numeric-guard.ts` (today value-membership only): emit a typed claim IR with prose, discharge it deterministically against the structured payload. Full extraction: `edge/extraction/2026-08-26-group-batch1.md`
79. arXiv:2505.23703 — Let's Reason Formally: Natural-Formal Hybrid Reasoning Enhances LLM's Math Capability — ignore — no fit
80. arXiv:2406.17947 — Do they mean 'us'? Interpreting Referring Expressions in Intergroup Bias — skill-doc (low) — **Content CORRECTED by full-text read:** the "linguistic WP beats numeric" claim the triage encoded is GPT-4o-few-shot-only and NOT significant under bootstrap — do not cite it as a finding. What does hold: GPT-4o treats win probabilities 1%–41% as uniformly "low" (supports the code-computes/LLM-narrates rule), yielding two eval-case families (probability-band consistency; complement inference) for the numeric-fidelity harness. Full extraction: `edge/extraction/2026-08-26-group-batch1.md`
...
82. arXiv:2606.28570 — Digitizing Coaching Intelligence: An Agentic Framework for Holistic Athlete Profiling using VLM — **skill-doc (medium) — RAISED by full-text read (was low)** — the concrete port is its fail-closed LLM-as-a-Judge recipe (enforced schema with confidence/justification/contradiction tags, acceptance gate, bounded feedback-retry, judge failure NEVER default-approves) for cross-referencing prose vs structured payload between `build-draft.ts` and `persist-draft.ts`. Adopt the pattern, not the results — the paper's own numbers are internally inconsistent. Full extraction: `edge/extraction/2026-08-26-group-batch1.md`
...
85. arXiv:2508.17157 — SPORTSQL: An Interactive System for Real-Time Sports Reasoning and Visualization — pattern (medium) — **RE-AIMED by full-text read:** the port is the *eval design*, not the feature — snapshot-pinned gold SQL fixtures, primitive-sliced accuracy reporting, byte-for-byte output≡query gate into `scripts/agent-eval/`. Its own results (~50% accuracy at ≥3 primitives, ~15% on joins/aggregation) BLOCK the user-facing Trend Lab NL-querying feature the triage proposed; that cliff becomes the admission gate any future NL feature must pass. Full extraction: `edge/extraction/2026-08-26-group-batch2.md`
...
88. arXiv:2607.12089 — Cross-Cutting Security Analysis of LLM-Generated Code via Metamorphic Testing and Association R — **skill-doc (medium, ops) — RAISED by full-text read (was low)** — cluster-aware checklist ordering for audit-secrets/audit-stripe/security-review (their co-violation mining: hard-coded-credential findings co-occur with crypto 57% / authz 35% — check those next); DB/auth prompts are the top risk class (14.2×); "write secure code" phrasing has zero measured effect (r=−0.04) — cited doctrine for post-generation verification over prompt hygiene; SHA-256-frozen-judge provenance pattern for `agent:eval`. Full extraction: `edge/extraction/2026-08-26-group-batch2.md`
...
91. arXiv:2608.18430 — Multi-Level Bayesian Calibration of a Multi-Component Dynamic System Model — pattern (medium) — **re-scoped by full-text read:** the port is partial pooling for thin sport×market calibration cells that never reach 250 settled (local/global parameter split + EM-like alternation, their Alg. 1) and posterior→prior chaining (Eq. 7) feeding `online-beta-recalibration.ts` between refits; their own figures show offline refit stays the gold standard. Gate: per-cell held-out ECE non-degradation + the C6 flip rule. Full extraction: `edge/extraction/2026-08-26-group-batch3.md`
...
93. arXiv:1906.05029 — A Bayesian Approach to In-Game Win Probability in Soccer — pattern (high) — **confirmed + sequenced by full-text read, 3 lanes:** (1) NOW: phase-bucketed ECE audit (their H1/H2/final-10% split) into `calibration-monitor.ts`; (2) NEXT: `ingame-soccer.ts` extending existing `dixon-coles.ts`/`poisson.ts`/`elo-estimator.ts` — future-goals Poisson with random-walk time-varying coefficients and their §4.4-validated simplified feature set (score/cards/Elo — no event-stream rights needed); (3) GATED: in-play value surface, on a cleared live-odds source. Edge-lab law, priced:false. Full extraction: `edge/extraction/2026-08-26-group-batch3.md`
