...
- [ ] Explicit YES: public picks (`PUBLIC_PICKS_ENABLED`) — observed ON in production 2026-09-02 (`gates.canExposePublicPicks: true`), but an observation is not a founder YES: stays unchecked until the founder records YES and `FORCE_NO_BET_IF_STALE=true` is confirmed in production (gate runbook 1b). The truth surface now reports this flag as `gates.forceNoBetIfStale` (`apps/web/app/api/ops/public-surface-truth/route.ts`), and `npm run launch:ready` prints a "stale-data kill switch" verdict from the same value — read it there (or via `launch:ready`) instead of a manual Vercel lookup; a Vercel lookup is still how you CHANGE the value.
