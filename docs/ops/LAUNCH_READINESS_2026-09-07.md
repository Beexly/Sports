...
| **C-137** | the remediation tool itself | 87 / 148 / 352 rows still live, which is **585 unique rows, not 586 or 587** [M]. CodeRabbit flagged that these figures did not reconcile and was right that neither candidate was correct: 87+148+352 sums to 587 because settled-before-kickoff and the off-ladder run lines **overlap on exactly 2 rows** (the other two pairs are disjoint), and my earlier 586 was simply wrong. Measured read-only 2026-09-07 22:1x UTC. Soccer and off-ladder populations are safe to run; **settled-before-kickoff is not**, per above. |
