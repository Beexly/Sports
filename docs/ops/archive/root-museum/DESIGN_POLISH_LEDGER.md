...
| 24 | Final visual-QA sweep | cross-page consistency re-check, ledger close-out | [x] | homepage hero eyebrow 0.28em -> 0.22em (only true one-off tracking value, matches the established page-eyebrow tier); bg-plasma solid pills using text-eclipse -> text-plasma-ink on picks + cockpit/history (6 instances) to match the documented plasma-CTA idiom; eyebrow tracking otherwise confirmed as a legitimate graduated multi-tier system (0.12/0.14/0.16/0.18/0.2/0.22/0.24/0.3em each consistently reused across many files by nesting depth/role), left alone; cockpit/admin H1s all confirmed text-2xl (only text-3xl hits left were stat-value numerals, not headings); raw-color sweep (incl. ring-offset-*) found only the 3 already-documented ALLOWLIST exceptions, no new drift. Flagged, not fixed (out of this cycle's scope, needs a token decision): solid bg-orbital-cyan/bg-ultraviolet buttons use 4 different ink tokens across cycles (text-eclipse, text-carbon, text-obsidian, text-ion-blue-ink) for the same role - a future cycle should pick one canonical ink per accent color the way plasma now has text-plasma-ink. |
...
- 2026-07-23 13:35 — Cycle 24 (final visual-QA sweep) done: 3 files fixed (app/page.tsx, app/picks/page.tsx, app/cockpit/history/page.tsx). Full suite: typecheck clean, lint clean (0 warnings), vitest 667 test files / 9,394 tests passed, 5 files / 68 tests skipped, 0 failures. Loop complete.
