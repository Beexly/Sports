...
decision.** Ledger C-276.

> **Renumbered four times, 2026-09-09.** Filed as C-262, then C-265, then C-267, now **C-276**. While this branch was open, PR #725 merged
> to `main` carrying its own C-261, C-262 and C-263 for unrelated work, so the three rows added
> on this branch were renumbered C-274, C-276 and C-275 when `main` was merged in. `main`'s rows
...
> so this row moved again to C-276. Its siblings C-274 and C-275 were not affected either time.
> Commit messages and PR comments written before those merges still say C-261 / C-262 / C-263 or
> C-265; they mean these rows.
>
> The fourth move came from PR #734, which took C-264, C-266 AND C-267 - all three ids this
> branch had just settled on - while also porting this branch's own C-244 satellite fix to main
> under the same id, so the merged file briefly held two C-244 rows for one task. This branch's
> three rows are now C-274 (team game log re-queue), C-275 (free-path no-op) and C-276 (this
> one), and the duplicate C-244 was dropped in favour of main's ported row, which cites this
> branch's commit 49074b9 and PR #720 as its source so no trail is lost.
>
> **Four collisions in under a day, all on the same file.** Ledger ids are allocated by reading the
