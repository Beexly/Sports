...
`../INDEX.md` (doc map) · `CURRENT_STATE` · `CALIBRATION_STATUS` · `AGENT_LEDGER` · `LAST_PLAN_2026-09-15` · `SESSION_LOG` · `OPEN_LEDGER` · `CRON_MATRIX` · `SMOKE` · `CREDENTIALS_CHECKLIST` · `FOUNDER_ONLY_CHECKLIST` · `FOUNDER_HANDOFF_MESSAGE` · `CLAUDE_COWORK_PROMPT_P0` · `JARVIS_COCKPIT_AUTO_RUN` · `CALIBRATION_PIPELINE` · `CALIBRATION_PUBLISH_CHECKLIST` · `FREE_FIRST_DATA` (under docs/) · runbooks that still gate go-live (`GO_LIVE_RUNBOOK`, `GATE_OPENING_RUNBOOK`, `STRIPE_GO_LIVE_CHECKLIST`, `PHASE_05B_REVEAL_PROTOCOL`, `INDEPENDENCE_GATES`, etc.)

Do not create another START_HERE / MASTER_PLAN / READ-ME-FIRST. `MASTER_PLAN_INDEX` is a redirect to this file.
