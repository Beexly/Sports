...
| W003 | DONE (2026-07-17) | Reality Receipt v0 | W002 ✓ | Reproducible visual/decision object — landed `apps/web/lib/reality-receipt` composing the W001 envelope digest + receipt tamper-check + W-OTS Bitcoin-anchor status into one object; `GET /api/proof/reality/[gameId]` (JSON) + `/image` (PNG); FREE-tier-only fail-closed loader (DEC-015); 31 tests | proof, public claims |
