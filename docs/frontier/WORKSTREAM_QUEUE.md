...
| W-OTS | DONE slices 1+2 (2026-07-17) | Bitcoin-anchored trustless time | founder packet | Slice 1: verbatim port, LIVE calendar+python checks, migration+flag. Slice 2: fail-open mint wire (post-transaction, never a freeze failure mode) + /api/proof/ots/[slateKey] (raw .ots + ?format=json) + llms.txt discovery. Slice 3 QUEUED: nightly calendar upgrade poll (needs an upgrade fetch added to the crypto module, new-tests-first) | proof, data |
