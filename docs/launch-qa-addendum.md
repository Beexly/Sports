...
- [ ] **SHOULD** — Calibration computation runs on a schedule. It does, as the
      `calibration-metrics` Vercel cron — there is no queue or worker process
      behind it. Confirm the route is still scheduled in `apps/web/vercel.json`
      and read the live cadence from `docs/ops/CRON_MATRIX.generated.md`.
...
- [ ] **MUST** — One real paid-path ingestion cycle completes. **No local script
      does this** — the previously named `smoke:ingestion` has never existed as an
      npm script. Drive the deployed route instead, the same way
      `docs/ops/OPERATOR.md` drives settle-picks:
      `curl -sS -H "Authorization: Bearer $CRON_SECRET" "$APP_URL/api/cron/refresh-odds"`.
      Then verify in the DB: ≥1 SourceSnapshot row, ≥1 Game row, ≥4 GameSignal
      rows (one per activated market factor), ≥1 Pick row.
      (Free-path equivalent, no key and no spend: `npm run free:doctor`.)
...
- [ ] **MUST** — `/api/health` returns 200 with its sub-checks green. The route
      reports exactly two: `database` and `ingestion`
      (`apps/web/lib/health/live-capability-probes.ts`). It has no cache-server
      probe and no per-provider probe — do not wait on sub-checks it never emits.
...
| `0 ingestion runs in last cron interval` | scheduled cron not firing (or firing and failing auth) | there is no worker process to restart — check the Vercel cron logs for `/api/cron/refresh-odds`, confirm the route is still scheduled in `apps/web/vercel.json`, and confirm `CRON_SECRET` is set (unset ⇒ every cron route 500s). Re-fire by hand: `curl -sS -H "Authorization: Bearer $CRON_SECRET" "$APP_URL/api/cron/refresh-odds"` |
