...
| Season resolution | `packages/data-ingestion/src/nflverse-season.ts` (season floor at `:32-46`, probe contract at `:65-67`, REG-row probe at `:125-136`) | floors at the last completed season unless a database probe is supplied; only `refresh-player-stats` passes the probe, so read-only surfaces show the prior season all year |
...
### Track G: customer surfaces and the certification boundary

**Charter.** The one line: no uncertified probability reaches a customer, everything up to that line runs free.

**Starts on day one, with no founder action and no dependency:**

- The factor-breakdown allowlist fix (G1), closing a real, verified, present-tense leak of an uncertified probability-shaped number into every PRO and ELITE subscriber's raw API response.
- The score-not-percent correction on the Ranking P and Independent-edge trueProb render blocks (G2), the highest-leverage single change on this track, reusing a pattern (pick-card.tsx:563-565) already proven correct elsewhere on the same card.
- The ranking-key rewrite and the pass-decision display filter (G3), using fields already present on every row today.
- The reason-code vocabulary module (G5), fully testable against the documented code list with no gate_decisions writer yet in place.
- The interval-display contract's empty state and its exposure counter (G6), so the sample of who even looks is not lost to the weeks before a head exists.
- The shadow-disclosure component's type shape (G7), built inert and structurally incapable of carrying a number.
- The signal-display contract and the first provenance strip (G8), composing fields already on the wire.
- Basis tagging on every public-calibration-page bucket (part of G2), so the partition a future bump needs is being written from day one.

| ID | Workstream | Concurrent | Owner | Entry files | Acceptance |
|---|---|---|---|---|---|
| G1 | Public factor-breakdown allowlist at the serialization boundary | yes | agent | `apps/web/lib/picks/parse-factor-breakdown.ts`, `apps/web/app/api/picks/route.ts`, `apps/web/lib/calibration/proven-path-rows.ts`, `apps/web/lib/picks/public-factor-breakdown.ts` | A new vitest fixture asserts a stored factorBreakdown blob carrying fairProbability and an arbitrary unknown key never appears in the serialized /api/picks response for a canSeeFactorBreakdown viewer, while factors, consensusScore, independentEdge, rankingP, and marketFairProb still do; npm run typecheck and npm run lint clean. |
| G2 | Score-not-percent correction on the three live uncertified-probability renders | yes | agent | `apps/web/components/picks/pick-card.tsx`, `apps/web/components/picks/value-gap.tsx`, `apps/web/lib/calibration/compute.ts`, `apps/web/lib/calibration/public-confidence.ts` | A render test on ConfidenceBadge, the Ranking P block, and the Independent-edge block asserts no percent sign and no aria-label containing probability, percent, or win rate renders for any uncertified source, under both CALIBRATION_ADJUSTMENTS_ENABLED states; a compute.ts test asserts every bucket carries the new basis field. |
| G3 | Ranking-key rewrite and the pass-decision display filter | yes | agent | `apps/web/lib/ranking/sort-key.ts`, `packages/types/src/index.ts`, `apps/web/lib/picks/adverse-edge-suppression.ts`, `apps/web/app/api/picks/route.ts` | A fixture test replays the 2026-09-13 MLB slate and asserts the D-backs row (largest edge, +0.2257) sorts before the Yankees row (smallest positive edge, +0.0217, highest confidence at 91); the nine known adverse rows and the PASS-with-nonnegative-edge specimen never render on either route.ts or state.ts; the 22-file partial-mock suite stays green. |
| G4 | Market-implied display extension to SPREAD and TOTAL | yes | agent | `apps/web/lib/picks/market-implied-display.ts`, `apps/web/components/picks/pick-card.tsx`, `apps/web/app/api/picks/route.ts` | New tests cover a real two-book SPREAD pick'em (both -110) asserting the probability still renders at 50 percent rather than being suppressed as the sentinel, and a TOTAL pick with two books rendering correct over/under language; existing MONEYLINE tests stay green. |
| G5 | Reason-code vocabulary module | yes | agent | `apps/web/lib/board/passes.ts`, `apps/web/lib/board/pass-reason.ts`, `packages/db/prisma/schema.prisma`, `apps/web/lib/picks/reason-codes.ts` | A table-driven test asserts every code in the vocabulary maps to a string under 240 characters, containing no banned phrase and no em dash checked against apps/web/lib/positioning-vocab.json, and that NoBetDetail.reasonCode round-trips through the mapping unchanged. |
| G6 | Interval-display contract and the three unwired honesty-surface entitlements | yes | agent | `packages/types/src/index.ts`, `apps/web/app/board/page.tsx`, `apps/web/app/api/board/passes/route.ts`, `apps/web/lib/board/passes.ts` | A test asserts the component renders the empty-state string and no numeric interval when its input is null, and renders lo, hi, width, method, and n verbatim when given a well-formed interval; a second test confirms cqr.ts refuses rather than clamps at n below the required rank. |
| G7 | Shadow-head existence disclosure | yes | agent | `apps/web/components/picks/pick-card.tsx`, `apps/web/components/picks/shadow-head-disclosure.tsx` | A prop-shape test confirms the component's props contain no field named or typed as a probability, percent, or confidence number; a render test confirms it never renders when the existence flag is false. |
| G8 | Signal-display contract and the first provenance strip | yes | agent | `apps/web/app/api/picks/route.ts`, `apps/web/components/picks/pick-card.tsx`, `apps/web/lib/picks/public-edge-score.ts`, `apps/web/lib/picks/teaser-text.ts` | A test asserts the provenance strip renders only from already-serialized fields with no new API field added, and a second test mirrors public-edge-score.ts's own leak check by asserting no new display value is arithmetically invertible into a gated quantity for a viewer who lacks it. |
| G9 | Founder voice and product review of every new customer-facing string | yes | founder | `apps/web/lib/picks/reason-codes.ts`, `apps/web/components/picks/shadow-head-disclosure.tsx`, `apps/web/lib/picks/interval-display.ts`, `docs/positioning.md` | npm run lint:brand and the trust-gate guard both green on the exact shipped strings, plus a written founder approval quoted in the pull request; the shadow-disclosure component ships default-off until that approval names it on. |

**What each one builds.**

- **G1, Public factor-breakdown allowlist at the serialization boundary.** A new toPublicFactorBreakdown() reconstruction function called at the API response boundary that whitelists only the fields the pick card and the public FactorBreakdown contract need, dropping fairProbability and any unrecognized key before the object reaches the client; parseFactorBreakdown itself is left untouched so internal server-side consumers keep full-fidelity access. Risk: Low. Purely subtractive at one boundary; grep across every .tsx file confirms no component reads fairProbability today, so nothing rendered depends on it leaking.
- **G2, Score-not-percent correction on the three live uncertified-probability renders.** Rewrites pick-card.tsx's Ranking P block and Independent-edge trueProb block to the same NN/100 score convention the confidence fallback already uses instead of a percent sign, removes the win-probability aria-label from the dormant isotonic-calibrated branch regardless of CALIBRATION_ADJUSTMENTS_ENABLED's state, and tags every bucket compute.ts's expectedFromConfidence produces with an explicit basis label. Capture that starts because of this row: Every public-calibration-page bucket gets an explicit basis tag, confidence_heuristic_v1, starting at ship, so nothing before the eventual head-basis bump is retroactively ambiguous. Risk: Medium. Touches a rendered, paying-tier surface (Ranking P and Independent edge are PRO/ELITE-visible today), so the visual change is real even though the underlying number is unchanged; a founder glance before wide release is prudent given rule 8's brand stakes, though not strictly required.
- **G3, Ranking-key rewrite and the pass-decision display filter.** Replaces sort-key.ts's rankingP-then-confidence fallback chain with independentEdge.expectedClv descending, tie-broken by trueProb-minus-marketFairProb, confidence never in the chain; adds isIndependentEdgePass beside pricesWorseThanMarket in @sports/types and a sibling display filter, pass-decision-suppression.ts, withholding a PASS-decision row whose expectedClv is non-negative the same way adverse-edge-suppression.ts already withholds a negative one. Risk: Medium. Reorders a customer-visible board; accompanying copy must say sorted by estimated value, not yet verified per Question Zero's open status, not imply a proven edge.
- **G4, Market-implied display extension to SPREAD and TOTAL.** Extends resolveMarketImplied, resolveWinProbability, and formatMarketImpliedLabel to accept SPREAD and TOTAL pickTypes with pickType-aware wording (cover probability for spreads, over/under probability for totals) instead of win-probability language, verified against a fixture test with two real -110/-110 SPREAD books to confirm the exact-0.5 sentinel guard does not wrongly suppress a genuine pick'em cover probability. Risk: Low, contingent on the fixture check above; the receipt already carries a generic marketFairProb for every pickType, so no new data path is needed.
- **G5, Reason-code vocabulary module.** A pure, fully tested mapping from every documented gate-decision reason code to one customer sentence in the founder's approved voice, capped to fit GateDecision.reason's 240-character column, built and shippable before gate_decisions has a writer. Capture that starts because of this row: The module's own code list becomes the queryable frequency table the moment any writer starts producing rows; nothing further to build on this track's side. Risk: Low. No writer dependency; starts working the moment gate_decisions gets one elsewhere.
- **G6, Interval-display contract and the three unwired honesty-surface entitlements.** A lo/hi/width/method/n/stratum interval renderer that refuses rather than clamps when its inputs are absent, wired for the first time to canSeeMultiprob, canSeeGlassLedger, and canSeeRecompute, rendering an honest empty state until a head supplies real bounds, plus a lightweight exposure counter on this panel and the existing No-Bet detail panel logging views from first render. Capture that starts because of this row: Panel-view exposure counts start logging at ship, independent of whether a head ever populates the panel, so the founder can later ask whether this surface changed subscriber behavior. Risk: Low for the empty state; the cqr.ts refusal fix is a narrowing of intent per the existing test file's own comment, not a weakened guard.
- **G7, Shadow-head existence disclosure.** A disclosure component whose prop type has no numeric field at all, so a probability cannot be wired into it by a future edit, rendering a single honest existence line only when an active flag and metadata (sport, market, as-of date) are present. Risk: Low; ships inert until another track's shadow-serving step exists. This component's type shape plus G1's allowlist are the two things that make that future step safe to land without a second review.
- **G8, Signal-display contract and the first provenance strip.** A generic label/value/source/n/asOf/caveat display contract, certification-exempt by design, and one real instance composing bookmakerCount, generatedAt age, and dataQualityScore, all already serialized on every pick, into one honestly sourced line, replacing three disconnected badges. Risk: Low; purely additive composition of existing wire fields, no schema or ingestion change.
- **G9, Founder voice and product review of every new customer-facing string.** A recorded founder approval of every new string this track introduces, the reason codes, the interval empty state, and the shadow-disclosure line, against the sharp-friend test, plus the founder's product decision on whether the shadow-disclosure ships at all. Risk: None to the codebase; this is a review gate, not a build.

**Invariants that must survive every future edit to this track:**

- A number may render, be labelled, or be serialized as a probability, a percent, or calibrated only if its claimant is the receipt's market de-vig (PickProofReceipt.marketFairProb, schema.prisma:616) or a certified head's PickProofReceipt.modelProb (schema.prisma:619) for that stratum; every other claimant renders only as a bounded score, never with a percent sign.
- The picks API and the board state module share one ranking function (apps/web/lib/ranking/sort-key.ts, imported at apps/web/app/api/picks/route.ts:22 and apps/web/lib/board/state.ts:23); confidence never enters it, at any level, including as a fallback.
- No key on Pick.factorBreakdown reaches the client response unless it is explicitly allowlisted at the serialization boundary; parseFactorBreakdown's full-fidelity pass-through (apps/web/lib/picks/parse-factor-breakdown.ts:53) stays available to internal, server-side-only consumers such as apps/web/lib/calibration/proven-path-rows.ts:146.
- A display of any signal needs a source and an n, not a certified head; certification gates probabilities, not facts with provenance.
- An interval refuses to render rather than clamping when its sample is too thin; it never appears without its method name and its n beside it.
- A shadow model's existence may be disclosed with metadata only; its numeric output has no path to any customer-facing prop, type, or serialized field.
- Every server-side redaction of a premium quantity happens at the query or serialization layer, matching the existing GSE-SEC-026 pattern in apps/web/lib/board/state.ts:301-302, never only in a client component.
- Every new customer string is em-dash-free, checked against apps/web/lib/positioning-vocab.json, and under GateDecision.reason's 240-character bound (schema.prisma:670) where it feeds that column.

The certification boundary is a single predicate evaluated at the moment a number is about to be rendered, labelled, or serialized as a probability, a percent chance, a win rate, or the word calibrated, and nowhere else. A number may cross it only if its claimant is one of exactly two kinds. The first is the market: the receipt's committed de-vigged price for the side taken, PickProofReceipt.marketFairProb, a required, non-null Float column (schema.prisma:616), which is not our model's claim and is exempt by construction because it is public arithmetic anyone holding the same book prices can recompute, exactly the reasoning already recorded in apps/web/lib/picks/market-implied-display.ts:13-29. The second is a certified head: PickProofReceipt.modelProb, a nullable column that already exists (schema.prisma:619), is already committed into the tamper-evident receipt hash with an explicit none sentinel when absent (pick-proof-receipt.ts:128), already asserts real-probability shape when present (pick-proof-receipt.ts:181-183), and is written as null at every mint today (packages/ingestion-pipeline/src/process-sport.ts:1446, :1461). Every other claimant, raw heuristic confidence, the independent estimators' blended trueProb, and the ranking probability rankingP that blends the two (packages/prediction-engine/src/ranking-prob.ts:42-107), fails the predicate today, unconditionally, and must render only as a bounded score, never with a percent sign, never inside an aria-label containing the words probability, percent, or win rate, and never as the forecast basis of a calibration bucket. This is deliberately narrow. It says nothing about what the engine may compute, store, log, or test against settled outcomes, only about the one moment a number is handed to a person who did not build it.

Measured against that predicate, Phase 0 is not a clean slate to design toward, it is three live violations to close and one correct pattern to extend. On every PRO or ELITE pick card, pick-card.tsx:349-366 renders a block labelled Ranking P with (breakdown.rankingP * 100).toFixed(1) plus a percent sign, a raw percentage of the same rankingP the type's own comment (packages/types/src/index.ts:112-115) describes as always a probability, never edge, rawEdge, or edgeScore, and section 1 of this document already measures over confident through the upper middle, n 1,390, top band 0.8934 claimed against 0.8286 realized. A few lines below it, pick-card.tsx:381-406 renders a second block labelled Independent edge with a literal trueProb percentage line whenever independentEdge.trueProb is finite, which on the 2026-09-13 NFL slate was a single-source Elo estimate with no market comparison for every signal pick. Both blocks already carry a caption trying to hedge the claim, model ranking key, not verified ROI on the first, signal only on the second, but neither withholds the percent sign, and a customer reads the percent, not the caption. The codebase already knows the right fix and has already applied it exactly once: the plain confidence badge's uncalibrated branch renders confidence over 100 (pick-card.tsx:563-565), not a percent, with a comment stating plainly that a percent sign reads as a win probability, which this number is not. That same function's other branch, reached only when CALIBRATION_ADJUSTMENTS_ENABLED is true, renders a calibrated percent with an aria-label stating about N percent win probability (pick-card.tsx:547-551) off an isotonic map of the identical anti-predictive raw score, which section 1 already shows a monotone map cannot repair. Phase 0's first and highest-leverage act is not a new feature, it is applying the pattern the confidence fallback already proves, a bounded score instead of a percent, uniformly to rankingP, to independentEdge.trueProb, and to the dormant calibrated branch, with zero dependency on a certified head and zero new infrastructure. A fourth site never renders visually but still needs the same discipline: apps/web/lib/calibration/compute.ts's expectedFromConfidence (:260) feeds confidence as forecast into every public Brier, ECE, and skill number on the public calibration page. Phase 0 tags every bucket it produces with an explicit basis, confidence_heuristic_v1, so the partition a certified head will eventually need is being written from day one instead of reconstructed after the fact.

A fifth site never renders at all and is more serious for exactly that reason. factorBreakdown.fairProbability, written at mint as the ranking probability whenever an independent estimator priced the pick and null otherwise (packages/prediction-engine/src/scoring.ts:695, :1289; null unconditionally on TOTAL at :946; the ranking probability again on the signal path at packages/ingestion-pipeline/src/generate-signal-slate.ts:470), reaches every PRO and ELITE viewer's raw /api/picks JSON today whenever an independent estimator priced the pick, the common case on the signal path and a frequent one on the book path, because parseFactorBreakdown validates only the required scalar scores and the factors array shape before returning the stored object by reference (apps/web/lib/picks/parse-factor-breakdown.ts:45-53, the pass through itself at :53), and the parsed object is spread whole into the response whenever canSeeFactorBreakdown is true (apps/web/app/api/picks/route.ts:245-247, :337). No component reads it, a search across every tsx file in the app returns nothing, so no customer has been visually misled by it, but the wire is part of what reaching a customer means under a plain reading of the founder's one line rule, and a paying subscriber's own network inspector already carries an uncertified, probability shaped number sitting beside the receipt's honest marketFairProb. This is also the exact hazard a shadow head must not repeat: writing a shadow head's output into the factor breakdown as an extra key, on the current serializer, is a same day leak, because nothing between mint and the wire enforces an allowlist. This track's fix is a new function at the response boundary, not a change to parseFactorBreakdown itself, since apps/web/lib/calibration/proven-path-rows.ts:146 legitimately reads the fairProbability field server side for calibration sampling and must keep full fidelity access. The new function reconstructs only the fields the public card contracts for and drops fairProbability and any unrecognized key before the object reaches the response. A shadow head's existence, once one is running, is disclosed the way a research budget is disclosed to an investor, as a fact with metadata and no number: active, sport, market, as of date, rendered as one honest line, we are testing a second read on some markets, it never changes what we publish until it is proven. The disclosure component's own prop type is built to physically reject a probability field, so a future engineer cannot wire a number into it by accident.

The ranking rule is the same predicate applied to order instead of to display, and it has the same current violation. apps/web/lib/ranking/sort-key.ts:11-28 derives its sort key from rankingP, then rankingScore, then falls back to raw confidence divided by 100 (:17, :28) whenever the factor breakdown is missing or unparsed, and both apps/web/app/api/picks/route.ts:22 and apps/web/lib/board/state.ts:23, :859-860 import the identical comparePicksByRanking, so one fix reaches both surfaces. On the last measured slate this fallback chain is why confidence 91, Yankees minus 1.5, led a board carrying the smallest positive edge on it, plus 0.0217, while confidence 85, D backs minus 1.5, carried the largest, plus 0.2257, and sat third. Phase 0's rule replaces the key with independentEdge.expectedClv where finite and positive, descending, tied by independentEdge.trueProb minus marketFairProb, following the same subtraction apps/web/components/picks/value-gap.tsx already performs and already labels as a disagreement with the market rather than a probability, then by recency. Rows without a finite positive value trail as an undifferentiated block, and confidence never appears in the chain at any level, including as a last resort. This needs nothing from any other track: every field it reads already exists on every row today. A second, closely related fix belongs beside it: apps/web/lib/picks/adverse-edge-suppression.ts withholds a row only when its expectedClv is negative, so a PASS decision row whose expected value happens to be at or above zero, the exact specimen this document's standing notes name, is not caught by it and still displays. A sibling display filter, pass-decision-suppression.ts, importing a new isIndependentEdgePass predicate placed beside pricesWorseThanMarket in packages/types/src/index.ts:86-90 rather than restated, closes this the same way, before ranking runs, in both route.ts and state.ts, writing nothing and settling every suppressed row honestly into the record exactly as adverse-edge-suppression.ts already does. The existing document sequences the ranking fix behind Step 5a's mint time withhold rules. That dependency is conventional, not proven. A corrected sort of already published rows is safe and strictly better than sorting by a measured inverted score regardless of whether mint time rules have shipped, because the coherence and adverse edge filters that run before ranking today are already independent of Step 5a. The one real caveat is interpretive, not technical: expectedClv's thresholds are hand set and its information content beyond the market is Question Zero's open question elsewhere in this document. Until that resolves, any copy describing this order must say sorted by estimated value, not yet verified, rather than implying a measured edge, so the fix ships today without overclaiming what it proves.

The reason code vocabulary is a pure mapping from a machine code to one sentence, and it can be built and fully tested today against the documented code list without gate_decisions having a writer yet, because the mapping function's contract is the string, not its source. It lands beside the existing NoBetDetail shape (apps/web/lib/board/passes.ts:27-38), which already carries a reasonCode, a confidence field explicitly typed as a diagnostic and not a performance claim, a modelVersion, and an evidenceRefCount, gated server side on Entitlements.canSeeNoBetDetail (packages/types/src/index.ts:198), and it respects the schema's own bound: GateDecision.reason is a varchar of 240 characters and reasonCode is a varchar of 80 (schema.prisma:670, :671), so every string in the table is written to fit inside 240 characters, no exceptions. In the founder's approved voice: no books maps to not enough sportsbooks are pricing this game yet, thin data or an uncertified stratum both map to we have not scored this game yet, every edge refusal, including the new pass with nonnegative edge code, maps to we passed, no edge at this price, a kicked off fixture maps to this one is under way, and an injury veto maps to we passed, a key player's status changed. None of these repeat a banned phrase and none use an em dash. The standing FIELD copy rule that a certain retired word reads as a defect and we passed reads as a finding is carried forward unchanged. GateDecision.isBootstrap defaults to true at the schema level (schema.prisma:675), so this mapping module, and every reader built on top of it, must set it explicitly false on every real write it touches or it silently vanishes from every lane that filters on it, the exact failure mode the existing gate_decisions writer gap already demonstrates.

An interval is shown honestly by never showing a point estimate with more precision than its own width allows, and today the platform shows no interval to any customer at all, which is itself the thing to say plainly rather than paper over. Three entitlement flags already exist in packages/types/src/index.ts:197-200, canSeeMultiprob, documented as interval bounds and width on a decision, canSeeGlassLedger, and canSeeRecompute, all resolved correctly per tier by getEntitlements and pinned by packages/types/src/__tests__/entitlements.test.ts:123-161, and none of the three has a single consuming call site anywhere in apps/web. Only their sibling canSeeNoBetDetail is ever read, at apps/web/app/board/page.tsx:59 and apps/web/app/api/board/passes/route.ts:41. They are paid access grants to a room that has not been built. The room's contract, once a certified head exists, is the lo and hi shape already used by the diagnostic abstention helpers (packages/prediction-engine/src/certificate/selective-abstention.ts:9-12), rendered with its method name and the stratum's n beside it, refusing to render rather than clamping when the sample is too thin to support the requested coverage, which is also the fix apps/web/lib/calibration/cqr.ts:12-15 needs for numeric lines, props and margins, and a hard requirement, never a clamp, for whatever produces the binary head's interval. This track's work is the contract and the empty state, buildable today: canSeeMultiprob gates a real component that reads for a certified bound on the relevant stratum and, finding none, renders we do not have a certified range for this market yet rather than nothing, so a paying subscriber learns the room exists before it has furniture in it. Because no interval reaches anyone today, instrumenting who even opens this panel from day one is worth doing now, not after a head ships: a lightweight, privacy safe exposure count on the No Bet detail panel and this new panel, logged from first render, is the only way the founder will later be able to ask whether investing further here changed anything, and deferring that log for weeks is exactly the sample this document's own capture doctrine says can never be bought back.

A display is not a probability and does not need certification, it needs a source and an n, and conflating the two rules is why this track's do not list is longer than its build list would otherwise require. The precedent already exists and is already correct: apps/web/lib/picks/public-edge-score.ts withholds the free Edge Index from a signal slate row shown to a viewer who cannot see confidence, because for that row edgeScore equals confidence minus 50 exactly and serving it would hand back the gated number through one subtraction, a live leak measured on 2026-09-05, edgeScore 18 read back as sixty eight percent, and apps/web/lib/picks/teaser-text.ts strips any at sign percent pattern from free tier prose for the same reason. The rule this generalizes to is: a display may show any T1 or even T2 value, source dated and sample sized, to any tier a business decision assigns it to, provided it is not itself invertible back into a quantity that tier is not entitled to, and provided its label never claims to be a probability. This track's deliverable is the contract, label, value, source, n, as of, an optional caveat, and one real instance built today from fields the wire already carries: a provenance strip composing bookmakerCount, the age of generatedAt, and dataQualityScore, all already serialized on every pick in apps/web/app/api/picks/route.ts's returned object, into one line that says where a number came from and how much of it there is, instead of three disconnected badges. This matters beyond one strip. The inventory in section 2 lists dozens of T1 and T2 signals with no display today, opponent adjusted EPA, expected turnover differential, drive outcome rates, kicker by distance splits, none of them blocked on a certified head, all of them blocked today only on nobody having built the card. With the contract and its entitlement slot already approved and tested, landing a new T1 table elsewhere in the engine becomes a data question, not a fresh design negotiation with this track every time, which is the concrete meaning of the engine carrying far more signal than it displays for a customer facing surface.

Every workstream above starts today with no founder action and no wait on another track. The factor breakdown allowlist, the score correction on the three live render sites, the ranking key rewrite, the pass decision display filter, the reason code mapping, the interval contract's empty state, the shadow disclosure component, and the provenance strip are all pure functions or React components over data already on the wire or already in the schema, each with its own vitest file, none touching packages/db/prisma/schema.prisma, .github/workflows, .claude, or any dotenv file. Two dependencies are real and are named as such rather than assumed. The interpretation of the ranking key as the engine's best read, not the ability to ship it, depends on Question Zero, owned elsewhere. Until it resolves, this track's own copy states the order as an estimate, not a finding. And the only thing that turns any of this track's dark code live, the founder's named version bump that lets process-sport.ts write a real value into PickProofReceipt.modelProb instead of null, is a law, not a convention, and this track's entire Phase 1 rendering path is built now, tested against a null modelProb, and simply switches on the day that column stops being null for a stratum. One conventional dependency in the standing document is explicitly rejected here: the ranking fix does not need Step 5a's mint time withhold rules to land first, because the filters that already run before ranking today, coherence and adverse edge suppression, already do not depend on Step 5a either, and a claimed dependency that is not proven defaults to independent.

The customer facing cost of all of this is visible and is the point, not a side effect to apologize for. Two render blocks that currently show a clean percentage lose the percent sign and gain a caveat. A sort order that currently rewards the loudest heuristic starts rewarding the largest measured edge and sometimes reorders the board. A small number of pass decision rows that currently display stop displaying. A paid entitlement that currently gates nothing starts gating an honest empty state instead of nothing at all. None of this waits for the founder, and none of it requires a single number the engine has not already computed. What does wait for the founder is narrow and named: the exact wording of every new customer string against the sharp friend test, the product decision to disclose a shadow head's existence at all, and the version bump that is the only event allowed to change what PickProofReceipt.modelProb contains. Everything else in this track is a correction, not a launch, and it ships the moment its own tests are green.

**Do not, on this track:**

- Do not render rankingP, independentEdge.trueProb, or any isotonic map of raw confidence with a percent sign or a probability-flavored aria-label, regardless of any flag's state (pick-card.tsx:349-366, :381-406, :547-551).
- Do not let a shadow head's numeric output reach any key that the current or future serializer would pass through to a client; factorBreakdown.fairProbability is the proof this risk is not hypothetical.
- Do not rank on confidence at any level of sort-key.ts, including the final fallback, on either the picks route or the board state module.
- Do not present the ranking-key reorder as a proven edge; Question Zero is still open, and the copy must say estimate, not finding.
- Do not clamp a small-sample interval into range (cqr.ts:12-15's current defect); refuse, and say why.
- Do not gate the market's own de-vig probability behind any tier, on any pick type, once extended to spreads and totals.
- Do not fabricate a source or an n on any new display card; an absent source means the card does not render, not that it renders with a placeholder.
- Do not flip CALIBRATION_ADJUSTMENTS_ENABLED, PRICING_PHASE, or any entitlement default from this track's work; build code that is correct under either state instead.
- Do not let the public calibration page's confidence-bucket forecast go unlabelled once a head basis exists; old and new rows must partition by an explicit basis tag.
- Do not restate pricesWorseThanMarket (packages/types/src/index.ts:86-90) or the new isIndependentEdgePass predicate anywhere but @sports/types; two spellings of one rule is how they drift, exactly as the existing document already found for the 22-file partial-mock boundary.

**Founder-only on this track:**

- Approve the exact wording of every new customer-facing string this track ships (reason codes, the interval empty state, the shadow-disclosure line, the recalibrated Ranking P and Independent-edge captions) against the sharp-friend test before default-on release.
- Decide the product question of whether a shadow head's existence is disclosed to customers at all, and if so, on which surface; this is a positioning call, not an engineering one.
- Name and flip the MODEL_VERSION bump that lets PickProofReceipt.modelProb (schema.prisma:619) stop being null for a certified stratum; extend the picks route's proofReceipt select (route.ts:151) to include modelProb only after that bump.
- Flip CALIBRATION_ADJUSTMENTS_ENABLED, PRICING_PHASE, or any entitlement default; this track only builds code that behaves correctly under either state.
- Sign off on the tier-policy consequences of another track's withhold rules (the single-source cap, totals free-tier-only) that will make the board thinner; this track renders whatever policy is decided, honestly, but does not set it.
- Decide the retention rule for gate-decision evidence references surfaced in NoBetDetail, since a real writer will produce rows per fixture per cycle once another track's step lands.
---
