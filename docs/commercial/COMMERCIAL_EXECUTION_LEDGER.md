...
| QB Burden Index | complete for governed shadow metric | `packages/prediction-engine/src/metrics/passing/qb-burden-index.ts`, `packages/prediction-engine/src/metrics/__tests__/qb-burden-index.test.ts`, metric birth certificate/export updates, `docs/math/GSE_PROPRIETARY_METRIC_BIBLE.md` | Role Volatility Index, Playable Window Score, and model/drift-card coverage before any promotion |
...
- Next overall gate: owner-reviewed API route design remains blocked; next safe local work is Role Volatility Index or Playable Window Score, then owner-reviewed production preview QA before any live push.
