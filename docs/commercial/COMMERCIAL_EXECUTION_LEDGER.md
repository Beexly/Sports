...
| Metric backlog YAC/Rush continuation | complete for four shadow primitives plus residual rollup helper | `yac-creation-gse`, `rush-environment-index`, `expected-rush-yards-gse`, `rush-over-expected-gse`, `buildMetricResidualRollups`, tests, metric bible | model/drift-card generators and source-policy adapter |
...
- Next overall gate: continue proprietary metric backlog with model/drift-card generators and source-policy adapter, then production preview QA before any live push.
