...
| Local review queue persistence simulator | complete for memory-shadow persistence | `apps/web/lib/workflows/local-review-queue-persistence.ts`, `apps/web/__tests__/local-review-queue-persistence.test.ts`, `docs/ops/LOCAL_REVIEW_QUEUE_PERSISTENCE_SIMULATOR.md` | API replay conflict promotion checks and abuse-response fixtures |
...
- Next overall gate: API replay conflict promotion checks and abuse-response fixtures, then production preview QA before any live push.
