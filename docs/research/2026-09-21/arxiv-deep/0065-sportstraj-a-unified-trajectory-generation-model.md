# [0065] Sports-Traj: A Unified Trajectory Generation Model for Multi-Agent Movement in Sports (arXiv:2405.17680v2)

**Citation:** Yi Xu, Yun Fu (2025). *Sports-Traj: A Unified Trajectory Generation Model for Multi-Agent Movement in Sports*. arXiv:2405.17680v2 (published at ICLR 2025). URL: https://arxiv.org/abs/2405.17680v2
**Ledger completed:** 2026-09-21. **Read:** full text (PDF, 1,678 lines, incl. appendices).
**Verdict:** ADOPT — the strongest trajectory-generation paper in the batch for GSE: NFL tracking data in-sample, open code/datasets/checkpoints, beats 9 baselines by 11–28% on minADE20, and one model unifying prediction, imputation, and recovery for the tracking pipeline.

## 1. Research question
Can a single unified trajectory-generation model replace separate models for trajectory prediction, trajectory imputation, and spatiotemporal recovery in multi-agent sports? The paper proposes **UniTraj**: treat every input format as an **arbitrary masked trajectory** and generate the complete trajectory from it. Contributions claimed (Abstract + §5): (1) unified task formulation subsuming prediction, imputation, and recovery; (2) **Ghost Spatial Masking (GSM)** module in a Transformer encoder for mask-aware spatial feature extraction; (3) **Bidirectional Temporal Mamba (BTM)** encoder plus a **Bidirectional Temporal Scaled (BTS)** module preserving temporal missing-pattern relationships; (4) three curated, benchmarked sports datasets with five masking strategies: **Basketball-U, Football-U** (built from NFL Big Data Bowl / Next Gen Stats tracking, 2017 weeks 1–6), and **Soccer-U**; (5) winner-take-all (WTA) diversity loss to generate K=20 diverse trajectories.

## 2. Dataset / schema
Three curated datasets (§3.1, Appendix C); all sequences length T=50 with 5 masking strategies applied (see §6). Coordinate systems and splits:
...
- **Five masking strategies** (Appendix C, ~50% masked rate each): (1) *Prediction Mask:* observe up to t∈{25,30,35,40}, predict to end; (2) *Random Consecutive Mask:* 1–5 random consecutive holes, each length 3, 4, or 5; (3) *Random Discrete Mask:* each location masked with probability 50–80%; (4) *Center Consecutive Mask:* one centered hole of length 25–40; (5) *Random Agent Mask:* 5 random agents fully masked per sequence. Each sequence gets one randomly chosen strategy.
...
- **Diversity loss (§2.4, Eq. 11):** weighted sum of three losses, λ1=λ2=λ3=1 (stated in Appendix D): MSE reconstruction term, a KL-type latent term, and a **winner-take-all** term — only the generated trajectory closest to ground truth (min over K=20) is backpropagated, encouraging diversity.
...
- **Assumptions:** fixed agent count (11+11+ball per sport template); sequences fixed at T=50; coordinates unnormalized per dataset's native system; mask patterns at ~50% rate.
...
- **Target:** the complete unmasked trajectory (all agents, all timesteps) — prediction, imputation, and recovery are all the same generation target under different mask patterns.
- **Prediction horizon:** observe t∈{25,30,35,40} of 50 frames, generate through frame 50 (under the Prediction Mask).

## 6. Validation design
- **Splits:** per-dataset train/test files (Football-U: 73 train / 18 test game files; Basketball-U: 93,490/11,543 sequences; Soccer-U: 48/12 files). Each sequence gets one randomly chosen masking strategy (~50% masked rate each).
...
- **Ablations:** GSM removal, unidirectional Mamba variants, BTS removal, ghost-mask pooling variants, Mamba/Transformer depth sweeps, temporal-architecture swaps, plus generalization tests on ETH-UCY/SDD pedestrians and Traffic-Guangzhou imputation (Appendix E).
...
- **Environment:** conda, Python 3.11.8, PyTorch 2.2.1, mamba-ssm (causal-conv1d≥1.2.0); seed 2024 fixed; full hyperparameters in Appendix D.
...
## 10. GSE overlap
- **Existing GSE stack:** Kalman/particle filters on tracking data (existing-research map). UniTraj is the *generative multi-agent* upgrade: where Kalman gives single-track smoothing, UniTraj gives joint, interaction-aware, multi-hypothesis generation for all 23 entities. No overlap with Garrett's momentum lane (outcome-momentum, not trajectory).
- **This batch:** 0063 (MambaTrack, ADAPT) — same Mamba lineage; MambaTrack handles online multi-object tracking/association, UniTraj handles generative trajectory modeling. Complementary, not duplicative: track with MambaTrack, generate/impute with UniTraj. 0066 (MambaMOT) is also online association — same pipeline position as MambaTrack.
- **Drive-58 dossiers:** diffusion trajectory modeling paper 2503.18589 was already read — head-to-head comparison against UniTraj is a natural next experiment.
- **Other trajectory literature:** Social-LSTM, Trajectron++, MotionDiffuser, Traj-MAE — the paper positions against these; UniTraj's differentiator is the unified masked formulation rather than prediction-only.
- Assessment: new capability (generative multi-agent trajectory modeling on NFL tracking), not a duplicate.
...
Verdict rationale: UniTraj directly targets NFL tracking data, open-sources code + datasets + checkpoints with a full hyperparameter spec, and beats 9 baselines by 11–28% on minADE20 — the implementation source of truth is the code (printed equations are garbled), and NFL validation must be time-ordered on modern NGS data.
1. **Reproduction criterion:** run the authors' code on Football-U and recover Table 1: minADE20 ≤ 3.73 yards (paper: 3.55, +5% tolerance) with OOB ≤ 2e-04 (paper: 1.12e-04).
