# Deep Research Ledger — 0070

- **file_index:** 0070
- **arxiv_id:** 2310.10386v1
- **title:** Rating of players by Laplace approximation and dynamic modeling
- **authors:** Hsuan-Fu Hua, Ching-Ju Chang, Tse-Ching Lin, Ruby Chiu-Hsing Weng (Dept. of Statistics, National Chengchi University, Taiwan)
- **venue:** arXiv preprint v1 (2023-10-02)
- **date read:** 2026-09-21
- **reading path:** Full text read via arXiv PDF (2,927 extracted lines, all read, incl. appendices). ar5iv not attempted (PDF rendered cleanly).
- **verdict:** ADAPT
...
- **Data:** Jeff Sackmann ATP matches 2010–2019; retirements/defaults/walkovers and carpet matches removed. **25,537 matches total** (Table 2): train 2010–2017 = 20,437 (Hard 11,613 / Clay 6,399 / Grass 2,425); test 2018–2019 = 5,100 (Hard 2,907 / Clay 1,559 / Grass 634). **771 players**; 135 with >250 appearances (Djokovic 685, Nadal 650, Federer 640 — motivating the variance-collapse concern).
...
1. **Reproduction criterion:** reimplement Algorithm 1 (vElo) on Sackmann ATP 2010–2019 and recover Table 3(c): accuracy ≈ 0.6387 at (A,B)=(1/5,80), within ±0.003 (paper: +21.9 to +25.0 of 5,100 matches over constant variance), per §10.
...
**ADAPT** — Not a new capability (dynamic Elo is already in GSE's stack), but a concrete, implementable refinement: the per-match Laplace variance update with reduction factor A and lower bound B, validated by McNemar-significant gains on 5,100 test matches and better early-career tracking of new players. Pair it with the surface-transfer formula as the template for NFL context-split ratings (grass/turf, dome/outdoor). Do not adopt the tennis parameterization itself.
...
- PDF extraction is clean for prose, tables, and scalar equations (1, 8, 18, 21a–b, 23–25). Matrix equations (Props. 1–2, Eqs. 12–16, 19–20a–c) and Algorithms 2–3 are heavily interleaved in extraction — flagged for source-level verification before implementation.
