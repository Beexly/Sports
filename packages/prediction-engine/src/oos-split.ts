/**
 * Out-of-sample (OOS) split harness â€” the governance framework for safe model promotion.
 *
...
// NOTE (rescue 2026-09-25): calibration-drift.ts was moved to attic/ on main
// after this module was written; the API is unchanged.
import type { ScoredOutcome, WindowCalibration } from "../attic/calibration-drift.js";
import { computeWindowCalibration } from "../attic/calibration-drift.js";

export interface SettledPickRecord {
  /** Unique pick ID. */
  readonly id: string;
  /** Model's predicted win probability (0â€“1) at prediction time. */
...
export interface OosSplitResult {
  ...
  readonly oosIsHealthy: boolean; // OOS Brier â‰¤ inSample Brier + tolerance
  readonly brierDelta: number; // OOS Brier âˆ’ inSample Brier
  /** "OOS is X% worse than in-sample" â€” plain-language health readout. */
