/**
 * Fourth-down grader â€” decision-quality grades on 4th-down calls.
 *
 * Per-play metric: delta = WP(go) âˆ’ WP(actual call).
 ...
 *   A â€” optimal (actual call achieved the maximum WP among the options)
 *   B â€” within 1 percentage point of optimal
 *   C â€” between 1 and 3 percentage points of optimal
 *   D â€” worse than 3 percentage points of optimal
 *
 * Invalid / out-of-range / non-finite win probabilities yield `null`
 * (fail-closed; never imputed).
 *
 * Additive research module â€” not wired into any live prediction path.
 */
...
export interface FourthDownGradeRow {
  ...
  wpGo: number;
  wpActual: number;
  /** WP(go) âˆ’ WP(actual call) â€” the module's headline metric. */
  wpGoMinusActual: number;
  /** WP(optimal) âˆ’ WP(actual call); drives the letter grade. */
  optimalGap: number;
