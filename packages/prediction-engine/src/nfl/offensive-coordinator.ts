...
// These return TYPE PREDICATES, not plain booleans. A plain `boolean` return
// type makes every downstream `look.x` access an error under strict TS, because
// the compiler cannot connect "this function returned true" to "look is not
// null" — the guard and the narrowing have to be the same fact. Runtime
// behaviour is unchanged; this only teaches the compiler what was already true.
function lookIsComplete(look: DefensiveLook | null | undefined): look is DefensiveLook {
...
function situationIsComplete(s: GameSituation | null | undefined): s is GameSituation {
  ...
  return {
    recommend(input) {
      ...
      const scored = input.candidates.map((c) => {
        // `baseEPA`, not `historicalEPA`: the latter is not a field on
        // PlayCandidate and never was. scorePlay() has always read baseEPA
        // here, so the live path was the inconsistent one.
        let epa = c.baseEPA;
        const exploits: string[] = [];
