...
import {
  ...
  logScoreStacking,
  regimeStackWeights,
} from "./2012-01643-diversity-weighted-combiner.js";
...
describe("diversity", () => {
  it("diversity matrix is symmetric with zero diagonal", () => {
    const F = [[0.1, 0.2, 0.3], [0.5, 0.5, 0.5], [0.9, 0.8, 0.7]];
    const D = diversityMatrix(F);
    expect(D[0]![0]).toBe(0);
    expect(D[0]![1]).toBeCloseTo(D[1]![0]!, 10);
    expect(D[0]![2]).toBeGreaterThan(D[0]![1]!);
  });
  it("diversityWeights upweights the diverse-but-skilled model", () => {
    const w0 = diversityWeights([1, 1, 1], [0.1, 0.5, 0.1], 0);
    expect(Math.abs(w0[0]! - w0[1]!)).toBeLessThan(1e-9);
    const w1 = diversityWeights([1, 1, 1], [0.1, 0.5, 0.1], 3);
