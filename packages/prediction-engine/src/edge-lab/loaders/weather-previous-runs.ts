/**
 * Previous-runs weather loader â€” the STRICT as-of backtest path W-WEATHER-REC
 * left open (DEC-023 closed the capability gate; this module is the wiring).
 *
 * The vendored loader (weather-edge.ts, verbatim per DEC-014 â€” behavioral
 ...
 * cannot prove a given hour's value came from a run issued â‰¤ asOfUtc. This
 ...
 *   N = max(1, ceil(leadTimeHours / 24)) â€” never a smaller N, so the selected
 *   run is issued â‰¤ asOfUtc by construction, rounding toward an OLDER run whenever
 ...
 *
 * Same input/output contract as the vendored loader (GameWeatherQuery â†’
 * WeatherFeatures), so the existing `toGameWeatherForecast` adapter and
 * `buildWeatherFeatureRows` leak gate consume it UNCHANGED â€” one canonical
 ...
 * the HOSTED free tier is non-commercial. A single-probe smoke is one thing â€”
...
/**
 * Open-Meteo's DOCUMENTED previous-runs window: `_previous_day1` â€¦
 * `_previous_day7`. HONESTY NOTE: DEC-023's live smoke
 * (scripts/weather-integration-smoke.mjs) empirically confirmed distinct
 * runs only through `_previous_day3` â€” days 4-7 are a documented, not yet
 ...
 * honest `unavailable` it uses for any other missing-field case â€” it does
 ...
 * lead hours: a 24.4h lead rounded to 24 would select day1 â€” a run
 * initialized ~24h before the valid hour, i.e. AFTER the freeze â€” which is
 ...
 * when the lead exceeds the API's window â€” the caller degrades to honest
 * unavailable.
 */
...
export async function getAsOfGameWeatherPreviousRuns(q: GameWeatherQuery, deps: WeatherDeps): Promise<WeatherFeatures> {
  ...
  // Exact (unrounded) lead in hours â€” feeds previousDayN's ceil() below.
  ...
  // instant) â€” exactly the lookahead this module exists to prevent.
  ...
  if (n === null) {
    return unavailable(
      ...
      leadTimeHours,
      api,
      `Lead time ${leadTimeHours}h exceeds the previous-runs window (${MAX_PREVIOUS_DAYS}d) â€” honest unavailable, never a leaky smaller run.`,
    );
  }
  ...
  return {
    ...
    provenance: {
      api,
      leadTimeHours,
      note: `Strict as-of backtest path: previous-runs forecast run _previous_day${n} for the kickoff hour (N = max(1, ceil(${leadTimeHours}h / 24)) â€” conservative toward an older run), issued <= asOfUtc by construction. Live-verified distinct-run capability per DEC-023's smoke.`,
    },
  };
}
