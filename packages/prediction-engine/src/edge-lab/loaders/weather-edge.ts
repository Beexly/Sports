 ...
 * (2026-07-17, 8/8 tests in-packet). This is the AS-OF WEATHER LOADER â€” the
 ...
 * backtest path: historical-forecast archive constrained to runs issued â‰¤
 * asOfUtc â€” NEVER observed weather), and every result carries provenance +
 ...
 * and the edge-lab feature are COMPLEMENTARY layers of one canonical path â€”
 * loader (this file) â†’ toGameWeatherForecast adapter â†’ buildWeatherFeatureRows
 * â†’ Â§5 trials registry. Behavioral edits here are forbidden; improvements go
 ...
 * license for production) â€” see source-rights-registry "open-meteo".
 */
/**
 * gse-weather-edge â€” leak-safe, AS-OF NFL weather features from Open-Meteo.
 *
 * WHY THIS EXISTS
 ...
 * commercial-OK â€” but a naive weather join LEAKS: using the weather that
 ...
 *   - Live/upcoming games (kickoff in the future): use the CURRENT forecast â€” a
 ...
 *     to admit or reject â€” this module makes NO accuracy/edge claim.
 *
 ...
 * as-of backtests (see INTEGRATION_SPEC.md â†’ "Strict as-of backtests").
 */
...
export interface StadiumSite {
  ...
  /** Dome or retractable-roof-closed â†’ no meaningful field weather. */
...
// â”€â”€ URL builders (documented Open-Meteo shape; imperial units) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Feature extraction â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/**
 * CANDIDATE signals â€” documented, monotone transforms of the raw fields. These
...
function indoorNeutral(q: GameWeatherQuery, leadTimeHours: number): WeatherFeatures {
  return {
    ...
    provenance: { api: null, leadTimeHours, note: "Indoor/closed-roof site â€” weather does not apply; neutral by construction." },
  };
}

/**
 * Return leak-safe, as-of weather features for one game. Throws only on a
 * genuine leak (asOf after kickoff) or malformed dates â€” everything else
...
export async function getAsOfGameWeather(q: GameWeatherQuery, deps: WeatherDeps): Promise<WeatherFeatures> {
  ...
  const leakNote =
    source === "historical-forecast"
      ? `Backtest path: archived forecast for the kickoff hour. LEAK CONTRACT â€” the feature store must confirm the underlying forecast run was issued <= asOfUtc (see INTEGRATION_SPEC.md â†’ Strict as-of backtests). Lead time ${leadTimeHours}h.`
