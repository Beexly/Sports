/**
 * B3 â€” the strict as-of previous-runs loader (weather-previous-runs.ts): the
...
describe("previousDayN â€” the conservative run-selection rule", () => {
  it("never selects a run newer than the lead time implies, and never day0", () => {
    expect(previousDayN(0)).toBe(1); // day0 IS the current run â€” can postdate the freeze
    expect(previousDayN(2)).toBe(1);
    expect(previousDayN(24)).toBe(1);
    expect(previousDayN(25)).toBe(2); // 25h lead: day1 (24h) could postdate asOf â€” round OLDER
  ...
  it("beyond the API window returns null â€” honest unavailable, never a leaky smaller N", () => {
    expect(previousDayN(MAX_PREVIOUS_DAYS * 24 + 1)).toBeNull();
  });

  it("MUST be fed the exact lead â€” rounding it first would select a run issued after the freeze", () => {
    // A 24.4h exact lead ROUNDS to 24h; ceil(24/24)=1 would (wrongly) select
    // _previous_day1, whose run is initialized ~24h before the valid hour â€”
...
describe("getAsOfGameWeatherPreviousRuns", () => {
  it("throws the leak guard when asOf is after kickoff â€” same contract as the vendored loader", async () => {
    await expect(
      getAsOfGameWeatherPreviousRuns(
  ...
  it("indoor sites neutralize identically to the vendored loader â€” no fetch at all", async () => {
  ...
  it("a fractional 24.4h lead (rounds to 24h) still requests _previous_day2, not the leaky day1", async () => {
    ...
    // be wrong here â€” this proves the call site feeds previousDayN the
    // UNROUNDED lead.
  ...
  it("candidate signals use the vendored loader's EXACT math â€” shared function, not a copy", async () => {
    const fetchJson = vi.fn().mockResolvedValue(previousRunsResponse(1));
    const result = await getAsOfGameWeatherPreviousRuns(
      { site: LAMBEAU, kickoffUtc: KICKOFF, asOfUtc: AS_OF_2H },
      { fetchJson },
  ...
  it("end to end: previous-runs output flows through the SAME adapter + leak gate unchanged", async () => {
    ...
    // asOf 2h pre-kickoff < the 1h decision cutoff â†’ passes the leak gate.
