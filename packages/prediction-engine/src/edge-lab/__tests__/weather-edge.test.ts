/**
 * W-WEATHER-REC â€” the vendored as-of loader + the adapter seam into the
 ...
 * end to end: loader â†’ adapter â†’ buildWeatherFeatureRows â†’ EvalRows with the
...
describe("vendored loader (packet parity)", () => {
  ...
  it("a future kickoff uses the live forecast API at the kickoff hour", async () => {
    ...
    const w = await getAsOfGameWeather(
      { site: LAMBEAU, kickoffUtc: KICKOFF, asOfUtc: AS_OF },
      {
        ...
        now: () => new Date(AS_OF), // "now" is freeze time â€” kickoff is future
      },
    );
  ...
  it("a fetch failure degrades to an honest available:false â€” never fabricated neutral", async () => {
    const w = await getAsOfGameWeather(
      { site: LAMBEAU, kickoffUtc: KICKOFF, asOfUtc: AS_OF },
...
describe("the adapter seam + end-to-end canonical path", () => {
  ...
  it("loader â†’ adapter â†’ feature builder yields leak-clean EvalRows", async () => {
    const w = await getAsOfGameWeather(
      { site: LAMBEAU, kickoffUtc: KICKOFF, asOfUtc: AS_OF },
  ...
  it("available:false maps to null â†’ the feature layer records an honest skip", () => {
    expect(
