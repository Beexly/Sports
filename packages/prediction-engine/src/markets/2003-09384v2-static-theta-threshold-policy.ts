/**
 * arXiv 2003.09384v2: Investigating the efficiency of the Asian handicap football betting market with ratings and Bayesian networks
 *
 * ADDITIVE utility. Not wired into any live model path (wiring changes predictions and is a NEEDS HUMAN CALL).
 *
 * MECHANISM (paper):
 * Profit-maximizing static-theta edge-threshold policy: optimize GSE's bet-selection threshold on model-vs-market edge for total realized profit (not per-week ROI) on chronological backtests, deploying the static theta that maximizes total profit when per-window-optimal theta is unstable; line-shopping uplift quantified by running the identical bet list against consensus-average vs best-available odds.
 *
 * IMPROVEMENT (wiring-wave2 slice, verbatim):
 * Adopt the profit-maximizing static-theta edge-threshold policy: optimize GSE's bet-selection threshold on model-vs-market edge for total realized profit (not per-week ROI) on chronological backtests, deploying the static theta that maximizes total profit when per-window-optimal theta is unstable; quantify the line-shopping uplift by running the identical bet list against consensus-average vs best-available-across-books odds -- threshold discipline as engine-honesty infrastructure, never a profit target.
 *
 * ACCEPTANCE GATE (verbatim):
 * Adopt the profit-maximizing static-theta threshold policy if, on the 2015-2024 NFL backtest: (i) some static theta yields total profit >= 20% above the current GSE threshold policy's total profit at comparable bet counts, AND (ii) per-season-optimal theta is unstable (optimal theta differs by >= 5 pp across >= 3 of the 10 seasons).
 *
