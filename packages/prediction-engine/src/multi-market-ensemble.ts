/**
 * Multi-market true-probability ensemble (precision-weighted) â€” the "find more
 * honest â‰¥70% picks" lever for docs/path-to-70.md Step 2.
 *
 * THE GAP IT FIXES
 ...
 * Bayesian sensor fusion): each source's weight âˆ� 1/ÏƒÂ², the combined estimate is
 * Î£Åµáµ¢Â·páµ¢, and its variance is 1/Î£(1/Ïƒáµ¢Â²) â€” strictly tighter than any single
 ...
 *   - `crossMarketDivergence` (weighted disagreement â€” a market-uncertainty signal
 *     a caller can pass straight into assessEdge's `uncertainty`), and
 *   - `independents`: the SAME estimators with principled `weight`s â€” a drop-in for
 *     edge-engine.assessEdge, so its agreement/CLV logic is unchanged but its blend
 *     becomes precision-weighted.
 *
 * Pure, no I/O, fully unit-tested. Additive: it changes no existing behaviour â€” it
 ...
 *
 * The inverse-variance math is principled; only the Ïƒ-from-reliability mapping uses
...
// â”€â”€ Ïƒ-derivation constants (tunable; documented heuristics, not fitted) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
/** Baseline standard error (prob points) of an ideal, vig-free, deep, fresh quote. */
export const SIGMA_BASE = 0.05;
/** Each 1pt of book hold/over-round multiplies Ïƒ by (1 + this). 5pt hold â‡’ Ã—1.25. */
const HOLD_COEF = 0.05;
/** Staleness: Ïƒ widens by (1 + AGE_COEF Â· age/AGE_REF_SECONDS). */
const AGE_COEF = 0.5;
const AGE_REF_SECONDS = 300; // 5 minutes
/** Model sample size that counts as "reference" tightness; Ïƒ âˆ� 1/âˆš(n/ref). */
const SAMPLE_REF = 20;
/** Clamp Ïƒ so a single source can neither dominate absolutely nor vanish. */
const SIGMA_MIN = 0.01;
const SIGMA_MAX = 0.5;

/** How noisy is this source? Lower Ïƒ â‡’ more weight in the blend. */
export interface EstimatorReliability {
  /** Book hold / over-round in percentage points (0 for a vig-free exchange). Higher â†’ noisier. */
  readonly holdPct?: number;
  /** Depth/coverage proxy (e.g. book count, exchange depth tier â‰¥ 1). Higher â†’ tighter (Ïƒ âˆ� 1/âˆšliquidity). */
  readonly liquidity?: number;
  /** Quote age in seconds. Older â†’ staler â†’ noisier. */
  readonly ageSeconds?: number;
  /** Sample size behind a MODEL estimate (e.g. games used for team rates). Higher â†’ tighter. */
...
export interface MarketEstimate {
  /** Source label â€” MUST NOT be the sportsbook line we are grading against. */
  readonly source: string;
  /** Independent P(side is correct), 0â€“1. */
  readonly prob: number;
  readonly reliability?: EstimatorReliability;
}

export interface EnsembleResult {
  /** Precision-weighted (inverse-variance) fair probability, 0â€“1; null when no valid estimate. */
  readonly fairProb: number | null;
  /** Standard error of the combined estimate: 1/âˆš(Î£ 1/Ïƒáµ¢Â²). Tighter than any single Ïƒ. */
  readonly stdError: number;
  ...
  /** Largest absolute disagreement between any two sources, 0â€“1. */
  readonly maxPairwiseDisagreement: number;
  /** Kish effective number of sources: (Î£w)Â²/Î£wÂ². ~1 when one source dominates. */
  ...
  /** The estimators with precision weights attached â€” a drop-in for edge-engine.assessEdge. */
 ...
 * `stdError` wins; otherwise Ïƒ grows with hold and staleness and shrinks with
...
export function precisionWeightedEnsemble(estimates: readonly MarketEstimate[]): EnsembleResult {
  ...
  // Inverse-variance combination: 1/Var = Î£ 1/Ïƒáµ¢Â²  â‡’  combined Ïƒ = 1/âˆšÎ£.
  ...
  // Kish effective sample size: (Î£w)Â²/Î£wÂ². Penalises one dominant source.
...
// ============================================================
// Adapter â€” fuse the engine's real independent fair-value sources
// ============================================================
 ...
 * vig-free and reasonably deep â†’ low Ïƒ â†’ more weight. Model estimators carry
 ...
 * fair-value snapshots (Kalshi exchange, Poisson/Elo/ML models, â€¦). Skips sources
