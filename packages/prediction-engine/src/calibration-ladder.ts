/**
 * Calibration ladder â€” path-to-70.md Step 1, the honest way.
 *
 * THE GAPS THIS CLOSES (vs calibration-apply.ts today)
 *   1. ISOTONIC-ONLY. Isotonic/PAVA overfits below ~1000 samples â€” and GSN will be
 ...
 *      sample (calibratedEce â‰¤ rawEce on the fitted data) â€” the leakage trap. We
 ...
 * assumed in CHRONOLOGICAL order (oldest â†’ newest) so the held-out split is a real
...
// ============================================================
// Platt scaling (sigmoid) â€” the small-n calibrator
// ============================================================
...
export interface PlattModel {
  ...
  /** Intercept. */
  readonly b: number;
  /** calibrated = sigmoid(aÂ·forecast + b), clamped to [0,1]. */
  readonly predict: (forecast: number) => number;
}

/**
 * Fit calibrated = sigmoid(aÂ·p + b) by minimising log-loss with Platt's target
 * smoothing (so all-win/all-loss buckets don't push the fit to Â±âˆž). Solved by
 * averaged-gradient descent â€” stable even when forecasts are near-constant (where
 * a Newton step is ill-conditioned). Empty input â†’ identity.
 */
export function plattScaling(samples: readonly CalibrationSample[]): PlattModel {
  ...
  const nn = n - np;
  // Platt's smoothed targets â€” regularise toward the prior, away from hard 0/1.
...
export interface EmpiricalBin {
  ...
  /** Wilson lower bound on the bin's win rate â€” the defensible public floor. */
...
// ============================================================
// Ladder â€” held-out, time-ordered method selection
// ============================================================
...
export interface CalibrationLadder {
  ...
  /** confidence 0â€“100 â†’ calibrated win probability (identity passthrough when inactive). */
  readonly apply: (confidence0to100: number) => CalibratedLadderProbability;
  /** confidence 0â€“100 â†’ the Wilson conservative floor for that bucket (always available). */
 ...
 *
 * `samples` MUST be chronological (oldest â†’ newest).
 */
