/**
 * Synthetic public-lean / fade signal â€” the honest, glass-box reframe of the
 ...
 * inflates the popular side's price â€” the value is often the other way (the
 ...
 *     handle ("X% of the public is on â€¦"). Every surfacing must be labelled
 *     `LABEL` below ("simulated audience model â€” not betting data").
 *   - It NEVER originates a pick and is never cited as public provenance.
 *   - It folds into edge ONLY as a SMALL, HARD-CAPPED nudge (â‰¤ MAX_FADE_NUDGE)
 *     with low confidence (â‰¤ MAX_CONFIDENCE) â€” structured market data stays the
 *     source of truth.
 ...
 *
 * Pure functions, no I/O â€” fully unit-testable. Inputs are structured public
 * facts only; this module infers nothing about health, injuries, or private life.
 */

/** Hard cap on the edge nudge (probability points). Matches the narrative Â±0.01 valve. */
export const MAX_FADE_NUDGE = 0.01;
/** Hard cap on the signal's confidence â€” it can inform, never carry. */
export const MAX_CONFIDENCE = 0.2;
/** Required label whenever this signal is surfaced anywhere. */
export const SYNTHETIC_FADE_LABEL = "simulated audience model â€” not betting data";

/** Public-attractor facts for ONE side of a market. All optional; absent = neutral. */
export interface PublicLeanInput {
  /** The side's market-implied probability (0â€“1). The public backs favourites. */
  readonly marketImpliedProb?: number;
  /** Marquee / large-market / nationally-followed team. */
  readonly isPopularTeam?: boolean;
  /** Nationally televised / primetime slot â€” amplifies casual action. */
...
export interface SyntheticPublicLean {
  ...
  /** Mandatory display label â€” must propagate to every surface. */
  readonly label: string;
}

// Weights over the public-attractor features (sum to 1). Documented heuristics,
// not fitted â€” the whole signal is gated OFF until backtested.
...
function round(v: number, d = 4): number {
  ...
  return r === 0 ? 0 : r; // normalise -0 â†’ 0 so signed-zero never leaks into output
}

/**
 * Estimate how heavily the public leans onto a side, and the capped contrarian
 * (fade) nudge. Transparent weighted proxy â€” no swarm, no fabricated handle.
 */
export function syntheticPublicLean(input: PublicLeanInput): SyntheticPublicLean {
  // Favouritism: public backs favourites. Map implied prob (0â€“1) â†’ lean (âˆ’1â€¦+1).
  const favLean =
