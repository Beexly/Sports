...
import { MIN_BOOKMAKERS } from "./constants.js";
...
export type PickForMarketP = {
  /**
   * Never read by this module. Optional so a PRE-INSERT caller - the signal
   * slate, resolving an anchor before the pick row exists - can use the same
   * resolver as the post-hoc calibration loader rather than a second copy of
   * this arithmetic. Callers that have an id still pass it.
   */
  readonly id?: string;
  readonly gameId: string;
