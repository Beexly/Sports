...
describe("recommendPick — 1.09 Achane vs Lamb", () => {
  ...
  it("selects the higher value-minus-scarcity player and names leading candidates", () => {
    ...
    // No `?? rec.scored...` fallback here. That fallback is what let a typo in
    // the first operand ('scoreed') pass unnoticed for the life of the test: it
    // silently evaluated the right-hand side and the assertion stopped testing
    // anything. Read the field directly or the test is decorative.
    expect(rec.scored[0]?.score).toBeCloseTo(
      Math.max(
