/**
 * arXiv 2006.06707v2: Learning to Learn Kernels with Variational Random Features
 *
 * ADDITIVE utility. Not wired into any live model path (wiring changes predictions and is a NEEDS HUMAN CALL).
 *
 * MECHANISM (paper):
 * MetaVRF few-shot regime model: tasks = team-seasons in sequential order, tabular MLP embedding psi on nflverse team-game stats, D=780 RFF bases, meta-learned KRR base-learner predicting next-game margin with an LSTM accumulating league meta-knowledge across seasons; weekly meta-test as a forward pass plus a 4x4 ridge solve (no gradient steps); classification variant (cover/no-cover) tested against the engine's classifier.
 *
 * IMPROVEMENT (wiring-wave2 slice, verbatim):
 * Deploy MetaVRF as the few-shot regime model: tasks = team-seasons in sequential order, tabular MLP embedding psi on nflverse team-game stats, D=780 RFF bases, meta-learned KRR base-learner predicting next-game margin with an LSTM accumulating league meta-knowledge across seasons; serve meta-test weekly as a forward pass plus a 4x4 ridge solve (no gradient steps); test the classification variant (cover/no-cover) against the engine's existing classifier.
 *
 * ACCEPTANCE GATE (verbatim):
 * ADOPT iff MetaVRF beats the league-average prior by >=0.01 Brier on new-regime first-4-game predictions (2023-2025 LOSO) AND beats fixed-bandwidth RBF-KRR by >=0.005 Brier. Reject if the LSTM context adds nothing over MetaVRF w/o LSTM.
 *
