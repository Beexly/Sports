/**
 * arXiv 2009.03228v3: Information Theoretic Meta Learning with Gaussian Processes
 *
 * ADDITIVE utility. Not wired into any live model path (wiring changes predictions and is a NEEDS HUMAN CALL).
 *
 * MECHANISM (paper):
 * Deep-kernel GP-VIB on nflverse 2015-2025 (tasks = team-seasons; meta-test = rookie-QB / new-HC / post-trade regimes 2023-2025): Gaussian-likelihood encoder, beta in {0.01, 0.1, 1.0} grid, target = next-game win-probability logit with support = first K games of the new regime; the KL term is the anti-memorization gate, beta tuned on 2023 leave-one-season-out; weekly refits per new-regime team with closed-form GP posterior, uncertainty bands feeding pick-confidence and Kelly sizing.
 *
 * IMPROVEMENT (wiring-wave2 slice, verbatim):
 * Deploy deep-kernel GP-VIB on nflverse 2015-2025 (tasks = team-seasons; meta-test = rookie-QB / new-HC / post-trade regimes 2023-2025): Gaussian-likelihood encoder, beta in {0.01, 0.1, 1.0} grid, target = next-game win-probability logit with support = first K games of the new regime; the KL term acts as the anti-memorization gate, beta tuned on 2023 leave-one-season-out; serve weekly refits per new-regime team with closed-form GP posterior, uncertainty bands feeding pick-confidence and Kelly sizing.
 *
 * ACCEPTANCE GATE (verbatim):
 * ADOPT iff GP-VIB (beta tuned) beats the league-average prior by >=0.01 Brier on new-regime first-4-game predictions (2023-2025 LOSO) AND the beta>0 model beats the beta=0 ablation on NLL by a statistically clear margin (paired t-test p<0.05).
 *
