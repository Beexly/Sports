...
// Away name for the (now REQUIRED) 8th arg - neutral: matches no test selection.
const VIS = "Visiting Opponents";
...
describe("calculatePickResult — SPREAD", () => {
  it("home covers (home -3.5, wins by 7)", () => {
    ...
    expect(calculatePickResult("SPREAD", "Kansas City Chiefs", -3.5, "Kansas City Chiefs", 21, 14, NFL, VIS)).toBe("WIN");
  });

  it("home fails to cover (home -3.5, wins by only 3)", () => {
    // homeMargin=3, line=-3.5, homeCoverMargin=-0.5 < 0 → home LOSS
    expect(calculatePickResult("SPREAD", "Kansas City Chiefs", -3.5, "Kansas City Chiefs", 17, 14, NFL, VIS)).toBe("LOSS");
  });

  it("away covers when home doesn't (home -3.5, home wins by 3)", () => {
    // away pick: homeCoverMargin=-0.5 < 0 → away WIN
    expect(calculatePickResult("SPREAD", "Denver Broncos", -3.5, "Kansas City Chiefs", 17, 14, NFL, VIS)).toBe("WIN");
  });

  it("away fails to cover (home -3.5, home wins by 7)", () => {
    // away pick: homeCoverMargin=3.5 > 0 → away LOSS
    expect(calculatePickResult("SPREAD", "Denver Broncos", -3.5, "Kansas City Chiefs", 21, 14, NFL, VIS)).toBe("LOSS");
  });

  it("exact push (home -7, wins by exactly 7)", () => {
    // homeMargin=7, line=-7, homeCoverMargin=0 → PUSH
    expect(calculatePickResult("SPREAD", "Kansas City Chiefs", -7, "Kansas City Chiefs", 28, 21, NFL, VIS)).toBe("PUSH");
  });

  it("exact push for away pick too", () => {
    expect(calculatePickResult("SPREAD", "Denver Broncos", -7, "Kansas City Chiefs", 28, 21, NFL, VIS)).toBe("PUSH");
  });

  it("home underdog covers (+3.5, loses by 2)", () => {
    // homeMargin=-2, line=3.5, homeCoverMargin=1.5 > 0 → home WIN
    expect(calculatePickResult("SPREAD", "Denver Broncos", 3.5, "Denver Broncos", 14, 16, NFL, VIS)).toBe("WIN");
  });

  it("home underdog fails to cover (+3.5, loses by 7)", () => {
    // homeMargin=-7, line=3.5, homeCoverMargin=-3.5 < 0 → home LOSS
    expect(calculatePickResult("SPREAD", "Denver Broncos", 3.5, "Denver Broncos", 14, 21, NFL, VIS)).toBe("LOSS");
  });

  it("home wins outright as underdog (covers)", () => {
    // homeMargin=3, line=3.5, homeCoverMargin=6.5 > 0 → home WIN
    expect(calculatePickResult("SPREAD", "Denver Broncos", 3.5, "Denver Broncos", 17, 14, NFL, VIS)).toBe("WIN");
  });
  ...
  it("home loses and away covers (away favored)", () => {
    ...
    expect(calculatePickResult("SPREAD", "Home Team", 7, "Home Team", 14, 17, NFL, VIS)).toBe("WIN");
  });
});
...
describe("calculatePickResult — MONEYLINE", () => {
  it("home ML win", () => {
    expect(calculatePickResult("MONEYLINE", "Chiefs", -3.5, "Chiefs", 21, 14, NFL, VIS)).toBe("WIN");
  });

  it("home ML loss", () => {
    expect(calculatePickResult("MONEYLINE", "Chiefs", -3.5, "Chiefs", 14, 21, NFL, VIS)).toBe("LOSS");
  });

  it("away ML win", () => {
    expect(calculatePickResult("MONEYLINE", "Broncos", -3.5, "Chiefs", 14, 21, NFL, VIS)).toBe("WIN");
  });

  it("away ML loss", () => {
    expect(calculatePickResult("MONEYLINE", "Broncos", -3.5, "Chiefs", 21, 14, NFL, VIS)).toBe("LOSS");
  });

  it("non-soccer tie is PUSH for home ML", () => {
    // NFL overtime tie (very rare but possible)
    expect(calculatePickResult("MONEYLINE", "Chiefs", 0, "Chiefs", 17, 17, NFL, VIS)).toBe("PUSH");
  });

  it("non-soccer tie is PUSH for away ML", () => {
    expect(calculatePickResult("MONEYLINE", "Broncos", 0, "Chiefs", 17, 17, NFL, VIS)).toBe("PUSH");
  });

  it("soccer draw is LOSS for home ML bet (3-way market)", () => {
    expect(calculatePickResult("MONEYLINE", "LA Galaxy", 0, "LA Galaxy", 1, 1, MLS, VIS)).toBe("LOSS");
  });

  it("soccer draw is LOSS for away ML bet (3-way market)", () => {
    expect(calculatePickResult("MONEYLINE", "Toronto FC", 0, "LA Galaxy", 1, 1, MLS, VIS)).toBe("LOSS");
  });

  it("soccer home win is WIN", () => {
    expect(calculatePickResult("MONEYLINE", "LA Galaxy", 0, "LA Galaxy", 2, 1, MLS, VIS)).toBe("WIN");
  });

  it("soccer away win is WIN for away pick", () => {
    expect(calculatePickResult("MONEYLINE", "Toronto FC", 0, "LA Galaxy", 1, 2, MLS, VIS)).toBe("WIN");
  });
});
...
describe("calculatePickResult — TOTAL", () => {
  it("over hits", () => {
    expect(calculatePickResult("TOTAL", "OVER", 47.5, "Chiefs", 28, 21, NFL, VIS)).toBe("WIN");
  });

  it("over misses", () => {
    expect(calculatePickResult("TOTAL", "OVER", 47.5, "Chiefs", 17, 14, NFL, VIS)).toBe("LOSS");
  });

  it("under hits", () => {
    expect(calculatePickResult("TOTAL", "UNDER", 47.5, "Chiefs", 17, 14, NFL, VIS)).toBe("WIN");
  });

  it("under misses", () => {
    expect(calculatePickResult("TOTAL", "UNDER", 47.5, "Chiefs", 28, 21, NFL, VIS)).toBe("LOSS");
  });

  it("exact total is PUSH", () => {
    expect(calculatePickResult("TOTAL", "OVER", 49, "Chiefs", 28, 21, NFL, VIS)).toBe("PUSH");
  });

  it("exact total UNDER is also PUSH", () => {
    expect(calculatePickResult("TOTAL", "UNDER", 49, "Chiefs", 28, 21, NFL, VIS)).toBe("PUSH");
  });
});
...
describe("calculatePickResult — home/away symmetry", () => {
  ...
  for (const [label, line, homeScore, awayScore] of cases) {
    it(`SPREAD home vs away must be inverse or both PUSH: ${label}`, () => {
      const homeResult = calculatePickResult("SPREAD", "Home Team", line, "Home Team", homeScore, awayScore, NFL, VIS);
      const awayResult = calculatePickResult("SPREAD", "Away Team", line, "Home Team", homeScore, awayScore, NFL, VIS);
      if (homeResult === "PUSH") {
...
describe("calculatePickResult — away name contains home name (no mis-settle)", () => {
  ...
  it("away ML pick wins when the away team (whose name contains 'Jets') wins", () => {
    ...
    expect(calculatePickResult("MONEYLINE", AWAY_SEL_ML, 0, HOME, 2, 4, "icehockey_nhl", VIS)).toBe("WIN");
  });

  it("away ML pick loses when the away team loses", () => {
    expect(calculatePickResult("MONEYLINE", AWAY_SEL_ML, 0, HOME, 4, 2, "icehockey_nhl", VIS)).toBe("LOSS");
  });

  it("home ML pick still settles correctly in the same matchup", () => {
    expect(calculatePickResult("MONEYLINE", HOME_SEL_ML, 0, HOME, 4, 2, "icehockey_nhl", VIS)).toBe("WIN");
  });
  ...
  it("away SPREAD pick (away -1.5, home line +1.5) loses when away wins by only 1", () => {
    ...
    expect(calculatePickResult("SPREAD", AWAY_SEL_SPREAD, 1.5, HOME, 2, 3, "icehockey_nhl", VIS)).toBe("LOSS");
  });
});
...
describe("calculatePickResult — home identifier is a prefix of the away identifier (no mis-settle)", () => {
  ...
  it("away SPREAD pick (LAC +3) LOSES when home covers -3", () => {
    ...
    expect(calculatePickResult("SPREAD", "LAC +3.0", -3, HOME, 27, 20, NFL, VIS)).toBe("LOSS");
  });

  it("away MONEYLINE pick (LAC ML) LOSES when the home team (LA) wins", () => {
    // pickedHome=false; homeWon=true → pickedHome !== homeWon → LOSS.
    expect(calculatePickResult("MONEYLINE", "LAC ML (+120)", 0, HOME, 27, 20, NFL, VIS)).toBe("LOSS");
  });
  ...
  it("home SPREAD pick (LA -3) WINS when home covers -3 (boundary match still fires)", () => {
    ...
    expect(calculatePickResult("SPREAD", "LA -3.0", -3, HOME, 27, 20, NFL, VIS)).toBe("WIN");
  });

  it("home MONEYLINE pick (LA ML) WINS when the home team wins", () => {
    expect(calculatePickResult("MONEYLINE", "LA ML (-140)", 0, HOME, 27, 20, NFL, VIS)).toBe("WIN");
  });
});
...
describe("calculatePickResult — no fabricated results", () => {
  it("throws on an unsupported pick type instead of silently returning PUSH", () => {
    ...
    expect(() =>
      ...
      calculatePickResult("PARLAY", "Team A", 0, "Team A", 21, 17, "americanfootball_nfl", VIS),
    ).toThrow(/unsupported pickType/i);
  });

  it("still settles the three supported pick types correctly", () => {
    expect(calculatePickResult("MONEYLINE", "Team A", 0, "Team A", 21, 17, "x", VIS)).toBe("WIN");
    expect(calculatePickResult("SPREAD", "Team A", -3, "Team A", 21, 17, "x", VIS)).toBe("WIN");
    expect(calculatePickResult("TOTAL", "OVER 40", 40, "Team A", 21, 17, "x", VIS)).toBe("LOSS");
  });
});
