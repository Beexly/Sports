...
describe("buildCalibrationLadder", () => {
  ...
  it("REJECTS the in-sample-leakage trap: time-clustered outcomes do NOT activate", () => {
    // Same 50%â†’70% miscalibration, but with all wins packed first. An in-sample
    // validator would fit 0.5â†’1.0 on the (all-win) past and call it improved.
    // The held-out future (all losses) exposes that as worse than raw â†’ identity.
  ...
  it("apply maps 0â€“100 â†’ [0,1] and rejects non-finite input", () => {
