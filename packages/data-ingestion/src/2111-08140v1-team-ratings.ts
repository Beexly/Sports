 ...
 *
 * arXiv:2111.08140v1 · lane:team_ratings · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Replace fixed-scale Elo in the ratings lane with a dynamic whole-history BT logistic model on
 * nflverse 2015-2025: weekly team strengths follow a Wiener process (Stan, ported from the paper's
 * skeleton), with the rating-scale slope m co-estimated each season (so 'a 100-point gap means X win
 * probability' is recalibrated, not asserted) plus opponent-uncertainty hierarchical error and
 * per-game weather/rest covariates; the learned slope stability across seasons is itself a reported
 * calibration metric.
 *
 * ACCEPTANCE GATE: Adopt iff the Wiener-drift + learned-slope model beats fixed-scale Elo log-loss by >=1.0% on the
 * 2024-2025 rolling window AND the learned slope m-hat is stable across seasons (coefficient of
 * variation <= 10%); reject and keep the simpler dynamic-Elo lane otherwise.
 *
...
export const ARXIV_ID = "2111.08140v1" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `Adopt iff the Wiener-drift + learned-slope model beats fixed-scale Elo log-loss by >=1.0% on the 2024-2025 rolling window AND the learned slope m-hat is stable across seasons (coefficient of variation <= 10%); reject and keep the simpler dynamic-Elo lane otherwise.`;
