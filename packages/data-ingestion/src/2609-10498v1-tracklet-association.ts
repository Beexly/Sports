 ...
 *
 * arXiv:2609.10498v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Port Field Converter's geometry-initialized temporal residual refinement to NFL broadcast as a world-grounded player-pose component of the NGS-replacement pipeline, grounding pose estimates in field geometry before residual refinement.
 *
 * ACCEPTANCE GATE:
 * ADAPT (adopt the NFL port) if: on held-out games, the residual-refined root error is < 50% of the geometry-only error AND < 25 cm absolute; reject otherwise (geometry alone or direct regression suffices, or broadcast calibration noise dominates).
 *
...
export const ARXIV_ID = "2609.10498v1" as const;
...
export const ACCEPTANCE_GATE = `ADAPT (adopt the NFL port) if: on held-out games, the residual-refined root error is < 50% of the geometry-only error AND < 25 cm absolute; reject otherwise (geometry alone or direct regression suffices, or broadcast calibration noise dominates).`;
