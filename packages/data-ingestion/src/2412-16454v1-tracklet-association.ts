 ...
 *
 * arXiv:2412.16454v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * Add a synchronized audio stream (broadcast crowd/whistle/pad-crack audio) as a second modality to the VideoMAE action classifier — a two-tower VideoMAE + audio-MAE model on tackle-type/block-shed labels — so audio disambiguates visually occluded contact frames (tackles, blocks, catches) where pure video confuses classes.
 *
 * ACCEPTANCE GATE:
 * Adopt the video-classification lane if the VideoMAE model beats the pose-based baseline by ≥10 percentage points of test accuracy on the week 16–18 window AND per-class F1 ≥ 0.70 on all 8 classes.
 *
...
export const ARXIV_ID = "2412.16454v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt the video-classification lane if the VideoMAE model beats the pose-based baseline by ≥10 percentage points of test accuracy on the week 16–18 window AND per-class F1 ≥ 0.70 on all 8 classes.`;
