 ...
 *
 * arXiv:2412.06794v1 · lane:nlp · verdict:ADAPT · owner:Motif-lab · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Replace index-level news sentiment averages with entity-level sentiment: attribute each article's sentiment to specific teams/players via NER plus coreference, then build team-day sentiment features — because in sports, whose news it is matters more than how much news there is.
 *
 * ACCEPTANCE GATE:
 * Adopt if sentiment features reduce RMSE by ≥3% vs the no-news baseline on the embargoed 2024 test AND directional accuracy improves by ≥1pp; reject otherwise.
 *
...
export const ARXIV_ID = "2412.06794v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt if sentiment features reduce RMSE by ≥3% vs the no-news baseline on the embargoed 2024 test AND directional accuracy improves by ≥1pp; reject otherwise.`;
