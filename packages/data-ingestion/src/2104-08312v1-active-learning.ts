 ...
 *
 * arXiv:2104.08312v1 · lane:active_learning · verdict:ADAPT · owner:Motif-lab
 *
 ...
 *
 * Improvement (wiring record): Training-data audit: fix the engine feature space (engineered features remove the
 * representation-learning caveat); train a KNN (or gradient-boosted) classifier on engine features for
 * binary cover/over; compute exact KNN-Shapley values (linear-time DP) of each training game/season
 * against a held-out validation slate (e.g., 2024 season); rank; drop or down-weight negative-value
 * games; retrain; feed valuation: for each candidate feed (nflverse, FTN charting subset, odds
 * history), compute the block-Shapley of the feed's data block via the same KNN proxy -- renew feeds
 * with positive value, drop/renegotiate the rest.
 *
