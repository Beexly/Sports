 ...
 *
 * arXiv:2501.08710v2 · lane:calibration · verdict:ADAPT · owner:Motif-lab · doctrine:BASELINE
 *
 ...
 *
 * Improvement (record):
 * Adopt a disentangled multi-task architecture for the win/spread/total heads, replacing the Naïve-Bayes marginal loss with a supervised contrastive loss on regime labels (dropping the paper's failed conditional-independence assumption) and learning noisy regime labels jointly via a VQ-VAE codebook instead of hand labels.
 *
 * ACCEPTANCE GATE:
 * Adopt the architecture if, on 2024–2025 walk-forward, the disentangled multi-task model beats the non-disentangled shared-encoder baseline by ≥0.003 Brier (win prob) or ≥0.1 points MAE vs. close on spread/total, AND achieves MIG ≥ 2× the baseline's.
 *
...
export const ARXIV_ID = "2501.08710v2" as const;
...
export const ACCEPTANCE_GATE = `Adopt the architecture if, on 2024–2025 walk-forward, the disentangled multi-task model beats the non-disentangled shared-encoder baseline by ≥0.003 Brier (win prob) or ≥0.1 points MAE vs. close on spread/total, AND achieves MIG ≥ 2× the baseline's.`;
