 ...
 *
 * arXiv:2412.15673v1 · lane:tracking · verdict:ADAPT · owner:Motif-lab · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * Add tactic-conditioned diffusion trajectory prediction to the tracking lane and turn it into a live-pricing scheme-sensitivity engine: generate trajectories under counterfactual defensive schemes (e.g., Cover 2 instead of recognized Cover 3) and widen live-prop uncertainty bands on plays where the factual-vs-counterfactual ΔE[outcome] is large.
 *
 * ACCEPTANCE GATE:
 * Adopt if the tactic-conditioned model beats the unconditioned diffusion baseline by ≥8% minADE on the weeks 17–18 test window AND auxiliary scheme classification Top-1 ≥ 55%; reject if conditioning gains vanish when tactics are recognized (not given).
 *
...
export const ARXIV_ID = "2412.15673v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt if the tactic-conditioned model beats the unconditioned diffusion baseline by ≥8% minADE on the weeks 17–18 test window AND auxiliary scheme classification Top-1 ≥ 55%; reject if conditioning gains vanish when tactics are recognized (not given).`;
