 ...
 *
 * arXiv:2506.02351v1 · lane:win_spread_total · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Port DIAMOND to NFL as the automated first pass for weekly recap content and clip selection: replace the paper's hand-set scoring heuristics (1–60 bands, +1..+20 LLM adjustments, LI rank bonus) with a learned ranker trained on (play features, LLM narrative embedding) → human preference labels from GSE's own recap-selection history, optimizing pairwise ranking loss against broadcast ground truth.
 *
 * ACCEPTANCE GATE:
 * ADOPT as a GSE content tool if the NFL port beats WPA-only selection on F1 (≥5pp) and blinded narrative preference (≥60%) on the 20-game test; REJECT the LLM stages if no gain — ship the WPA+LI selector alone.
 *
...
export const ARXIV_ID = "2506.02351v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT as a GSE content tool if the NFL port beats WPA-only selection on F1 (≥5pp) and blinded narrative preference (≥60%) on the 20-game test; REJECT the LLM stages if no gain — ship the WPA+LI selector alone.`;
