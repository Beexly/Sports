 ...
 *
 * arXiv:2511.17733v1 · lane:win_spread_total · verdict:ADAPT · owner:Motif-lab · doctrine:BASELINE
 *
 ...
 *
 * Improvement (record):
 * GSE adopts hierarchical Bayesian log5 matchup models: QB-vs-defensive-unit and OL-vs-DL per-play success/turnover/explosive-play probabilities with scheme-matchup offsets and recency weighting, fit with NUTS.
 *
 * ACCEPTANCE GATE:
 * ADOPT if the unit-matchup model shows >=0.5% relative GMP improvement over the current baseline AND the decision simulation shows >=0.1 added wins/season attributable to the matchup upgrade.
 *
...
export const ARXIV_ID = "2511.17733v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT if the unit-matchup model shows >=0.5% relative GMP improvement over the current baseline AND the decision simulation shows >=0.1 added wins/season attributable to the matchup upgrade.`;
