 ...
 *
 * arXiv:2508.13396v1 · lane:data_infra · verdict:ADAPT · owner:Hermes · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * GSE standardizes on Delta Lake for gold feature tables and the ML training path (Spark-native integration, schema enforcement at write time, time travel), adding Iceberg only if a second query engine enters the picture.
 *
 * ACCEPTANCE GATE:
 * CONFIRM Delta iff on GSE's 5-season replay: Delta's point-in-time range-query latency is within 20% of Iceberg's AND total storage is within 15% of Iceberg's.
 *
...
export const ARXIV_ID = "2508.13396v1" as const;
...
export const ACCEPTANCE_GATE = `CONFIRM Delta iff on GSE's 5-season replay: Delta's point-in-time range-query latency is within 20% of Iceberg's AND total storage is within 15% of Iceberg's.`;
