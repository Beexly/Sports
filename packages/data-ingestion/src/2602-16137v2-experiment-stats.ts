 ...
 *
 * arXiv:2602.16137v2 · lane:experimental · verdict:ADAPT · owner:Motif-lab · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * GSE uses binary-assortment experiment designs to detect substitution between DFS contest types (entry-share shifts when contest menus change) and to A/B test X content variants efficiently, avoiding the trap of mistaking correlated variants for independent ones.
 *
 * ACCEPTANCE GATE:
 * ADAPT if the synthetic replication matches the paper's best-at-all-data-sizes claim directionally (mean Rand index >= Benson-et-al.-2016 baseline + 0.05) before any GSE-data application.
 *
...
export const ARXIV_ID = "2602.16137v2" as const;
...
export const ACCEPTANCE_GATE = `ADAPT if the synthetic replication matches the paper's best-at-all-data-sizes claim directionally (mean Rand index >= Benson-et-al.-2016 baseline + 0.05) before any GSE-data application.`;
