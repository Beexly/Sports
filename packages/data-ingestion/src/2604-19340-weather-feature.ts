 ...
 *
 * arXiv:2604.19340 · lane:weather · verdict:ADAPT · owner:Hermes · doctrine:SITUATIONAL
 *
 ...
 *
 * Improvement (record):
 * Build weather/lgbm_postprocess.py: LightGBM post-processing of HRRR forecasts at all 30 NFL stadiums (11x11 surface + 7x7 pressure-level windows at kickoff lead times, FS0 tau=0.9, Optuna tuned) for temp/wind/precip, paired with the ANET2 probabilistic model as mean + uncertainty.
 *
 * ACCEPTANCE GATE:
 * ADOPT if LightGBM beats raw HRRR by ≥ 10% RMSE on wind speed AND ≥ 5% on temperature on the 2023 holdout (matching the paper's margin scale).
 *
...
export const ARXIV_ID = "2604.19340" as const;
...
export const ACCEPTANCE_GATE = `ADOPT if LightGBM beats raw HRRR by ≥ 10% RMSE on wind speed AND ≥ 5% on temperature on the 2023 holdout (matching the paper's margin scale).`;
