 ...
 *
 * arXiv:2501.16565v1 · lane:team_ratings · verdict:ADAPT · owner:Mimo · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Model division winners with position-1-breaking Plackett-Luce and extend to the full NFL playoff seed order (7 seeds × 2 conferences, 2002–2025) to test whether entire seed orders carry joint information worth modeling, or whether pairwise/first-place-only structure suffices as in the paper's APA result.
 *
 * ACCEPTANCE GATE:
 * Adopt position-1-breaking PL for division-winner modeling if, on the 2021–2025 time-ordered test, it matches or beats the market baseline on log-loss (within 0.02) AND beats pairwise BT on top-1 hit rate by ≥1 additional correct division.
 *
...
export const ARXIV_ID = "2501.16565v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt position-1-breaking PL for division-winner modeling if, on the 2021–2025 time-ordered test, it matches or beats the market baseline on log-loss (within 0.02) AND beats pairwise BT on top-1 hit rate by ≥1 additional correct division.`;
