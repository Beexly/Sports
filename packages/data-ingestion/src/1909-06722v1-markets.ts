 ...
 *
 * arXiv:1909.06722v1 · lane:markets · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): Train the GSE pick-ranker with the Plackett-Luce listwise loss (each weekly slate a query, each
 * candidate bet a document, graded relevance = realized CLV bucket -- an engine-honesty training
 * signal, not a profit objective) using boosted trees when per-slate feature richness is low,
 * replacing pairwise LambdaMART-style ranking.
 *
...
export const ARXIV_ID = "1909.06722v1" as const;
export const LANE = "markets" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADAPT confirmed if the PL-loss ranker beats the pairwise ranker on 2024-slate NDCG@10 by >=0.005 OR top-decile realized CLV is higher with p<0.1. If parity, the instability design rule still stands as the takeaway.`;
