 ...
 *
 * arXiv:2605.29395v2 · lane:team_ratings · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Fit a low-rank context-specific team-strength matrix (rows = game-context clusters from spread/total/rest/dome/divisional, columns = 32 teams; nuclear-norm + refinement BTL on outcomes) for early-season sparse contexts, and run three-way pick certification (certified vs unresolved) on the weekly published sheet.
 *
 * ACCEPTANCE GATE:
 * Adopt the low-rank context-rating estimator if it beats per-context BTL on next-season SU log-loss by ≥ 0.003/game averaged over the 2023–2024 test seasons AND produces strictly narrower mean rank CIs at ≥ nominal coverage on a 2024 holdout.
 *
...
export const ARXIV_ID = "2605.29395v2" as const;
...
export const ACCEPTANCE_GATE = `Adopt the low-rank context-rating estimator if it beats per-context BTL on next-season SU log-loss by ≥ 0.003/game averaged over the 2023–2024 test seasons AND produces strictly narrower mean rank CIs at ≥ nominal coverage on a 2024 holdout.`;
