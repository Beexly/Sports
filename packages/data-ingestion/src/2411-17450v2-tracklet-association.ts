 ...
 *
 * arXiv:2411.17450v2 · lane:tracking · verdict:ADAPT · owner:Motif-lab · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * Build a temporal per-frame GNN lane (CrystalConv per frame → temporal attention over a 1-second window) predicting forward-looking play success, adding a scheme-family node/graph feature, to resolve the static model's 40–60% midfield coin-flip problem where single frames cannot read developing leverage.
 *
 * ACCEPTANCE GATE:
 * Adopt the per-frame GNN lane if the scheme-family-specific GNNs beat the pooled GNN by ≥0.02 ROC-AUC on weeks 16–18 AND beat the logistic baseline by ≥0.05 ROC-AUC, with ECE ≤ 0.10 after recalibration.
 *
...
export const ARXIV_ID = "2411.17450v2" as const;
...
export const ACCEPTANCE_GATE = `Adopt the per-frame GNN lane if the scheme-family-specific GNNs beat the pooled GNN by ≥0.02 ROC-AUC on weeks 16–18 AND beat the logistic baseline by ≥0.05 ROC-AUC, with ECE ≤ 0.10 after recalibration.`;
