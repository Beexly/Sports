 ...
 *
 * arXiv:1601.00991v1 · lane:signal_discovery_alpha_mining · verdict:ADOPT · owner:Motif-lab
 *
 ...
 *
 * Improvement (wiring record): Build the 'SportsAlpha' formulaic factor miner: team-game panel from nflverse (EPA/play, success
 * rate, explosiveness, pressure rate, yards/play, turnover margin, pace, rest days) plus market inputs
 * (closing line, line movement, total movement) with division-neutralization, random formula
 * generation + GP mutation over the Appendix-A operator set (rank, delay, ts_rank, signedpower,
 * decay_linear, ...) scored by out-of-sample IC vs spread-cover, keeping factors with pairwise |corr|
 * < 0.5 to build a factor zoo combined into a game-level edge signal calibrated to spread/total
 * probabilities.
 *
...
export const ARXIV_ID = "1601.00991v1" as const;
export const LANE = "signal_discovery_alpha_mining" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT if: mined factor zoo achieves mean out-of-sample |IC| >= 2x the hand-built baseline IC on the 2023-2025 test block AND mean pairwise |corr| <= 0.25 AND the combined signal improves base-model Brier by >= 0.002 on held-out season with deflated-Sharpe / reality-check multiple-testing gate passed.`;
