 ...
 *
 * arXiv:2509.26325v2 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * GSE restores low-quality film (old All-22, compressed clips, grainy sideline footage) with a V3 super-resolution enhancement layer before visual review or detection/pose pipelines — never as a tracking-data source.
 *
 * ACCEPTANCE GATE:
 * Adopt if V3-restored frames beat bicubic by >=2 dB PSNR on the realistic NFL degradation set AND the detection-mAP check on restored frames recovers >=50% of the mAP lost to degradation.
 *
...
export const ARXIV_ID = "2509.26325v2" as const;
...
export const ACCEPTANCE_GATE = `Adopt if V3-restored frames beat bicubic by >=2 dB PSNR on the realistic NFL degradation set AND the detection-mAP check on restored frames recovers >=50% of the mAP lost to degradation.`;
