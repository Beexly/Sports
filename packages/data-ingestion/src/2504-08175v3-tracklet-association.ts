 ...
 *
 * arXiv:2504.08175v3 · lane:tracking · verdict:ADAPT · owner:Motif-lab · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Add the iLQR physics-based collision-refinement stage to GSE's pose pipeline, replacing the paper's empirical loss-weight schedule with automated Bayesian weight-tuning over w_1…w_6 and testing capsule-based collision geometry to see whether the Hi4D penetration regression is a geometry-resolution artifact.
 *
 * ACCEPTANCE GATE:
 * ADOPT the iLQR collision-refinement stage only if, on the paper's released supplementary dataset, it beats the kinematics-only baseline by the paper's own margins (≥10% reduction in e_MPJPE and ≥50% reduction in e_foot,vxy); otherwise REJECT.
 *
...
export const ARXIV_ID = "2504.08175v3" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the iLQR collision-refinement stage only if, on the paper's released supplementary dataset, it beats the kinematics-only baseline by the paper's own margins (≥10% reduction in e_MPJPE and ≥50% reduction in e_foot,vxy); otherwise REJECT.`;
