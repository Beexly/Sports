...
export const SOURCE_REGISTRY: Readonly<Record<string, LegalSource>> = {
  ...
  "espn-hidden-api": {
    ...
    reason:
      ...
      "General/bulk ESPN JSON stays forbidden; the ONE narrow founder-approved carve-out is " +
      "'galaxy-espn-inline' (scoreboard inline odds facts, low volume) — see that entry.",
    baseUrl: null,
    datasets: [],
  ...
  "galaxy-espn-inline": {
    ...
    attributionText: null,
    robotsRespected: true,
    rateLimit: "Scoreboard: ~4 requests/sport/cycle; core /odds fallback capped at 24 events/cycle; every request carries a timeout.",
    verdict: "use-with-caution",
    reason:
      "FOUNDER DECISION 2026-08-27 (recorded, do not silently widen): GSE is its own provider — the Galaxy " +
  ...
  therundown: {
    ...
    verdict: "use-with-caution",
    reason:
      "Official odds vendor (DK/FD/MGM/Pinnacle + Kalshi/Polymarket, same schema). Client already exists as Odds-API failover. Free tier is self-serve; commercial display should match their plan. HOW: RUNDOWN_API_KEY. Not a scrape.",
  ...
  sportsbookish: {
    ...
    verdict: "paid-required",
    reason:
      "Vendor that publishes Kalshi implied vs US-book consensus. Live API commercial rights are a paid plan. Daily HF snapshot is CC-BY-4.0 with attribution. Do not burn the public demo Bearer from their README. HOW: buy a key, then flip this verdict to licensed.",
  ...
  "smartstake-mlb-props": {
    ...
    verdict: "forbidden",
    reason:
      "621M rows of minute-by-minute player-prop odds including Bet365, DraftKings, FanDuel. Card says research/educational. Redistributing Bet365/DK/FD quotes into a paid SaaS is the same ToS class we already refuse to scrape. HOW for live props: Odds API event-odds / TheRundown / SharpAPI paid — not this tape.",
  ...
  "sportsbook-software-json-api": {
    ...
    verdict: "forbidden",
    reason:
      "GitHub sportsbook-json-php is an API for THEIR sportsbook's logged-in players (account, bets, casino), not a public multi-book odds dump. Market-data functions require a sales subscription. GSE is not an Entertastic operator. HOW: TheRundown / Odds API.",
    baseUrl: null,
    datasets: [],
  ...
  "huggingface-kalshi-api-dump": {
    ...
    verdict: "forbidden",
    reason:
      "RooseveltHonaker/kalshi-trades-data (and TrevorJS/kalshi-trades, thomaswmitch/kalshi-prediction-markets-*) are third-party archives of Kalshi market-data API responses. The dataset card itself says users must comply with the Kalshi Developer Agreement. Dev Agreement v1.1 §3 bans collecting/caching/aggregating/storing API data except for own-account trading and bans sharing with third parties. Kalshi Data Terms (kalshi-public-docs S3 PDF) further restrict Kalshi Data to personal non-commercial use and bar ML training / redistributed archives without written consent. HOW: PredExon (vendor, key held) not the HF dump.",
  ...
  "convokit-sportsbook-reddit": {
    ...
    verdict: "forbidden",
    reason:
      "Academic Reddit comment dumps, not sportsbook odds or official DK/FD feeds. They do not legalize DraftKings/FanDuel HTML or hidden-API scrape. GSE already has a gated Reddit narrative path; do not treat ConvoKit as a line source.",
  ...
  prophetx: {
    ...
    verdict: "paid-required",
    reason:
      "CFTC-regulated DCM. Market Data API is a read-only display/analytics SKU for partners — not the Trading API (orders/wallet). No self-serve key; apply as a display partner. Until written access, GSE does not call ProphetX. Do not scrape prophetx.co HTML.",
  ...
  novig: {
    ...
    verdict: "paid-required",
    reason:
      "Live Novig books require the authenticated API at api.novig.us (OAuth). Not a public unauthenticated odds feed. Founder-gated until we hold credentials and a commercial-use grant. Public daily CSVs are a separate source (novig-public-csv).",
