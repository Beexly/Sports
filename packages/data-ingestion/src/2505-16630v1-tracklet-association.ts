 ...
 *
 * arXiv:2505.16630v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Build GSE's broadcast-video understanding QA pipeline on the SoccerChat recipe: turn NFL broadcast video into a ~49k-pair QA instruction dataset (the "labeling factory" pattern with the QA-generation layer added), training jointly on multiple datasets rather than sequentially (joint > sequential is the empirical rule) on top of the strongest general pretraining base available.
 *
 * ACCEPTANCE GATE:
 * Ledger states no numeric gate; the durable contributions are the reproducible instruction-dataset pipeline (49,120 QA pairs, 90,834 clips) and the empirical rule that joint multi-dataset training beats sequential fine-tuning.
 *
...
export const ARXIV_ID = "2505.16630v1" as const;
...
export const ACCEPTANCE_GATE = `Ledger states no numeric gate; the durable contributions are the reproducible instruction-dataset pipeline (49,120 QA pairs, 90,834 clips) and the empirical rule that joint multi-dataset training beats sequential fine-tuning.`;
