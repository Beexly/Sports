 ...
 *
 * arXiv:2608.23808v2 · lane:signal_discovery_alpha_mining · verdict:ADOPT · owner:Mimo · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Build gse.validation.minerva as a statistical robustness grade for every mined GSE signal: DSR deflated by the actual candidate count, PBO over season partitions, SPA vs the benchmark signal set, MinTRL, and a regime composite, with seal thresholds recalibrated once N > 1,000 scored signals, so only signals that survive the full robustness battery reach production.
 *
 * ACCEPTANCE GATE:
 * ADOPT if on permuted-label nulls the sports MinervaScore achieves AUROC >= 0.95 separating null from historically-profitable signals AND the Seal's pass rate on null signals is <= 5% (false-seal control); REJECT (fall back to plain deflated-Sharpe + PBO) if the aggregation adds no discrimination over DSR-alone.
 *
...
export const ARXIV_ID = "2608.23808v2" as const;
...
export const ACCEPTANCE_GATE = `ADOPT if on permuted-label nulls the sports MinervaScore achieves AUROC >= 0.95 separating null from historically-profitable signals AND the Seal's pass rate on null signals is <= 5% (false-seal control); REJECT (fall back to plain deflated-Sharpe + PBO) if the aggregation adds no discrimination over DSR-alone.`;
