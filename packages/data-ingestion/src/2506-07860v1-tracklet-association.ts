 ...
 *
 * arXiv:2506.07860v1 · lane:tracking · verdict:ADAPT · owner:Motif-lab · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Adapt the event-based ball trajectory predictor to football flight with a drag+EKF model plus a Magnus/lift term whose spin-axis parameter is estimated from the ball ellipse's rotation across frames — testing whether spin-aware prediction beats drag-only on punts (highest spin) vs kickoffs.
 *
 * ACCEPTANCE GATE:
 * PURSUE the football flight-model adaptation if the drag+EKF model beats the constant-velocity baseline by ≥ 2 m on landing-point RMSE over the 20-clip window; otherwise REJECT and treat ball-flight estimation as buy-not-build (NGS air-distance family).
 *
...
export const ARXIV_ID = "2506.07860v1" as const;
...
export const ACCEPTANCE_GATE = `PURSUE the football flight-model adaptation if the drag+EKF model beats the constant-velocity baseline by ≥ 2 m on landing-point RMSE over the 20-clip window; otherwise REJECT and treat ball-flight estimation as buy-not-build (NGS air-distance family).`;
