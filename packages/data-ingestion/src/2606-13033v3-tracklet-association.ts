 ...
 *
 * arXiv:2606.13033v3 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Adopt the margin-dispatched orchestration pattern for GSE's broadcast-footage tracking lane: base tracker (ByteTrack) every frame, expensive modules (VOS mask propagation, jersey OCR) firing only on margin-signal windows, with the asymmetric override rule (expensive output overrides the base only when confident AND contradicting), plus NFL GTA (pose-guided jersey OCR + team classification).
 *
 * ACCEPTANCE GATE:
 * ADAPT the dispatch + gated-correction design if, on the NFL annotation set, adding margin-dispatched VOS improves AssA by ≥ 3.0 points over the base tracker with the same detections, and false-positive SWAPs stay below 10% of SWAP windows.
 *
...
export const ARXIV_ID = "2606.13033v3" as const;
...
export const ACCEPTANCE_GATE = `ADAPT the dispatch + gated-correction design if, on the NFL annotation set, adding margin-dispatched VOS improves AssA by ≥ 3.0 points over the base tracker with the same detections, and false-positive SWAPs stay below 10% of SWAP windows.`;
