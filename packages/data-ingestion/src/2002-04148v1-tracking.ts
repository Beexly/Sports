 ...
 *
 * arXiv:2002.04148v1 · lane:tracking · verdict:ADAPT · owner:Motif-lab
 *
 ...
 *
 * Improvement (wiring record): Build an NFL play-complexity (ID) feature from Big Data Bowl tracking: per-frame posterior-median
 * intrinsic dimension via the two-NN Pareto-mixture (Hidalgo, K=3); use pre-snap motion ID as an
 * EPA/play-action-success predictor, post-snap route-combination ID as a defense-confusion feature,
 * and team-level ID as a weekly scheme-unpredictability rating.
 *
...
export const ARXIV_ID = "2002.04148v1" as const;
export const LANE = "tracking" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `Adopt the ID feature into GSE's NGS lane if post-snap peak ID adds >=0.005 out-of-sample R-squared to EPA/play over the down-distance-yardline-shotgun baseline, OR pre-snap motion ID beats the motion-flag dummy by >=0.01 AUC on play-action success -- weeks 13-18 holdout, direction matching the paper.`;
