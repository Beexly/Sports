/**
 * 1717 From evidence-based medicine to knowledge graph: retrieval-augmented generation for sports rehabilitation and a domain benchmark
 *
 * arXiv:2601.00216 · lane:nlp · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE builds a tiered evidence-retrieval pipeline for injury news (T1 official reports outrank T4 social rumors), with schema-guided HyDE query expansion, a monotone-bias pairwise reranker, and faithfulness-constrained generation so every injury claim cites a retrieved source.
 *
 * ACCEPTANCE GATE:
 * ADAPT the architecture (tiered pools + schema-guided HyDE + monotone-bias reranker + faithfulness metric); proceed if the reproducible test meets faithfulness and conflict-resolution criteria on GSE's injury corpus.
 *
...
export const ARXIV_ID = "2601.00216" as const;
...
export const ACCEPTANCE_GATE = `ADAPT the architecture (tiered pools + schema-guided HyDE + monotone-bias reranker + faithfulness metric); proceed if the reproducible test meets faithfulness and conflict-resolution criteria on GSE's injury corpus.`;
