 ...
 *
 * arXiv:2512.08591v1 · lane:win_spread_total · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE stacks a long-sequence LSTM/GRU prior over ~150-200 team games inside the existing ensemble as a fallback memory layer for regime shifts.
 *
 * ACCEPTANCE GATE:
 * ADOPT the LSTM-fallback prior if the stacked ensemble's log-loss on the 2023-2024 holdout beats the no-prior baseline by >=0.005 AND the LSTM component alone beats logistic regression on the same features.
 *
...
export const ARXIV_ID = "2512.08591v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the LSTM-fallback prior if the stacked ensemble's log-loss on the 2023-2024 holdout beats the no-prior baseline by >=0.005 AND the LSTM component alone beats logistic regression on the same features.`;
