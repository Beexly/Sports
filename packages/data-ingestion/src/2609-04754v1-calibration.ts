 ...
 *
 * arXiv:2609.04754v1 · lane:calibration · verdict:ADAPT · owner:Mimo · doctrine:SITUATIONAL
 *
 ...
 *
 * Improvement (record):
 * Build GSE-Cal, a post-hoc calibration layer for the engine's spread/total/WP outputs: per-bucket empirical residual maps of engine predictions vs outcomes over score/time/weather/referee-crew buckets from nflverse 2015-2025, corrected by a small residual model with a zero-initialized final layer trained with Huber loss on temporal splits.
 *
 * ACCEPTANCE GATE:
 * ADOPT the calibration-layer approach if, on the 2024-2025 test window: (a) the layer reduces mean |bias| per state-bucket by >=15% on totals without increasing overall RMSE vs the uncorrected engine, and (b) WFR_5 improves by >=2 percentage points; REJECT if RMSE increases on the test window or the bucketed bias reduction is <10%.
 *
...
export const ARXIV_ID = "2609.04754v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the calibration-layer approach if, on the 2024-2025 test window: (a) the layer reduces mean |bias| per state-bucket by >=15% on totals without increasing overall RMSE vs the uncorrected engine, and (b) WFR_5 improves by >=2 percentage points; REJECT if RMSE increases on the test window or the bucketed bias reduction is <10%.`;
