 ...
 *
 * arXiv:2006.08189v1 · lane:team_ratings · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Build a weekly League Competitiveness Score content metric: estimate the cross-sectional skill
 * distribution of NFL teams each season from win/loss data (rank-centrality + deconvolution KDE) and
 * report -h(P-hat) as a narrative stat ('2026 is the most wide-open NFL season since...'), plus
 * division-level per-season scores; descriptive only, never wired into the prediction engine.
 *
 * ACCEPTANCE GATE: ADOPT for content production if, on the 2015-2025 nflverse window, |Spearman(negative entropy,
 * same-season upset rate)| >= 0.5 AND it exceeds the Elo-stdev baseline correlation by >= 0.15;
 * otherwise REJECT as a content metric.
 *
...
export const ARXIV_ID = "2006.08189v1" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT for content production if, on the 2015-2025 nflverse window, |Spearman(negative entropy, same-season upset rate)| >= 0.5 AND it exceeds the Elo-stdev baseline correlation by >= 0.15; otherwise REJECT as a content metric.`;
