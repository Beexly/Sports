 ...
 *
 * arXiv:2409.09607 · lane:weather · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Add data-augmented CNN ensemble post-processing to GSE's weather pipeline with stadium-specific
 * post-processors (conditioned on bowl openness and elevation) to fix the paper's mountain-grid
 * failure, and apply the same augmentation to dual-resolution wind tails where truncated-normal EMOS
 * is weakest.
 *
...
export const ARXIV_ID = "2409.09607" as const;
export const LANE = "weather" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT if augmentation improves Brier score at tail thresholds by ≥ 5% on hold-out extreme games. REJECT if no gain.`;
