 ...
 *
 * arXiv:2609.00905v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Build a Pref-MH exact-MCMC slate sampler for GSE's DFS lineup generation: pairwise slate preferences elicited through a BT-consistent judge, with exact sampling from the induced preference distribution replacing temperature-scaled i.i.d. draws.
 *
 * ACCEPTANCE GATE:
 * ADAPT (adopt as GSE's slate sampler) if: Pref-MH slates achieve >= 15% higher realized mean score at equal-or-better diversity than i.i.d. p0 draws on walk-forward 2025 weeks, AND the judge passes BT-consistency diagnostics (transitivity >= 95%, vote independence); reject otherwise.
 *
...
export const ARXIV_ID = "2609.00905v1" as const;
...
export const ACCEPTANCE_GATE = `ADAPT (adopt as GSE's slate sampler) if: Pref-MH slates achieve >= 15% higher realized mean score at equal-or-better diversity than i.i.d. p0 draws on walk-forward 2025 weeks, AND the judge passes BT-consistency diagnostics (transitivity >= 95%, vote independence); reject otherwise.`;
