 ...
 *
 * arXiv:2410.01304v1 · lane:experimental · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): Build a two-stream (video + NGS tracking coordinates as auxiliary neck input) action spotter for NFL
 * core classes (snap, handoff, pass release, off-ball actions) as GSE's charting pipeline, which
 * neither pure-video nor pure-tracking methods can match on unshown/off-ball actions.
 *
 * ACCEPTANCE GATE: Adopt (proceed to scale-up) if the fine-tuned spotter reaches mAP@1 ≥ 0.60 on the 3 held-out NFL
 * games for the core classes (snap, handoff, pass release). Reject/shelve otherwise.
 *
...
export const ARXIV_ID = "2410.01304v1" as const;
export const LANE = "experimental" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `Adopt (proceed to scale-up) if the fine-tuned spotter reaches mAP@1 ≥ 0.60 on the 3 held-out NFL games for the core classes (snap, handoff, pass release). Reject/shelve otherwise.`;
