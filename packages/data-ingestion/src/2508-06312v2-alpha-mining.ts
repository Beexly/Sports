 ...
 *
 * arXiv:2508.06312v2 · lane:signal_discovery_alpha_mining · verdict:ADAPT · owner:Motif-lab · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE mines alpha factors with an LLM generation/optimization chain: the model proposes formulas over nflverse and line-move features, parsed via a safe evaluator and backtested on RankIC, RankICIR, turnover, and novelty against the effective pool.
 *
 * ACCEPTANCE GATE:
 * ADAPT->build if the LLM chain's selected factor set beats the GP baseline's set by >=0.002 Brier on 2022-2025 AND at least 5 factors survive Benjamini-Hochberg (q<0.10) on the test block; REJECT if LLM factors are regurgitated known signals.
 *
...
export const ARXIV_ID = "2508.06312v2" as const;
...
export const ACCEPTANCE_GATE = `ADAPT->build if the LLM chain's selected factor set beats the GP baseline's set by >=0.002 Brier on 2022-2025 AND at least 5 factors survive Benjamini-Hochberg (q<0.10) on the test block; REJECT if LLM factors are regurgitated known signals.`;
