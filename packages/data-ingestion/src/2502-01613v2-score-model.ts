/**
 * Statistical Enhanced Learning for Modeling and Prediction of Tennis Matches at Grand Slam Tournaments
 *
 * arXiv:2502.01613v2 · lane:win_spread_total · verdict:ADAPT · owner:Mimo · doctrine:BASELINE
 *
 ...
 *
 * Improvement (record):
 * Adopt the enhanced feature set over the Elo-only logistic baseline for game-outcome modeling, and learn the shape of the age transform empirically — fit a 1D P-spline on QB age and roster mean age inside a GAM, then distill the fitted spline into a piecewise-linear feature — instead of hand-picking |age−30| / [28,32], producing an interpretable age-curve artifact.
 *
 * ACCEPTANCE GATE:
 * ADOPT the enhanced feature set if, pre-registered: pooled held-out log-loss improves by ≥ 0.005 over the Elo-only logistic baseline AND the gain appears in ≥ 5 of 7 held-out seasons (consistency, not one lucky year).
 *
...
export const ARXIV_ID = "2502.01613v2" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the enhanced feature set if, pre-registered: pooled held-out log-loss improves by ≥ 0.005 over the Elo-only logistic baseline AND the gain appears in ≥ 5 of 7 held-out seasons (consistency, not one lucky year).`;
