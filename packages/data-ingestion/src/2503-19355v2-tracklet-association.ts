 ...
 *
 * arXiv:2503.19355v2 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Ship a KinQA-tuned vision-language assistant for GSE content work that answers kinematic questions from video, trained on multi-step kinematic reasoning chains grounded in exact NGS labels (e.g., was separation at catch point above league-average NGS separation for this route type?) to exploit the label luxury the paper's pseudo-labeling lacked.
 *
 * ACCEPTANCE GATE:
 * ADOPT the KinQA-tuned assistant for GSE content work only if it reaches ≥55% accuracy on the 5,000-pair held-out set with paraphrased questions AND ≥95% on comparison questions; otherwise REJECT.
 *
...
export const ARXIV_ID = "2503.19355v2" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the KinQA-tuned assistant for GSE content work only if it reaches ≥55% accuracy on the 5,000-pair held-out set with paraphrased questions AND ≥95% on comparison questions; otherwise REJECT.`;
