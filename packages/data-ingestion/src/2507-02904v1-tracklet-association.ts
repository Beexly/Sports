 ...
 *
 * arXiv:2507.02904v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE builds film-analysis content tooling as a hybrid CV-to-text-to-LLM pipeline: cheap per-frame detectors convert footage to compact text (trajectories, ball positions, formation labels) for LLM narration and tactical Q&A, never MLLM-first video.
 *
 * ACCEPTANCE GATE:
 * PURSUE only if structured-feature prompting beats raw-frame prompting by >=25pp on event-sequence edit score on the 50-play window AND matches a cheap heuristic baseline.
 *
...
export const ARXIV_ID = "2507.02904v1" as const;
...
export const ACCEPTANCE_GATE = `PURSUE only if structured-feature prompting beats raw-frame prompting by >=25pp on event-sequence edit score on the 50-play window AND matches a cheap heuristic baseline.`;
