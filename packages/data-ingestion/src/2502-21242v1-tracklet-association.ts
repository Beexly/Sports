 ...
 *
 * arXiv:2502.21242v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Adopt jersey-number + team-ID + field-coordinate edge features for any GSE video-tracking prototype, with a constant-velocity Kalman predictor on field coordinates gating edge creation (only connect tracklets whose predicted positions overlap within a radius) — motion being the cue that survives jersey illegibility.
 *
 * ACCEPTANCE GATE:
 * ADOPT the jersey-number + team-ID + field-coordinate feature set for any GSE video-tracking prototype only if a pilot on 10 NFL broadcast plays shows jersey-number OCR legibility ≥60% of player-frames and the combined edge features reduce ID switches vs ByteTrack alone by ≥25%.
 *
...
export const ARXIV_ID = "2502.21242v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the jersey-number + team-ID + field-coordinate feature set for any GSE video-tracking prototype only if a pilot on 10 NFL broadcast plays shows jersey-number OCR legibility ≥60% of player-frames and the combined edge features reduce ID switches vs ByteTrack alone by ≥25%.`;
