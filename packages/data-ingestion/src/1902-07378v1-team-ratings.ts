 ...
 *
 * arXiv:1902.07378v1 · lane:team_ratings · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): Replace the dynamic-Elo ratings layer with a GP team-strength prior (Matern 3/2 on time x ARD RBF on
 * context covariates, Bradley-Terry likelihood) emitting posterior mean strengths plus credible
 * intervals for the engine and weekly form-curve content plots.
 *
 * ACCEPTANCE GATE: Adopt GP-over-Elo for the GSE ratings layer only if: (a) walk-forward log loss on 2021-2026 NFL
 * beats dynamic Elo by >= 0.002; AND (b) the time x context kernel beats the time-only kernel by >=
 * 0.001 log loss; AND (c) fit time per weekly refit < 60 s on the lab VM.
 *
...
export const ARXIV_ID = "1902.07378v1" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `Adopt GP-over-Elo for the GSE ratings layer only if: (a) walk-forward log loss on 2021-2026 NFL beats dynamic Elo by >= 0.002; AND (b) the time x context kernel beats the time-only kernel by >= 0.001 log loss; AND (c) fit time per weekly refit < 60 s on the lab VM.`;
