 ...
 *
 * arXiv:2203.08489v2 · lane:team_ratings · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Build the hierarchical logistic QB/team/coach decomposition: logit P(A beats B) = (theta_QB_A +
 * theta_team_A + ...) - (theta_QB_B + ...), cross-classified random effects for QB, head coach,
 * offensive/defensive units + seasonal form (Stan/numpyro on 2009-2025 nflverse); identification via
 * QB team-changes and backup-QB starts; report variance shares (QB vs roster vs coach) with CIs; add
 * the QB x team interaction the paper assumes away as a 'scheme-fit rating' for free-agency analysis;
 * serve the counterfactual engine P(team X with QB Y beats team Z with QB W).
 *
 * ACCEPTANCE GATE: ADOPT iff: (a) QB/team/coach variance shares are stable across two independent fit windows
 * (2009-2015 vs 2016-2022; same rank order, magnitudes within 50%), AND (b) on the 2023-2024 QB-change
 * test set the decomposed model beats team-only Elo log-loss by >=1%.
 *
...
export const ARXIV_ID = "2203.08489v2" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT iff: (a) QB/team/coach variance shares are stable across two independent fit windows (2009-2015 vs 2016-2022; same rank order, magnitudes within 50%), AND (b) on the 2023-2024 QB-change test set the decomposed model beats team-only Elo log-loss by >=1%.`;
