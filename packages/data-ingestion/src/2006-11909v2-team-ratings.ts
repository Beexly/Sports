 ...
 *
 * arXiv:2006.11909v2 · lane:team_ratings · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): Deploy the permutation two-sample test (Algorithm 2) as a regime-change monitor: split team-vs-team
 * (or QB-vs-defense) matchup outcomes into pre/post windows around candidate change points (coaching
 * changes, QB injuries, trade deadline, season halves), run the test at alpha=0.05, and only re-fit
 * team-strength priors when the test rejects; also use it for model monitoring (test whether the
 * engine pick outcome distribution shifted after a model-version deploy).
 *
...
export const ARXIV_ID = "2006.11909v2" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT as the regime-change gate if Test 1's false-rejection rate is within [2%, 10%] (calibrated) AND Test 3 rejects at p<0.05 (power against a real break); otherwise restrict to multi-season aggregates only.`;
