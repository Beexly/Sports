/**
 * A novel methodological framework for analyzing the momentum effect in tennis singles
 *
 * arXiv:2509.01243v1 · lane:win_spread_total · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE adds CUSUM momentum-flow features to the live game model: a composite drive-flow index (EWM over EPA/drive, success rate, explosive-play rate, turnover/3-and-out penalties) with CUSUM change-point detection within each game.
 *
 * ACCEPTANCE GATE:
 * ADOPT if CUSUM flow features improve the game-model log-loss >=0.002 on the 2022-2024 holdout or the chi-square test shows >=2pp conditional-probability effects.
 *
...
export const ARXIV_ID = "2509.01243v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT if CUSUM flow features improve the game-model log-loss >=0.002 on the 2022-2024 holdout or the chi-square test shows >=2pp conditional-probability effects.`;
