 ...
 *
 * arXiv:2310.01748v2 · lane:markets · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): As engine-honesty infrastructure, build a GSE 'game simulator' in the paper's image: concrete first
 * build = in-play NFL win-probability surface from nflverse play-by-play + NGS tracking - fit a
 * generative drive-outcome model conditioned on field position, time, score, and personnel;
 * forward-simulate 10k rest-of-game paths per live game state from any game state; publish the WP
 * surface with uncertainty bands as an independent cross-check of the production live WP model; run
 * counterfactual experiments ('what if the blitz had come from the other side') for model diagnostics
 * - plus an odds-aware variant: calibrate the simulator's outputs against Pinnacle closing lines
 * (Platt scaling per game-state bucket), testing whether miscalibration residuals predict line moves.
 *
...
export const ARXIV_ID = "2310.01748v2" as const;
export const LANE = "markets" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADAPT confirmed if the nflverse-level rest-of-game simulator's win probabilities achieve Brier score <= the published nflfastR WP model's Brier on the 2024 season (paired by game), AND the simulator's 80% WP intervals cover realized outcomes at 78-82%; if it cannot match the discriminative baseline, the counterfactual machinery still stands for diagnostics use.`;
