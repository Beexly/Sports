 ...
 *
 * arXiv:2606.07433v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Structure GSE's broadcast-video pipeline as watch->remember->reason: watch = the tracking stack (detections, tracks, play grounding), remember = a play-structured game memory keyed on play boundaries with evidence pointers to timestamps, reason = query-driven evidence retrieval returning timestamp/box-grounded answers (e.g., 'every 3rd-and-long blitz in Q3' returns clips + boxes, not text).
 *
 * ACCEPTANCE GATE:
...
export const ARXIV_ID = "2606.07433v1" as const;
...
export const ACCEPTANCE_GATE = `ADAPT the architecture (watch–remember–reason with budgeted evidence search) for GSE's video lane if: the primary read-throughs (DeepSport, FineQuest) replicate their core claims on inspection, and a prototype reasoner on the NFL query set beats uniform-sampling VLM QA by ≥ 15 points of answer accuracy with evidence IoU ≥ 0.5.`;
