 ...
 *
 * arXiv:2603.11016v3 · lane:experimental · verdict:ADAPT · owner:Motif-lab · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE attributes drive EPA to individual players via restricted-support Shapley values (model-based worth function, observed participation coalitions, bootstrap SEs, studentized contributions), starting with rushing plays.
 *
 * ACCEPTANCE GATE:
 * ADAPT only if bootstrap PRSV rankings are stable on NFL data; import only the attribution recipe (restricted support + model-based worth + bootstrap SE + studentization).
 *
...
export const ARXIV_ID = "2603.11016v3" as const;
...
export const ACCEPTANCE_GATE = `ADAPT only if bootstrap PRSV rankings are stable on NFL data; import only the attribution recipe (restricted support + model-based worth + bootstrap SE + studentization).`;
