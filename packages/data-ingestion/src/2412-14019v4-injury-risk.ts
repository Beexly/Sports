/**
 * Retrieving Classes of Causal Orders with Inconsistent Knowledge Bases (MATS)
 *
 * arXiv:2412.14019v4 · lane:causal_injury · verdict:ADAPT · owner:Motif-lab · doctrine:SITUATIONAL
 *
 ...
 *
 * Improvement (record):
 * Use MATS as an LLM-based causal hypothesis ranker for GSE research questions only, with self-consistency scores calibrated to empirical conditional-independence test p-values weighting the feedback-arc-set objective — and every MATS-proposed edge must survive a data-based test (CI test or IV check) before it ever enters the engine.
 *
 * ACCEPTANCE GATE:
 * ADAPT as hypothesis-ranker if: the pipeline recovers ≥4/5 known-direction pairs from GSE data AND the enumerated order classes are stable across two different LLMs (rank correlation of pairwise scores >0.7). REJECT any direct use of LLM orders in the engine.
 *
...
export const ARXIV_ID = "2412.14019v4" as const;
...
export const ACCEPTANCE_GATE = `ADAPT as hypothesis-ranker if: the pipeline recovers ≥4/5 known-direction pairs from GSE data AND the enumerated order classes are stable across two different LLMs (rank correlation of pairwise scores >0.7). REJECT any direct use of LLM orders in the engine.`;
