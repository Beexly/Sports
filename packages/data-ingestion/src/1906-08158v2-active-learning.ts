 ...
 *
 * arXiv:1906.08158v2 · lane:active_learning · verdict:ADAPT · owner:Motif-lab
 *
 ...
 *
 * Improvement (wiring record): Add slate-weighted, cost-aware BatchBALD for the weekly charting batch: greedy joint-entropy
 * acquisition over label configs (MC dropout or 5-model ensemble posterior draws), weighted by
 * expected edge x handle per game and divided by sqrt(charting cost).
 *
 * ACCEPTANCE GATE: ADOPT iff BatchBALD at 20% charting budget achieves 2024 held-out log-loss <= random at 40% budget
 * (the lane's standard efficiency bar), AND beats BALD-top-10 at equal budget by >= 0.005 log-loss, in
 * the 2024 holdout.
 *
...
export const ARXIV_ID = "1906.08158v2" as const;
export const LANE = "active_learning" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT iff BatchBALD at 20% charting budget achieves 2024 held-out log-loss <= random at 40% budget (the lane's standard efficiency bar), AND beats BALD-top-10 at equal budget by >= 0.005 log-loss, in the 2024 holdout.`;
