 ...
 *
 * arXiv:2512.17266 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE prototypes an EventGPT-like autoregressive drive-event model with player/QB conditioning and a residual future-EPA target, enabling counterfactual player-embedding substitution for scheme-fit simulations.
 *
 * ACCEPTANCE GATE:
 * ADAPT if on held-out 2024 drives the player-conditioned autoregressive model beats the game-state EPA regression on next-play yards MAE by >=5% AND the QB-substitution face-validity check passes (elite QB embedding raises predicted drive EPA in >=80% of weak-offense contexts).
 *
...
export const ARXIV_ID = "2512.17266" as const;
...
export const ACCEPTANCE_GATE = `ADAPT if on held-out 2024 drives the player-conditioned autoregressive model beats the game-state EPA regression on next-play yards MAE by >=5% AND the QB-substitution face-validity check passes (elite QB embedding raises predicted drive EPA in >=80% of weak-offense contexts).`;
