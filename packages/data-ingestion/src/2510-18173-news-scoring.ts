/**
 * 1722 Moneyball with LLMs: analyzing tabular summarization in sports narratives
 *
 * arXiv:2510.18173 · lane:nlp · verdict:ADAPT · owner:Hermes · doctrine:SITUATIONAL
 *
 ...
 *
 * Improvement (record):
 * GSE extracts facts from NFL game narratives as atomic (Entity, Attribute, Value, Evidence-span) tuples compiled by a rule-based unit-tested aggregator with conflict flagging, scoring over/under-count separately via the HOI metric.
 *
 * ACCEPTANCE GATE:
 * ADAPT the T3 recipe (tuple extraction + rule-based compiler + HOI) with the two hardening modifications (evidence spans, conflict flagging); the raw T3 is rejected for production without them.
 *
...
export const ARXIV_ID = "2510.18173" as const;
...
export const ACCEPTANCE_GATE = `ADAPT the T3 recipe (tuple extraction + rule-based compiler + HOI) with the two hardening modifications (evidence spans, conflict flagging); the raw T3 is rejected for production without them.`;
