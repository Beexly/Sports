 ...
 *
 * arXiv:2601.06112v1 · lane:data_infra · verdict:ADAPT · owner:Hermes · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * GSE stands up a reliability benchmark harness for its tool-using agents, measuring performance R(k,eps,lam) under real fault modes (429s, quota exhaustion, schema changes) with deterministic state-based verifiers to decide which agents are cleared for unattended production.
 *
 * ACCEPTANCE GATE:
 * ADAPT if the GSE harness reproduces the paper's directional findings, costs <$25 per agent per full grid run, and deterministic verifiers cover >=80% of agent tasks.
 *
...
export const ARXIV_ID = "2601.06112v1" as const;
...
export const ACCEPTANCE_GATE = `ADAPT if the GSE harness reproduces the paper's directional findings, costs <$25 per agent per full grid run, and deterministic verifiers cover >=80% of agent tasks.`;
