 ...
 *
 * arXiv:2409.05714v2 · lane:team_ratings · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Adopt the score-driven dynamic Plackett-Luce model for GSE's pre-season standings forecasts,
 * replacing modal-rank evaluation with expected-rank metrics (E[rank_i], P(rank_i ≤ k) from 10k
 * simulated draws per season) so model selection uses a defensible rank-accuracy metric instead of
 * unreliable modal-rank MAE.
 *
...
export const ARXIV_ID = "2409.05714v2" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `Adopt the score-driven dynamic PL for GSE's pre-season standings model if, on 2015–2025 rolling forecasts, the AIC-final penalized model beats the static model on average log-likelihood by ≥0.05/season AND achieves P(division winner) ≥ market-implied baseline within 0.03.`;
