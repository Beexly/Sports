 ...
 *
 * arXiv:1910.01863 · lane:nlp · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): Adapt the CRF-select -> copy-enabled generation pipeline to NFL box-score/play-by-play -> recap
 * generation: align historical sentences to play events, train an event-selection model (67.1%-F
 * floor), generate per-event sentences conditioned on play features, and select models on
 * factual-error rate (never BLEU).
 *
 * ACCEPTANCE GATE: ADAPT accepted on the public corpus + code + rare journalist product evaluation + honest failure
 * taxonomy + the BLEU-vs-factuality warning; deploy recaps only if drafts post-edit to <=10%
 * minimum-edit WER by GSE editors.
 *
...
export const ARXIV_ID = "1910.01863" as const;
export const LANE = "nlp" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADAPT accepted on the public corpus + code + rare journalist product evaluation + honest failure taxonomy + the BLEU-vs-factuality warning; deploy recaps only if drafts post-edit to <=10% minimum-edit WER by GSE editors.`;
