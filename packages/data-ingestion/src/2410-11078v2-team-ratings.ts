 ...
 *
 * arXiv:2410.11078v2 · lane:team_ratings · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Replace fixed Gaussian label-noise clipping in GSE's spread-margin models with a two-component
 * label-noise mixture (Gaussian plus heavy-tailed component for known noisy regimes like backup-QB
 * games and extreme weather), with the mixture weight a function of game metadata (roster flags,
 * wind), so the outliers are modeled explicitly instead of clipped away.
 *
...
export const ARXIV_ID = "2410.11078v2" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `Adopt if any of: (a) sampled-target model reduces held-out MAE on 2023–2024 by ≥ 0.15 points vs the identical point-target baseline (averaged over 5 seeds), or (b) the MAZ analogue improves ≥ 3% relative while MAE does not worsen; improvement experiment adds per-slice (weather/roster-flag) MAZ improvement ≥ 5%.`;
