 ...
 *
 * arXiv:1908.11490v2 · lane:team_ratings · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): Add NFL player-career GP ability trajectories (log nu(t) ~ GP with multiplicative context effects
 * and per-game opponent defensive strength adjustment) as the season-long player-props baseline,
 * replacing raw trailing averages: emit current-ability nu(now) with credible intervals, rookie
 * breakout timing, and P(nu(t) < replacement) decline flags.
 *
 * ACCEPTANCE GATE: ADAPT into the season-long props pipeline if the GP beats the best SMA baseline by >=5% pooled
 * out-of-sample MSE on 2023-2025 holdout AND 68% intervals achieve 60-76% empirical coverage.
 *
...
export const ARXIV_ID = "1908.11490v2" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADAPT into the season-long props pipeline if the GP beats the best SMA baseline by >=5% pooled out-of-sample MSE on 2023-2025 holdout AND 68% intervals achieve 60-76% empirical coverage.`;
