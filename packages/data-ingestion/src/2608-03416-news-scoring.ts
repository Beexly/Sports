/**
 * 1724 AI World Cup 2026: benchmarking large language models for end-to-end football tournament prediction
 *
 * arXiv:2608.03416 · lane:nlp · verdict:ADOPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Implement the frozen-snapshot plus common-schema plus additive-scoring plus audit tournament-evaluation protocol as the mandatory seasonal harness for comparing GSE model configurations, adding a proper-score (Brier/log-loss) leaderboard and variance-normalized component ranks as required modifications.
 *
 * ACCEPTANCE GATE:
 * ADOPT the protocol (frozen snapshot + common schema + additive scoring + audit) — it is the rare evaluation paper whose method transfers wholesale. Two mandatory modifications: (1) add a proper scoring rule alongside the points leaderboard and report both; (2) normalize component variances before ranking or report component ranks separately. REJECT any GSE leaderboard that displays model self-reported confidence without calibration evidence.
 *
...
export const ARXIV_ID = "2608.03416" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the protocol (frozen snapshot + common schema + additive scoring + audit) — it is the rare evaluation paper whose method transfers wholesale. Two mandatory modifications: (1) add a proper scoring rule alongside the points leaderboard and report both; (2) normalize component variances before ranking or report component ranks separately. REJECT any GSE leaderboard that displays model self-reported confidence without calibration evidence.`;
