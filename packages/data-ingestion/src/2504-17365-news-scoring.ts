 ...
 *
 * arXiv:2504.17365 · lane:nlp · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Build end-to-end NFL commentary generation with an MLLM trained on GSE's recipe, adopting the paper's training-free MoFA-Select compression module, adding a jersey-number detection head for player-identity grounding (fixing the anonymity limitation), and testing MoFA-Select standalone as a motion-budget-driven highlight-clip selector.
 *
 * ACCEPTANCE GATE:
...
export const ARXIV_ID = "2504.17365" as const;
...
export const ACCEPTANCE_GATE = `ACCEPTED (ADAPT): first end-to-end SDVC MLLM + training-free reusable compression module + systematic ablations. ADAPT (not ADOPT): no code release, soccer-only, modest absolute precision — adopt the MoFA-Select module and training recipe, retrain the MLLM on NFL data.`;
