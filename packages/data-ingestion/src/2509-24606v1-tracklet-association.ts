 ...
 *
 * arXiv:2509.24606v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE segments NFL plays into biomechanical/tactical phases (presnap -> dropback -> throw -> catch/RAC; pass-rush phases) with unsupervised optimal-transport segmentation on NGS tracking, with K selected automatically.
 *
 * ACCEPTANCE GATE:
 * Adopt the OT-segmentation layer if, with K selected automatically (no oracle), it matches or beats the supervised BiLSTM baseline on boundary F1 (+-3 frames) on the 500-play validation set AND phase mIoU >=0.55.
 *
...
export const ARXIV_ID = "2509.24606v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt the OT-segmentation layer if, with K selected automatically (no oracle), it matches or beats the supervised BiLSTM baseline on boundary F1 (+-3 frames) on the 500-play validation set AND phase mIoU >=0.55.`;
