 ...
 *
 * arXiv:2101.08175v1 · lane:team_ratings · verdict:ADAPT · owner:Motif-lab
 *
 ...
 *
 * Improvement (wiring record): Two portable pieces: (1) fit GARCH(1,1) errors on player-season (or team-season) random intercepts
 * for a prop-relevant metric (weekly receiving yards for WRs, team EPA/play) on nflverse 2015-2025;
 * test whether volatility clustering exists and whether the conditional variance h_is predicts
 * next-season prop-line beat rates -- i.e., use estimated volatility as a 'consistency' feature in
 * prop models; (2) reuse the additive decomposition for the trajectory model.
 *
...
export const ARXIV_ID = "2101.08175v1" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `Adopt if the GARCH specification wins on LOO by a clear margin (Delta-elpd > 2xse) for at least one prop-relevant metric AND the volatility feature improves a prop hit-rate model (log-loss) by >=0.3% on 2024-2025 holdout. Reject if volatility is unclustered or the feature adds nothing.`;
