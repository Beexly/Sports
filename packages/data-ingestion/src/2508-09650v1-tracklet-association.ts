 ...
 *
 * arXiv:2508.09650v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE ports the cheap framework-independent tracking ideas: visibility-weighted loss (re-weight by occlusion proxy) and occlusion augmentation (mask the ball region in training) for any future vision-based tracking build.
 *
 * ACCEPTANCE GATE:
 * Adopt the WBCE + occlusion-augmentation recipe if a GSE tracking trainer's fully-occluded-frame RMSE improves by >=15% over the unweighted-loss control on a held-out NFL broadcast clip set with synthetic occlusion labels, with no regression on fully-visible frames.
 *
...
export const ARXIV_ID = "2508.09650v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt the WBCE + occlusion-augmentation recipe if a GSE tracking trainer's fully-occluded-frame RMSE improves by >=15% over the unweighted-loss control on a held-out NFL broadcast clip set with synthetic occlusion labels, with no regression on fully-visible frames.`;
