 ...
 *
 * arXiv:2607.18461v1 · lane:nlp · verdict:ADAPT · owner:Hermes · doctrine:SITUATIONAL
 *
 ...
 *
 * Improvement (record):
 * Rebuild fandom radii from 2024-2026 X data to geo-target GSE clips and posts, and add pregame/halftime fan-sentiment-arc features (transformer sentiment on sports text, entity-resolved team mentions) to GSE's viewership and betting-handle models.
 *
 * ACCEPTANCE GATE:
 * Adopt the features if they add >=2pp of out-of-sample R^2 to the viewership baseline on 2024 data; adopt the geography if replicated radii rank-correlate (Spearman >=0.7) with an independent fanbase measure; reject otherwise.
 *
...
export const ARXIV_ID = "2607.18461v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt the features if they add >=2pp of out-of-sample R^2 to the viewership baseline on 2024 data; adopt the geography if replicated radii rank-correlate (Spearman >=0.7) with an independent fanbase measure; reject otherwise.`;
