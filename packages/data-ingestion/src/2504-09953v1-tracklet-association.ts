 ...
 *
 * arXiv:2504.09953v1 · lane:tracking · verdict:ADAPT · owner:Motif-lab · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Build full 3D pose uplifting (including joint rotations) for the video lane, removing the biggest deployment blocker by estimating per-player body shape from a few frames via the A2B anthropometric approach and fine-tuning the uplifter jointly with shape estimation instead of requiring GT body shapes.
 *
 * ACCEPTANCE GATE:
 * PURSUE full replication if the plausibility test passes with zero twist failures and end-to-end runtime ≤ 50 ms/frame; REJECT for broadcast use if twist failures persist despite the VPoser prior or the detector pushes the pipeline past 100 ms/frame.
 *
...
export const ARXIV_ID = "2504.09953v1" as const;
...
export const ACCEPTANCE_GATE = `PURSUE full replication if the plausibility test passes with zero twist failures and end-to-end runtime ≤ 50 ms/frame; REJECT for broadcast use if twist failures persist despite the VPoser prior or the detector pushes the pipeline past 100 ms/frame.`;
