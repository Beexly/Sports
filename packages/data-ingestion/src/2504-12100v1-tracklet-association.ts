 ...
 *
 * arXiv:2504.12100v1 · lane:tracking · verdict:ADAPT · owner:Motif-lab · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Adopt generative open-vocabulary relation decoding with a large rounding vocabulary as GSE's paradigm for semantically ambiguous sports action/event vocabularies (quarterback-releases-ball type relations instead of fixed-class discrimination), evaluated with the T2I-retrieval/SPICE proxy metrics that solve the models-richer-than-annotations evaluation problem.
 *
 * ACCEPTANCE GATE:
 * Ledger states no numeric gate: this is a stills-only research prototype (50 DDIM steps, no sports data, no latency numbers) — the adaptation is conceptual and evaluative, not a pipeline to deploy.
 *
...
export const ARXIV_ID = "2504.12100v1" as const;
...
export const ACCEPTANCE_GATE = `Ledger states no numeric gate: this is a stills-only research prototype (50 DDIM steps, no sports data, no latency numbers) — the adaptation is conceptual and evaluative, not a pipeline to deploy.`;
