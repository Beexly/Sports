/**
 * SportMamba Multi-Object Tracking
 *
 * arXiv:2506.03335v1 · lane:tracking · verdict:ADAPT · owner:Motif-lab · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Adopt SportMamba's association design with jersey-number-conditioned association: replace the generic Re-ID embedding with a two-stream identity (a) a jersey-number OCR head producing a number posterior per detection and (b) team-side classification via jersey color clustering per game — association cost becomes a weighted sum of HA-EIoU, number-posterior agreement, and team agreement, attacking the paper's stated failure mode of near-identical uniforms.
 *
 * ACCEPTANCE GATE:
 * ADOPT the association design for GSE's video-tracking lane only if: on the football annotation set, the SportMamba-style pipeline beats ByteTrack by ≥ 3.0 HOTA points with the same detector, and the gain concentrates on occlusion-heavy plays.
 *
...
export const ARXIV_ID = "2506.03335v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the association design for GSE's video-tracking lane only if: on the football annotation set, the SportMamba-style pipeline beats ByteTrack by ≥ 3.0 HOTA points with the same detector, and the gain concentrates on occlusion-heavy plays.`;
