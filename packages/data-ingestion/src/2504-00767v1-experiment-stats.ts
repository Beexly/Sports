 ...
 *
 * arXiv:2504.00767v1 · lane:experimental · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Auto-generate wordalised explanations of the engine's daily output for content drafts (human/voice-gate approval before posting), with deterministic fidelity checking (parse claimed factor directions from text and compare against true contribution signs, no LLM judge) and SHAP-based wordalisation of the actual non-additive model instead of requiring a linear one.
 *
 * ACCEPTANCE GATE:
 * Adopt as a draft-generation step if: (a) sentence-level fidelity ≥ 90% on the 50-pick test, and (b) Garrett's blinded engagement rating favors wordalised by ≥ 0.5 points, and (c) contribution extraction runs automatically on the daily engine output. Reject (keep as reference) if fidelity < 85%.
 *
...
export const ARXIV_ID = "2504.00767v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt as a draft-generation step if: (a) sentence-level fidelity ≥ 90% on the 50-pick test, and (b) Garrett's blinded engagement rating favors wordalised by ≥ 0.5 points, and (c) contribution extraction runs automatically on the daily engine output. Reject (keep as reference) if fidelity < 85%.`;
