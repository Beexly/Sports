/**
 * Load Management and the Healthy-Worker Survivor Effect in the NBA
 *
 * arXiv:2603.26935v1 · lane:causal_injury · verdict:ADAPT · owner:Mimo · doctrine:SITUATIONAL
 *
 ...
 *
 * Improvement (record):
 * Replicate MS-PEM (participation propensity via gradient boosting, stabilized truncated IPW, piecewise-exponential injury hazard with spline weighted cumulative load lags) on nflverse player-week data as GSE's causal check: flag any injury load feature whose naive coefficient flips sign under IPW as survivor-contaminated.
 *
 * ACCEPTANCE GATE:
 * ADOPT the MS-PEM design as GSE's causal check iff the replication reproduces the attenuation pattern (IPW moves naive lag weights ≥ 40% toward zero) on 2020–2024 data with player-grouped CV.
 *
...
export const ARXIV_ID = "2603.26935v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the MS-PEM design as GSE's causal check iff the replication reproduces the attenuation pattern (IPW moves naive lag weights ≥ 40% toward zero) on 2020–2024 data with player-grouped CV.`;
