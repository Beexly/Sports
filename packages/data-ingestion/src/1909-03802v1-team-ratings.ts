/**
 * Bayesian isotonic logistic regression via constrained splines
 *
 * arXiv:1909.03802v1 · lane:team_ratings · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Replace binned recalibration with a BILR monotone calibration map (logit P(win) = sum beta_m
 * b_m(model_prob), fully monotone, B-spline order 4, ~9 bases on [0,1]) fit in Stan, emitting smooth
 * calibration maps with 95% bands applied to all engine probabilities before publishing, plus monotone
 * 4th-down conversion curves by yards-to-go and field zone.
 *
 * ACCEPTANCE GATE: ADAPT the calibration module into the engine pipeline if BILR-calibrated probabilities achieve lower
 * or equal out-of-sample log-loss vs. current calibration AND zero monotonicity violations by
 * construction; REJECT if it underperforms binned isotonic on log-loss by >1%.
 *
...
export const ARXIV_ID = "1909.03802v1" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADAPT the calibration module into the engine pipeline if BILR-calibrated probabilities achieve lower or equal out-of-sample log-loss vs. current calibration AND zero monotonicity violations by construction; REJECT if it underperforms binned isotonic on log-loss by >1%.`;
