 ...
 *
 * arXiv:2204.11142v1 · lane:tracking · verdict:ADAPT · owner:Motif-lab
 *
 ...
 *
 * Improvement (wiring record): Take only the graph-representation idea (discard the DQN/RL scaffolding): per-play player graphs
 * from NGS tracking frames — 22 player nodes + ball node, node features = (x, y, speed, accel,
 * orientation, team, down/distance context), distance-weighted or learned adjacency edges — GCN/GAT
 * encoder -> play/drive outcome head (EPA, success prob) trained supervised on nflverse 2020-2025
 * joined to tracking; then upgrade to spatio-temporal GCN (snap-1s to +2s frames) and test whether
 * learned edges recover football structure (OL-DL, WR-CB matchups) as interpretable pick-model
 * features.
 *
 * ACCEPTANCE GATE: ADAPT iff on the 2025 tracking holdout the supervised player-graph GCN beats the hand-feature GBM
 * baseline by >=5% EPA-RMSE AND the attention/GAT edge weights correlate sensibly with football
 * structure (nearest defenders/opponents weighted); REJECT the GNN-tracking direction if it fails.
 *
...
export const ARXIV_ID = "2204.11142v1" as const;
export const LANE = "tracking" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADAPT iff on the 2025 tracking holdout the supervised player-graph GCN beats the hand-feature GBM baseline by >=5% EPA-RMSE AND the attention/GAT edge weights correlate sensibly with football structure (nearest defenders/opponents weighted); REJECT the GNN-tracking direction if it fails.`;
