 ...
 *
 * arXiv:2507.08108v1 · lane:team_ratings · verdict:ADAPT · owner:Mimo · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE aggregates weekly NFL power rankings from ~15-20 public and market-implied sources with an L_alpha-Mallows model, learning the league's upset-profile parameter alpha as a first-class feature.
 *
 * ACCEPTANCE GATE:
 * Adopt L_alpha-Mallows if on the 2025 held-out season it beats both baselines by >=2pp on pairwise accuracy AND >=5pp on top-5 hit rate in at least 3 of 4 seasons rolling; REJECT if alpha estimates are unstable.
 *
...
export const ARXIV_ID = "2507.08108v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt L_alpha-Mallows if on the 2025 held-out season it beats both baselines by >=2pp on pairwise accuracy AND >=5pp on top-5 hit rate in at least 3 of 4 seasons rolling; REJECT if alpha estimates are unstable.`;
