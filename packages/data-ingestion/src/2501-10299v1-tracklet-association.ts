 ...
 *
 * arXiv:2501.10299v1 · lane:tracking · verdict:ADAPT · owner:Motif-lab · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * Build GSE's team-style similarity engine on sliced-Wasserstein embeddings, but quantize the league-wide frame pool once into a shared codebook of formation atoms ("Cover-2 shell", "bunch trips") and represent each team as a histogram over shared atoms — team similarity becomes a cheap Hellinger distance and the atoms give a human-readable formation taxonomy for free.
 *
 * ACCEPTANCE GATE:
...
export const ARXIV_ID = "2501.10299v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the sliced-Wasserstein embedding + quantization pipeline as GSE's team-style similarity engine if the NGS pilot reproduces two paper signatures: (i) embedding beats raw-coordinate baselines on a formation-label prediction task by ≥10 pp, and (ii) the resulting 32×32 similarity matrix correlates ≥0.5 with an independent style proxy (e.g., charted man/zone rates).`;
