 ...
 *
 * arXiv:2602.16830v1 · lane:causal_injury · verdict:ADAPT · owner:Motif-lab · doctrine:SITUATIONAL
 *
 ...
 *
 * Improvement (record):
 * GSE ports three-stage double machine learning to causal NFL game-plan questions (personnel groupings, coverage shells, 4th-down go decisions) with XGBoost nuisance models, cross-fitting by season, and mandatory placebo refutation checks.
 *
 * ACCEPTANCE GATE:
 * ADAPT the DML harness if the placebo test passes (|beta_hat_placebo| < 0.2*|beta_hat_real|) AND estimates are stable across two nuisance learners with sign agreement on all significant effects.
 *
...
export const ARXIV_ID = "2602.16830v1" as const;
...
export const ACCEPTANCE_GATE = `ADAPT the DML harness if the placebo test passes (|beta_hat_placebo| < 0.2*|beta_hat_real|) AND estimates are stable across two nuisance learners with sign agreement on all significant effects.`;
