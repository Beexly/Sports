 ...
 *
 * arXiv:2202.00583v1 · lane:team_ratings · verdict:ADAPT · owner:Motif-lab
 *
 ...
 *
 * Improvement (wiring record): Build latent style allocation on NGS tracking spatial summaries (start: WR route
 * endpoints/catch-point locations): K,M in 2..8 grid via ELPD, covariates =
 * down/distance/formation/man-zone, player offsets eta (receiver) and delta (defender) — descriptive
 * validity via ELPD vs finite-mixture baselines on 2024, predictive value via receiver
 * style-membership improving completion-probability log-loss; then make styles dynamic (HMM layer over
 * styles within a game) and defender-specific to capture in-game tendency shifts.
 *
 * ACCEPTANCE GATE: ADOPT tracking archetypes as features iff BOTH: (a) ELPD gain over finite mixture replicates (>=2%)
 * on 2024 NGS data, AND (b) style features improve completion-probability log-loss by >=1% on 2024;
 * reject as engine features if (b) fails (keep as content/visualization only).
 *
...
export const ARXIV_ID = "2202.00583v1" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT tracking archetypes as features iff BOTH: (a) ELPD gain over finite mixture replicates (>=2%) on 2024 NGS data, AND (b) style features improve completion-probability log-loss by >=1% on 2024; reject as engine features if (b) fails (keep as content/visualization only).`;
