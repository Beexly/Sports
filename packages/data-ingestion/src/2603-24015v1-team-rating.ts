 ...
 *
 * arXiv:2603.24015v1 · lane:team_ratings · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE wires areal-Poisson offensive tendency maps (team x field-zone relative rates from a hierarchical model over nflverse play-by-play) into matchup features, replacing raw empirical tendency tables.
 *
 * ACCEPTANCE GATE:
 * ADOPT if the full hierarchical model beats the fixed-effects baseline by Delta-elpd >= 500 nats on the holdout AND team x zone relative-rate estimates are stable across the two test windows (Spearman >=0.7).
 *
...
export const ARXIV_ID = "2603.24015v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT if the full hierarchical model beats the fixed-effects baseline by Delta-elpd >= 500 nats on the holdout AND team x zone relative-rate estimates are stable across the two test windows (Spearman >=0.7).`;
