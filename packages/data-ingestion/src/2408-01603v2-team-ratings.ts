 ...
 *
 * arXiv:2408.01603v2 · lane:team_ratings · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Audit GSE's team-rating weights by testing equal weights against current weights on season-blocked
 * log-loss (drop/dampen any weight that loses), adopt the ordinal margin-of-victory Elo likelihood if
 * margin earns its keep, and extend to a score-driven dynamic-ordinal model (AR(1) strength dynamics
 * fused with the ordinal MOV likelihood) so form swings and margin information live in one rating.
 *
 * ACCEPTANCE GATE: Adopt the audit's recommendations if, on season-blocked CV, equal weights beat GSE's current weights
 * on log-loss (then drop/dampen the weights) AND the ordinal-MOV model beats binary Elo on
 * win-probability log-loss by ≥0.005.
 *
...
export const ARXIV_ID = "2408.01603v2" as const;
export const LANE = "team_ratings" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `Adopt the audit's recommendations if, on season-blocked CV, equal weights beat GSE's current weights on log-loss (then drop/dampen the weights) AND the ordinal-MOV model beats binary Elo on win-probability log-loss by ≥0.005.`;
