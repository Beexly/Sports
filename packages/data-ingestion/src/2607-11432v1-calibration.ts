 ...
 *
 * arXiv:2607.11432v1 · lane:calibration · verdict:ADAPT · owner:Mimo · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Add a multi-objective pick-selection layer that fits an MOBT softmax over GSE's posted-vs-skipped and abstained pick pairs to recover Garrett's implicit weights over edge, cover probability, and variance, replacing the scalar pick-score ranking with a Pareto-frontier filter.
 *
 * ACCEPTANCE GATE:
...
export const ARXIV_ID = "2607.11432v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the multi-objective selection layer if on 2025 H1 the MOBT model achieves >=0.05 nats/comparison better test log-likelihood than the scalar BT baseline AND the Pareto-frontier filter on 2025 H2 yields posted-pick CLV >= the scalar-ranking baseline's CLV with no worse cover rate. REJECT if the incomparability labels are uninformative (MOBT ~= BT log-likelihood) or the comparison dataset has <500 usable pairs.`;
