 ...
 *
 * arXiv:2604.01491v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Build ridge-regularized multinomial Bradley-Terry severity ratings for blockers/rushers from BDB tracking engagement windows (2.5 s win rule) and feed weekly opponent-adjusted rusher/blocker ratings as matchup features into GSE's dropback-EPA and sack-probability submodels.
 *
 * ACCEPTANCE GATE:
 * ADAPT (proceed to pilot) only if on the Big Data Bowl replication the severity BT beats the smoothed-frequency matchup baseline by ≥ 0.5% relative log-loss AND rusher ratings correlate ≥ 0.50 across seasons.
 *
...
export const ARXIV_ID = "2604.01491v1" as const;
...
export const ACCEPTANCE_GATE = `ADAPT (proceed to pilot) only if on the Big Data Bowl replication the severity BT beats the smoothed-frequency matchup baseline by ≥ 0.5% relative log-loss AND rusher ratings correlate ≥ 0.50 across seasons.`;
