 ...
 *
 * arXiv:1111.0693v1 · lane:tracking · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): Apply the paper's CLT variance-tilt rule as an overlay on GSE's underdog game-management
 * recommendations: when GSE win probability < 35% (underdog), upweight higher-variance actions among
 * near-optimal choices; when > 65% (favorite), downweight them -- the tilt computed over empirical
 * play-outcome distributions conditioned on (down, distance, field position, score differential, time
 * remaining).
 *
...
export const ARXIV_ID = "1111.0693v1" as const;
export const LANE = "tracking" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT the CLT variance tilt for underdog game-management content IF the backtest shows >= 0.5 percentage points of additional win probability per game attributable to tilted recommendations in underdog spots across 2024-2025, with no degradation in favorite spots.`;
