 ...
 *
 * arXiv:2604.09143v1 · lane:team_ratings · verdict:ADOPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * Replace plain Elo in the GSE stack with score-driven Skellam-margin rating (eq. 6-7): run ratings on neutral-site skill with HFA as a separate offset, K and α grid-searched on 1999-2020, and later scale K by Fisher-information normalization (scaled score) so blowouts with high expected variance get smaller effective updates.
 *
 * ACCEPTANCE GATE:
 * ADOPT (score-driven Skellam-margin rating replaces plain Elo in the GSE stack) if mean out-of-sample log-loss on 2021–2025 is ≥ 0.005 lower than standard Elo AND the rating series passes a stability check (no single-team single-week rating swing > 3x the median weekly swing).
 *
...
export const ARXIV_ID = "2604.09143v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT (score-driven Skellam-margin rating replaces plain Elo in the GSE stack) if mean out-of-sample log-loss on 2021–2025 is ≥ 0.005 lower than standard Elo AND the rating series passes a stability check (no single-team single-week rating swing > 3x the median weekly swing).`;
