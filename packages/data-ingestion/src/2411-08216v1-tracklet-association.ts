 ...
 *
 * arXiv:2411.08216v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * Adopt GTA into the NGS-replacement tracking pipeline with a jersey-number-aware ReID connector (auxiliary jersey-number classification head, multi-task identity embedding + number OCR) so the distance matrix separates same-team players by number rather than kit color, using a field-coordinate spatial gate instead of the paper's fixed-camera assumption.
 *
 * ACCEPTANCE GATE:
 * Adopt GTA into the NGS-replacement tracking pipeline if it improves HOTA by ≥3 points and reduces ID switches by ≥15% on the All-22 validation set with the paper's default hyperparameters.
 *
...
export const ARXIV_ID = "2411.08216v1" as const;
...
export const ACCEPTANCE_GATE = `Adopt GTA into the NGS-replacement tracking pipeline if it improves HOTA by ≥3 points and reduces ID switches by ≥15% on the All-22 validation set with the paper's default hyperparameters.`;
