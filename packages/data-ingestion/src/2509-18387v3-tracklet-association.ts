 ...
 *
 * arXiv:2509.18387v3 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * GSE adopts the blur-label schema (midpoint + blur vector) for ball detection on broadcast video, improving position/velocity estimates in the first frames after release where motion blur is worst.
 *
 * ACCEPTANCE GATE:
 * Adopt the blur-label schema if the blur-aware detector beats the center-point baseline by >=2 F1 points on the NFL test set AND position+blur trajectory fitting reduces catch-point MAE vs NGS ground truth by >=25% relative to position-only.
 *
...
export const ARXIV_ID = "2509.18387v3" as const;
...
export const ACCEPTANCE_GATE = `Adopt the blur-label schema if the blur-aware detector beats the center-point baseline by >=2 F1 points on the NFL test set AND position+blur trajectory fitting reduces catch-point MAE vs NGS ground truth by >=25% relative to position-only.`;
