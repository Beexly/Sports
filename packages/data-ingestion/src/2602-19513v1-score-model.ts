 ...
 *
 * arXiv:2602.19513v1 · lane:win_spread_total · verdict:ADAPT · owner:Hermes · doctrine:PROPRIETARY_EDGE
 *
 ...
 *
 * Improvement (record):
 * GSE adopts a closed-form Brownian live win-probability model estimated from drive-level cumulative EPA/success-rate stats with a variance-stabilized T-score target, plus an IoF/X-index latent player metric measuring persistent traits.
 *
 * ACCEPTANCE GATE:
 * ADOPT the Brownian closed-form WP if on 2023-2024 its Brier score is <= the nfl4th-grid Brier (within 0.0005) with max decile deviation <0.03; ADOPT the X-index player metric if season X-index has test-retest stability >=0.5 across season halves.
 *
...
export const ARXIV_ID = "2602.19513v1" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the Brownian closed-form WP if on 2023-2024 its Brier score is <= the nfl4th-grid Brier (within 0.0005) with max decile deviation <0.03; ADOPT the X-index player metric if season X-index has test-retest stability >=0.5 across season halves.`;
