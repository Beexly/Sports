 ...
 *
 * arXiv:2508.15956v1 · lane:markets · verdict:ADAPT · owner:Mimo · doctrine:BASELINE
 *
 ...
 *
 * Improvement (record):
 * GSE forecasts bookmaker odds with a dynamic graph model: offensive/defensive ratings fit by weighted least squares on historical log-transformed spreads with geometric time decay, forecasting opening/closing spread+total for any matchup and flagging news-driven anomalies.
 *
 * ACCEPTANCE GATE:
 * ADAPT if on 2024 holdout: (a) closing-spread MAE <=3.0 points AND beats carry-forward by >=0.75 points, or (b) the anomaly detector flags >=10 verifiable news-driven line moves with precision >=50%; REJECT if neither holds.
 *
...
export const ARXIV_ID = "2508.15956v1" as const;
...
export const ACCEPTANCE_GATE = `ADAPT if on 2024 holdout: (a) closing-spread MAE <=3.0 points AND beats carry-forward by >=0.75 points, or (b) the anomaly detector flags >=10 verifiable news-driven line moves with precision >=50%; REJECT if neither holds.`;
