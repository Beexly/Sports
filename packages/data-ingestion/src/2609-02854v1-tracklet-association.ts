 ...
 *
 * arXiv:2609.02854v1 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * Port MuyBridge's analytic fusion to NFL offline: center-of-mass estimation from broadcast video with sparse anchor fusion, as a template for fusing depth and anchor signals for player kinematics where NGS tracking is unavailable.
 *
 * ACCEPTANCE GATE:
 * ADAPT (adopt the NFL port) if: analytic fusion achieves >= 30% relative reduction in range AbsRel vs monocular-depth-only AND vertical CoM error < 60 mm on held-out games; reject otherwise (the anchors don't buy enough over raw depth at NFL broadcast ranges).
 *
...
export const ARXIV_ID = "2609.02854v1" as const;
...
export const ACCEPTANCE_GATE = `ADAPT (adopt the NFL port) if: analytic fusion achieves >= 30% relative reduction in range AbsRel vs monocular-depth-only AND vertical CoM error < 60 mm on held-out games; reject otherwise (the anchors don't buy enough over raw depth at NFL broadcast ranges).`;
