 ...
 *
 * arXiv:2502.02785v2 · lane:tracking · verdict:ADAPT · owner:Hermes · doctrine:INFRA
 *
 ...
 *
 * Improvement (record):
 * Adopt the UIED/SAR schema pattern for GSE's data layer (nflverse + NGS into a single queryable play/frame schema), and replace sparse terminal EPV rewards with dense per-frame reward shaping (per-frame ΔEPA) for any GSE off-ball multi-agent valuation work, addressing paper 0372's off-ball credit-assignment gap.
 *
 * ACCEPTANCE GATE:
 * ADOPT the UIED/SAR schema pattern for GSE's data layer if a 2-week prototype converts nflverse + NGS into a single queryable play/frame schema with <1% row loss on joins; REJECT porting any OpenSTARLab model to NFL data.
 *
...
export const ARXIV_ID = "2502.02785v2" as const;
...
export const ACCEPTANCE_GATE = `ADOPT the UIED/SAR schema pattern for GSE's data layer if a 2-week prototype converts nflverse + NGS into a single queryable play/frame schema with <1% row loss on joins; REJECT porting any OpenSTARLab model to NFL data.`;
