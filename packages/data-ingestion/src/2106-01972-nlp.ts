 ...
 *
 * arXiv:2106.01972 · lane:nlp · verdict:ADAPT · owner:Hermes
 *
 ...
 *
 * Improvement (wiring record): Adapt the sparse-state-tracking protocol to NFL text streams: build a corpus of timestamped NFL
 * beat-writer/X commentary aligned to official play-by-play events (injuries, turnovers, lineup
 * changes); train a classifier over rolling windows of text to flag state-changing items, evaluated
 * with positive recall at fixed precision under heavy class imbalance; apply the paper's
 * sparsity-ablation protocol to measure how non-event chatter degrades detection; use the 10-bit
 * joint-output trick (or multi-label head) to model correlated events (injury + substitution
 * co-occurring) — then replace frozen-BERT+GRU with a modern long-context encoder and extend the state
 * ontology to NFL-relevant events (injury, ejection, weather delay, coach challenge).
 *
...
export const ARXIV_ID = "2106.01972" as const;
export const LANE = "nlp" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `For GSE: positive recall >= 0.5 at precision >= 0.6 on a held-out month of NFL news->event labels passes the gate.`;
