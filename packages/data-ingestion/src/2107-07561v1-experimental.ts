 ...
 *
 * arXiv:2107.07561v1 · lane:experimental · verdict:ADAPT · owner:Mimo
 *
 ...
 *
 * Improvement (wiring record): Adapt MultCOMP as a fast two-step estimator (COM-Poisson margins + delta/omega moment-match, no
 * MCMC) for joint low-count NFL outcomes: per-game paired counts of (home explosive plays, away
 * explosive plays) where negative in-game dependence exists (possessions are a zero-sum clock
 * resource); extend the paper with a covariate-dependent delta(z) = clip(delta_0 + beta^T z) inside
 * the per-pair validity interval, letting dependence vary with game-state proxies (pregame spread,
 * total, pace) — stronger in slow, low-total games, weaker in shootouts — fit by profile held-out
 * likelihood on 2024.
 *
...
export const ARXIV_ID = "2107.07561v1" as const;
export const LANE = "experimental" as const;

/** Numeric acceptance gate, verbatim from the wiring record (evaluated offline on backtest data). */
export const ACCEPTANCE_GATE = `ADOPT the adapted estimator if, on the 2024 holdout window, MultCOMP beats independent Poisson by >= 0.01 nats/observation in mean joint log-likelihood AND the derived P(home out-gains away) calibration slope in decile bins is within [0.9, 1.1].`;
