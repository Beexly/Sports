 ...
 * PRODUCTION_RUNTIME==="true" (declared by the Docker worker images). The
 * Docker workers run NODE_ENV=production with no VERCEL_ENV — the
 * Vercel-only gate would leave refresh/settlement writes silently droppable
 * there. Deliberately NOT NODE_ENV: NODE_ENV is "production" during every
...
describe("stub client fail-closed in the production runtime (O-1.7)", () => {
  ...
  it("THROWS in a non-Vercel production runtime declared via PRODUCTION_RUNTIME (worker images)", async () => {
    // The Docker workers run NODE_ENV=production with no VERCEL_ENV — the
    // Vercel-only gate left refresh/settlement writes silently droppable
    // there. The worker images declare PRODUCTION_RUNTIME=true and must
    // trip the same guard.
