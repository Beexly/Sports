...
main()
  ...
  // ────────────────────────────────────────────────────────────
  // Phase 8 — internal-only content drafts. None of these ever
  // auto-publish. Each is seeded with the source coverage and
  // compliance evidence the cockpit needs to render a readiness
  // verdict. publishedAt is NEVER set (and never may be set).
  // ────────────────────────────────────────────────────────────
  ...
  if (dbAny.contentDraft) {
    try {
      const existing = await dbAny.contentDraft.count();
      if (existing === 0) {
        await dbAny.contentDraft.create({
          data: {
            title: "Tonight's NFL slate brief (DRAFT — internal review)",
            slug: "internal-nfl-slate-brief-seed",
            contentType: "DAILY_BRIEF",
            status: "NEEDS_REVIEW",
            visibility: "INTERNAL",
            sport: "NFL",
            ...
            bannedPhraseScanClean: true,
            draftBody:
              "Internal slate brief draft. Sourced exclusively from approved picks and platform-published data. Awaiting operator review. NOT PUBLISHED.",
            generatedBy: "seed",
            // publishedAt: null — INTENTIONALLY OMITTED, engine never sets this.
            sources: {
              create: [
                {
                  sourceType: "ODDS",
                  sourceLabel: "The Odds API daily snapshot",
                  sourceStatus: "FRESH",
                  trustLevel: "PLATFORM",
                  notes: "Internal sample seed only.",
                },
                {
                  sourceType: "PICK",
                  sourceLabel: "internal canonical pick set",
                  sourceStatus: "FRESH",
                  trustLevel: "PLATFORM",
                  notes: "Internal sample seed only.",
                },
        ...
        await dbAny.contentDraft.create({
          data: {
            title: "Model accountability note (INTERNAL ONLY)",
            slug: "internal-model-accountability-note-seed",
            contentType: "MODEL_ACCOUNTABILITY_NOTE",
            status: "DRAFT",
            visibility: "INTERNAL",
            relatedPickIds: [],
            ...
            responsibleGamingIncluded: false,
            affiliateDisclosureIncluded: false,
            performanceGateStatus: "GATE_OFF_REQUIRED",
            bannedPhraseScanClean: true,
            draftBody:
              "Internal calibration evidence summary. No public claim, no performance assertion. INTERNAL_ONLY.",
            generatedBy: "seed",
            sources: {
              create: [
                {
                  sourceType: "CALIBRATION",
                  sourceLabel: "internal calibration roll-up",
                  sourceStatus: "FRESH",
                  trustLevel: "PLATFORM",
                  notes: "Auto-generated from /cockpit/calibration.",
                },
              ],
            },
          },
        } as unknown as never);
        console.log("  ✓ ContentDraft seeded (2 internal drafts)");
      } else {
        console.log(`  - ContentDraft already has ${existing} rows — skipping.`);
      }
    } catch (err) {
      console.log(
        `  ! ContentDraft seed skipped: ${err instanceof Error ? err.message : "unknown"}`
      );
    }
  } else {
    console.log("  - ContentDraft model not generated yet — run prisma generate first to seed drafts.");
  }

  // ────────────────────────────────────────────────────────────
  // CalibrationProposal — informational only. The model-freeze
  // guardrail looks for status: "IMPLEMENTED" + matching
  // modelVersion. The seed below is an OPEN proposal (not
  // IMPLEMENTED) so it does NOT satisfy the freeze guard on its
  // own — that is what `docs/calibration-proposals/FROZEN.md`
  // is for. This row is here so the cockpit has a non-empty
  // proposals list to render during local development.
  // ────────────────────────────────────────────────────────────
  const dbAny2 = db as unknown as {
    calibrationProposal?: {
      create: (args: unknown) => Promise<{ id: string }>;
      count: () => Promise<number>;
    };
  };
  if (dbAny2.calibrationProposal) {
    try {
      const existing = await dbAny2.calibrationProposal.count();
      if (existing === 0) {
        await dbAny2.calibrationProposal.create({
          data: {
            modelVersion: "v5.0.0",
            kind: "CONFIDENCE_BUCKET_DRIFT",
            observation:
              "Internal placeholder. Real proposals are emitted by computeCalibration() once per-bucket settled-pick sample sizes pass 30.",
            proposedChange:
              "No change. Continue collecting outcome evidence under the FROZEN baseline.",
            evidence: { sampleSize: 0, source: "seed" },
            status: "OPEN",
          },
        } as unknown as never);
        console.log("  ✓ CalibrationProposal seeded (OPEN placeholder)");
      } else {
        console.log(`  - CalibrationProposal already has ${existing} rows — skipping.`);
      }
    } catch (err) {
      console.log(
        `  ! CalibrationProposal seed skipped: ${err instanceof Error ? err.message : "unknown"}`
      );
    }
  }
}
...
main()
  ...
  .finally(async () => {
    await db.$disconnect();
            ...
            "Internal placeholder promotion record (truncated). Do not surface publicly. INTERNAL_ONLY.",
        offerCategory: "OTHER",
        affiliateType: "NONE",
        status: "BLOCKED",
        complianceStatus: "BLOCKED",
...
// ─────────────────────────────────────────────
// Phase 5 — Daily brief seed (one INTERNAL draft)
// ─────────────────────────────────────────────
async function seedDailyBrief(): Promise<void> {
  const dbAny = db as unknown as {
    dailyBrief?: { count: () => Promise<number>; create: (args: unknown) => Promise<unknown> };
  };
  if (!dbAny.dailyBrief) {
    console.log("  - DailyBrief model not generated yet — skipping daily-brief seed.");
    return;
  }
  try {
    const existing = await dbAny.dailyBrief.count();
    if (existing > 0) {
      console.log(`  - DailyBrief already has ${existing} rows — skipping.`);
      return;
    }
    await dbAny.dailyBrief.create({
      data: {
        briefDate: new Date(new Date().toISOString().slice(0, 10) + "T00:00:00Z"),
        status: "DRAFT",
        visibility: "INTERNAL",
        title: "Internal slate brief — local seed",
        summary: "Internal-only daily brief seeded for local cockpit walkthroughs. Never published.",
        slateSummary: "0 games seeded. Replace with live data via the ingestion pipeline.",
        responsibleGamingText:
          "If you or someone you know has a gambling problem, call 1-800-522-4700.",
        generatedBy: "seed",
        // publishedAt: null — INTENTIONALLY OMITTED.
      },
    } as unknown as never);
    console.log("  ✓ DailyBrief seeded (INTERNAL draft)");
  } catch (err) {
    console.log(`  ! DailyBrief seed skipped: ${err instanceof Error ? err.message : "unknown"}`);
  }
}

// ─────────────────────────────────────────────
// Phase 8 — Draft-only content engine seed.
// publishedAt is NEVER set. visibility defaults to INTERNAL on the
// ones we create here. The model-freeze guardrail does not consult
// this file (it consults FROZEN.md and CalibrationProposal rows).
// ─────────────────────────────────────────────
async function seedContentDrafts(): Promise<void> {
  const dbAny = db as unknown as {
    contentDraft?: {
  ...
  if (dbAny.contentDraft) {
    ...
    console.log("  - ContentDraft model not generated yet — run prisma generate to seed drafts.");
  }
  if (dbAny.calibrationProposal) {
    try {
      const existing = await dbAny.calibrationProposal.count();
      if (existing === 0) {
        await dbAny.calibrationProposal.create({
          data: {
            modelVersion: "v5.0.0",
            kind: "CONFIDENCE_BUCKET_DRIFT",
            observation:
