...
export function middleware(req: NextRequest): NextResponse {
  ...
  if (requiresAuth) {
    // Dev-mode bypass: when DEV_FAKE_ADMIN=true (non-production only), the
    // auth() helper returns a synthetic admin session, so we must NOT redirect
    // here. In production, auth() will throw if DEV_FAKE_ADMIN is set, so the
    // bypass is never safe to activate there.
    if (
