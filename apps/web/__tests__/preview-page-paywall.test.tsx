...
// Open the record-integrity layer (real readiness gates read these env vars
// fresh on every call) so the PAYWALL layer is the only gate under test here.
// The record-integrity gating itself is pinned by preview-projectable-pick.test.ts.
beforeEach(() => {
  vi.stubEnv("PUBLIC_PICKS_ENABLED", "true");
  vi.stubEnv("FORCE_NO_BET_IF_STALE", "false");
});
afterEach(() => {
  vi.unstubAllEnvs();
});
...
// confidence 91 and movement -4.5 collide with no other fixture value.
// (The movement delta must be a canonical half-point NFL value — the page
// renders market numbers only after `normalizeMarketPoint` validation.)
const CONFIDENCE = 91;
const MOVEMENT = -4.5; // renders as "-4.5 (spread)" for entitled viewers
...
function gameFixture() {
  return {
    ...
    // The record-integrity layer (readiness/freshness/quality gates) is
    // exercised by preview-projectable-pick.test.ts; here the board is open so
    // the PAYWALL layer is what decides every visibility outcome.
    dataQualityScore: 85,
...
describe("/preview/[sport]/[slug] — server-side paywall (line movement + confidence)", () => {
  ...
  it("PRO: movement delta and confidence render exactly as before, no locked hints", async () => {
    ...
    expect(text).toContain("-4.5 (spread)");
