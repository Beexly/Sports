...
/**
 * Validates structural integrity of the command palette COMMANDS array.
 *
...
// These critical routes MUST be in the palette.
...
describe("CommandPalette COMMANDS array", () => {
  ...
  for (const route of REQUIRED_ROUTES) {
    it(`includes required route: ${route}`, () => {
      expect(
        hrefs.some((h) => h === route || h.startsWith(`${route}?`)),
