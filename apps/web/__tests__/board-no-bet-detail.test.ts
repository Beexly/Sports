...
vi.mock("@sports/db", () => ({
  db: {
    gateDecision: { findMany: (...a: unknown[]) => findMany(...a), /* groupBy: the pass lane's withdrawal watermark (C-161). A db mock has to carry every method the code calls. */ groupBy: async () => [] },
    game: { findMany: async () => [] },
  },
  isStubMode: () => false,
