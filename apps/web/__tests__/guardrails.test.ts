...
describe("Phase 9 guardrails", () => {
  ...
  /**
   * The root `guardrails` script delegates to scripts/guardrails/run-all.mjs (which
   * runs every guard and reports all failures instead of short-circuiting on the
   * first). The chain that matters is therefore the GUARDS table in that file.
   */
  function guardrailsChain(): string {
    const pkg = JSON.parse(readFileSync(resolve(REPO_ROOT, "package.json"), "utf8"));
    const script: string = pkg.scripts.guardrails;
    if (script.includes("run-all.mjs")) {
      return readFileSync(resolve(REPO_ROOT, "scripts/guardrails/run-all.mjs"), "utf8");
    }
    return script;
  }
  ...
  it("root guardrails chain includes commercial, performance, payload-rights, OpenAPI, and raw-NGS checks", () => {
    ...
    const chain = guardrailsChain();
    expect(chain).toContain("commercial-copy-scan.mjs");
    expect(chain).toContain("no-unsupported-performance-claims.mjs");
    expect(chain).toContain("no-raw-ngs-export.mjs");
    expect(chain).toContain("partner-offer-compliance-scan.mjs");
    expect(chain).toContain("api-payload-rights-scan.mjs");
    expect(chain).toContain("openapi-security-scan.mjs");
  });
  ...
  it("root guardrails chain includes the sealed-holdout-open-scan check", () => {
    ...
    expect(guardrailsChain()).toContain("sealed-holdout-open-scan.mjs");
  });
});
