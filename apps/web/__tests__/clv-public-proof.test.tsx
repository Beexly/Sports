 ...
 *      settled picks â€” published, non-bootstrap, graded against a captured
 *      closing line â€” and skips the DB entirely while the readiness gate is
 *      closed.
...
/** 25 graded rows: 15 beat / 8 lost / 2 matched â†’ 60% beat rate. */
...
describe("ClvProofStat â€” render gate", () => {
  ...
  it("falls back to the collecting treatment below the sample floor â€” no percentages", () => {
    ...
    // No rate may leak while collecting â€” a fabricated-looking "100%" from a
...
describe("loadPublicCalibrationReport â€” CLV aggregation from real rows", () => {
  ...
  it("aggregates graded CLV from published, non-bootstrap settled picks when the gate is open", async () => {
    ...
    // Per-pick premium fields are not selected â€” aggregate only.
...
describe("CalibrationPanel â€” wiring (source-level)", () => {
