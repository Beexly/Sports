...
describe("performance segments (units & win% per sport / market)", () => {
  ...
  it("cuts per sportÃ—market with readable labels", () => {
    const segs = bySportMarket(bets);
    expect(segs.find((s) => s.key === "nba|Total")!.label).toBe("NBA Â· Total");
  });

  it("edgeBoard ranks real-sample segments by ROI and benches tiny samples", () => {
    // Only NBAÂ·Total clears a small minSettled; both are below 20, so both are low-sample.
    ...
    // NFLÂ·Spread has only 4 settled â†’ benched even at minSettled 5
