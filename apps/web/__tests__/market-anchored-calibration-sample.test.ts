...
describe("three-way moneyline exclusion uses the engine's helper", () => {
  ...
  it("(a) a soccer moneyline WITH a receipt is excluded from the eligibility sample and counted", () => {
    ...
    expect(out.excluded).toEqual({ three_way_market: 2, no_market_probability: 0, non_moneyline_market: 0, in_play: 0 });
  ...
  it("(a) the shared row builder drops the soccer moneyline from the bake-off rows and counts it", () => {
    ...
    expect(report.excluded).toEqual({ three_way_market: 1, no_market_probability: 0, non_moneyline_market: 0, in_play: 0 });
  });
});
...
describe("(b) no market probability: excluded, never scored on confidence/100", () => {
  it("a MONEYLINE pick with neither factor-breakdown fair nor receipt is counted, not scored", () => {
    ...
    expect(out.samples).toEqual([]);
    expect(out.excluded).toEqual({ three_way_market: 0, no_market_probability: 2, non_moneyline_market: 0, in_play: 0 });
  ...
  it("C-110: single-book resolver results are scored and counted apart from the two-or-more-book recompute", () => {
    ...
    expect(out.excluded).toEqual({ three_way_market: 0, no_market_probability: 1, non_moneyline_market: 0, in_play: 0 });
  ...
  it("the shared eligibility builder never falls back to confidence for a MONEYLINE pick", () => {
    ...
    expect(payload.exclusions).toEqual({ three_way_market: 0, no_market_probability: 1, non_moneyline_market: 0, in_play: 0 });
  });
});
...
describe("C-110 streak basis: market_anchored_v2 restarts a streak counted under market_anchored (v1)", () => {
  ...
  it("pure helpers: the builder tags v2; a v1 prior zeroes the streak and is named on the reset; a v2 prior seeds it", () => {
    ...
    expect(metricsPBasis(payload)).toBe("market_anchored_v3");
    ...
    expect(consecutiveGreenPriorForBasis({ ...v1Snap, pBasis: MARKET_ANCHORED_P_BASIS }, "market_anchored")).toEqual({
      consecutiveGreenPrior: 0,
      streakResetFromBasis: "market_anchored_v3",
    });
  });
  ...
  it("cron path: a GREEN 3/3 streak accumulated under v1 restarts at 1/3 on the first v2 artifact, then builds", async () => {
    ...
    let snap = lastSnap();
    expect(snap.pBasis).toBe("market_anchored_v3");
...
describe("(c) bySport and byModelVersion slices", () => {
  ...
  it("slice n sums to the pooled n; slices use the same metric functions as the pooled row", () => {
    ...
    expect(payload.exclusions).toEqual({ three_way_market: 1, no_market_probability: 1, non_moneyline_market: 0, in_play: 0 });
...
describe("byMarket: the pooled floors sample is two-way moneyline only and the artifact says so", () => {
  ...
  it("keeps only two-way moneylines in the pooled floors sample and counts the rest by market", () => {
    ...
    expect(built.exclusions).toEqual({ three_way_market: 0, no_market_probability: 0, non_moneyline_market: 8, in_play: 0 });
  ...
  it("a pick with no pickType cannot be verified as a two-way moneyline and is excluded, fail closed", () => {
    ...
    expect(built.exclusions).toEqual({ three_way_market: 0, no_market_probability: 0, non_moneyline_market: 1, in_play: 0 });
    const payload = buildDurableMetricsFromSamples({ ...built, samples: built.samples, taggedSamples: built.taggedSamples });
    expect(payload.n).toBe(0);
