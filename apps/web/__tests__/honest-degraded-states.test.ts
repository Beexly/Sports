 ...
 *      calm truth ("Quiet slate", "0 cleared · 0 gated", "Intake warming up").
 *
 * COPY DRIFT, 2026-09-13. Every literal below was remapped after the humanizer
 * pass rewrote the homepage fallbacks ("Live board data unavailable" became
 * "Board temporarily unavailable", "N cleared · N gated" became "N picks · N
 * passes", "Gate holding. No forced action" became "Quiet slate. Nothing
 * forced."). The rewrite was right — "cleared/gated" is the jargon the copy
 * doctrine retired — and this file simply pinned the old words.
 *
 * The part worth remembering is ASYMMETRIC. Six POSITIVE assertions failed
 * loudly, which is how the drift was found. But this file's real teeth are its
 * NEGATIVES — "the outage state must not say Gate holding", "the healthy state
 * must not say unavailable" — and a drifted literal makes a negative pass
 * VACUOUSLY. Nine of those were guarding nothing and complaining about it not
 * at all. When a positive assertion in a file like this goes red, re-check
 * every negative beside it before assuming the rest is fine.
...
describe("/ (home) — outage renders neutral unavailable, not reassuring live zeros", () => {
  ...
  it("nflverse source-error alone withholds the player metric while the healthy board counts stay", async () => {
    ...
    expect(text).toContain("2 picks · 1 pass");
