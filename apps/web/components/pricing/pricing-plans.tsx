    ...
    <div>
      ...
      <div className="flex items-center justify-center gap-3">
        <div
          ...
          className="inline-flex rounded-full border border-mineral bg-eclipse/60 p-1"
        >
            ...
            >
              ...
              <div className="mb-4">
                ...
                <div className="mt-2 flex items-baseline gap-1">
                  {plan.id === "FREE" || plan.monthly === null ? (
                    ...
                    <>
                      ...
                      <span className="text-sm text-ink-300">/year</span>
                    </>
                  ) : (
                    <>
                ...
                {isPaid && !annual && plan.annual !== null && (
                  <p className="mt-1 text-xs text-ink-400">or ${plan.annual}/year</p>
                )}
              ...
              <ul className="mb-6 flex flex-col gap-3">
                {plan.features.map(({ label, included }) => (
                  <li key={label} className="flex items-center gap-2 text-sm">
                    ...
                    <span className={included ? "text-ion-1" : "text-ink-400"}>{label}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="mt-auto">
                {plan.id === "FREE" ? (
                  <Link
                    href="/auth/signin"
                    className="block w-full rounded-xl border border-mineral bg-titanium py-2.5 text-center text-sm font-semibold text-ion-1 transition-colors hover:bg-titanium"
                  >
                    {plan.cta}
                  </Link>
                ) : (
  ...
  return (
    <button
      ...
      className={[
        ...
        active ? "bg-brand-600 text-white" : "text-ink-300 hover:text-ion-1",
      ].join(" ")}
    >
      {children}
    </button>
