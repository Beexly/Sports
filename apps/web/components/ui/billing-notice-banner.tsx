...
export function BillingNoticeBanner({ notice }: { notice: BillingNotice }): JSX.Element {
  ...
  return (
    <div
      ...
      role="alert"
      className="mb-6 rounded-2xl border border-caution/60 bg-caution/10 p-5"
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-sm font-bold text-caution">{headline}</h2>
          <p className="mt-1 max-w-2xl text-xs leading-5 text-ion-1">{detail}</p>
        </div>
