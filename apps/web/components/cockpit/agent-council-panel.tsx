  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-titanium/30 px-5 py-3">
        ...
        <div className="flex gap-4 font-mono text-micro text-ion-3">
          <span>
      ...
      >
        {ledger.storeAvailable ? (
          <p className="font-mono text-micro uppercase tracking-widest text-ion-3">
            Handoff ledger:{" "}
  ...
  return (
    <div className="rounded-xl border border-titanium/30 bg-obsidian/20 p-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div>
          <p className="font-mono text-label font-bold uppercase tracking-widest text-ion-2">
            {department}
          </p>
          <p className="mt-0.5 font-mono text-micro text-ion-3">
        ...
        {hasOwnerApproval && (
          <span className="rounded border border-caution/40 bg-caution/20 px-1.5 py-0.5 font-mono text-micro-xs font-bold uppercase tracking-widest text-caution">
            owner approval
          </span>
        )}
      </div>
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 min-w-0">
          <p className="font-mono text-label font-bold tracking-wider text-ion-white truncate">
            {member.codename}
          </p>
      ...
      {/* Role */}
      <p className="mt-0.5 text-micro font-semibold text-ion-2">{member.role}</p>

      {/* Charter */}
      <p className="mt-1 text-micro leading-snug text-ion-3">{member.charter}</p>
      ...
      {member.ownsCapabilities.length > 0 && (
        <div className="mt-1.5 flex flex-wrap gap-1">
          {member.ownsCapabilities.map((id) => (
            <span
              key={id}
              className="rounded bg-titanium/40 px-1.5 py-0.5 font-mono text-micro-xs text-ion-3"
            >
              {id}
            </span>
          ))}
      ...
      <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5">
        <p className="font-mono text-micro-xs uppercase tracking-wider text-ion-3/50">
        ...
        {member.canSpawnSubagents && (
          <p className="font-mono text-micro-xs uppercase tracking-wider text-plasma/70">
            can spawn subagents
          </p>
        )}
        {member.ownerApprovalRequired && (
          <p className="font-mono text-micro-xs uppercase tracking-wider text-caution/70">
      ...
      {member.status === "NOT_WIRED" && member.reviewGates.length > 0 && (
        <p className="mt-1 font-mono text-micro-xs text-ion-3/40">
