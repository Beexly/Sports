...
export function AskJarvisPanel({ summary }: { summary: OwnerSummary }) {
  ...
  return (
    <section
      ...
      className="overflow-hidden rounded-2xl border border-white/[0.10]/50 bg-white/[0.03]/80"
    >
      {/* Terminal chrome */}
      <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-3">
        <div className="flex items-center gap-3">
          ...
          <span className="font-mono text-[10px] text-ink-500">
            ...
            <span className="text-ink-400">@gse</span>
            <span className="text-ink-500"> — ask</span>
          </span>
        </div>
        <div className="flex items-center gap-3">
          <p className="hidden text-[9px] uppercase tracking-widest text-ink-500 sm:block">
            Deterministic · No model call · Grounded in live state
          </p>
          {active && (
            <button
              onClick={() => { setActive(null); setAnswer(null); }}
              className="font-mono text-[9px] text-ink-500 transition-colors hover:text-ion"
            >
              esc
            </button>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2">
        {/* Command list */}
        <div className="border-b border-white/[0.10]/30 p-4 lg:border-b-0 lg:border-r">
          <p className="mb-3 font-mono text-[10px] text-ink-500">
          ...
          <div className="space-y-3">
            {(Object.keys(JARVIS_INTENT_GROUPS) as JarvisIntentGroup[]).map((group) => (
              <div key={group}>
                <p className="mb-1 px-3 font-mono text-[8px] font-bold uppercase tracking-widest text-ink-500">
                ...
                <div className="space-y-0.5">
                  {JARVIS_INTENT_GROUPS[group].map((intent) => (
                    ...
                    >
                      <span className="w-5 flex-shrink-0 font-mono text-[9px] tabular-nums text-ink-500">
                        {String(JARVIS_INTENT_ORDER.indexOf(intent) + 1).padStart(2, "0")}
                      </span>
                      <span
                        className={[
                          "flex-1 text-[11px] transition-colors",
                          active === intent ? "text-white" : "text-ink-400",
                        ].join(" ")}
                      >
          ...
          {answer ? (
            ...
            >
              ...
              {answer.supportingState.length > 0 && (
                <div className="mb-3 rounded-lg border border-white/[0.06] bg-obsidian/60 p-3">
                  <ul className="space-y-0.5">
                    {answer.supportingState.map((s, idx) => (
                      <li key={idx} className="font-mono text-[9px] text-ink-400">
                        <span className="select-none text-ink-500">· </span>{s}
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              {answer.caveat && (
                <p className="mt-3 text-[9px] italic leading-relaxed text-ink-500">
                  {answer.caveat}
                </p>
              )}

              {answer.nextAction && (
                <div className="mt-3 rounded-lg border border-white/[0.06] bg-obsidian/40 px-3 py-2">
                  <p className="mb-0.5 font-mono text-[8px] uppercase tracking-widest text-ink-500">
                    next_action
                  </p>
                  <p className="text-[10px] leading-snug text-white">{answer.nextAction}</p>
                </div>
              )}
            </div>
          ) : (
              ...
              <div className="flex flex-1 items-center justify-center">
                <div className="text-center">
                  <p className="font-mono text-[10px] text-ink-500">
                    <span className="text-accent-500">$</span>{" "}
                    <span className="text-white">jarvis ask</span>
                    <span className="ml-0.5 inline-block h-3 w-0.5 animate-cursor-blink bg-accent-500 align-middle" />
                  </p>
              ...
              <div className="mt-4 flex flex-wrap gap-1.5">
                {(["picks", "launch-ready", "what-is-wired"] as const).map((intent) => (
                  <button
                    key={intent}
                    onClick={() => handleSelect(intent)}
                    className="rounded-full border border-white/[0.10]/50 bg-obsidian/60 px-3 py-1 font-mono text-[8px] text-ink-400 transition-all hover:border-plasma/40 hover:bg-plasma/10 hover:text-plasma"
                  >
                    {JARVIS_QUESTIONS[intent]}
                  </button>
                ))}
