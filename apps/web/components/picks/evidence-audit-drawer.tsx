    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          {/* Drawer */}
          <aside
            className="relative ml-auto flex h-full w-full max-w-md flex-col overflow-y-auto border-l border-mineral bg-void text-ion-white shadow-2xl motion-safe:animate-[slideInRight_180ms_ease-out]"
          >
            <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-mineral bg-void/95 px-6 py-5 backdrop-blur">
              <div>
              ...
              <button
                ...
                type="button"
                onClick={handleClose}
                className="inline-flex min-h-11 min-w-11 items-center justify-center rounded-md p-2 text-ink-300 transition hover:bg-titanium hover:text-white"
                aria-label="Close"
              >
                <svg
                  width="20"
            ...
            <footer className="border-t border-mineral px-6 py-4 text-[11px] leading-relaxed text-ion-1">
      ...
      <section>
        ...
        <ul className="mt-3 space-y-1.5">
          {audit.signalCategories.map((row) => (
            <li
              key={row.category}
              className="flex items-start justify-between gap-3 rounded-md border border-mineral bg-eclipse/50 px-3 py-2"
            >
              <div>
        ...
        {audit.sourceSnapshots.length === 0 ? (
          ...
          <ul className="mt-3 space-y-1.5">
            {audit.sourceSnapshots.map((snap) => (
              <li
                key={snap.id}
                className="rounded-md border border-mineral bg-eclipse/50 px-3 py-2 text-[11px] leading-relaxed"
              >
                ...
                <div className="mt-1 grid grid-cols-2 gap-x-3 text-ink-300">
                  <span>
      ...
      {preMortem.riskDrivers.length > 0 && (
        <ul className="mt-3 space-y-1.5">
          {preMortem.riskDrivers.map((driver) => (
            <li key={driver} className="text-xs leading-5 text-ink-200">
              {driver}
            </li>
          ))}
        </ul>
...
function Stat({
  ...
  return (
    <div className="rounded-md border border-mineral bg-eclipse/50 px-3 py-2">
...
function Row({ k, v, mono }: { k: string; v: string; mono?: boolean }) {
  return (
    <div className="flex items-baseline justify-between gap-3 border-b border-mineral pb-1.5 last:border-0">
...
function SignalStatusBadge({
  ...
  const styles =
    status === "LIVE"
      ...
      : "border-mineral bg-eclipse text-ion-1";
