...
const PICK_GRADE_STYLES: Record<PickGrade, string> = {
  ...
  LEAN: "text-ink-300 bg-titanium/40",
};
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:justify-between">
        <span className="rounded-full bg-titanium px-2.5 py-0.5 text-xs font-semibold text-ink-200">
          {pick.game.sport}
        </span>
...
function FactorBreakdownPanel({ breakdown }: { breakdown: FactorBreakdown }) {
  ...
  return (
    <div className="rounded-lg border border-mineral/60 bg-void/40 p-3">
      ...
      {breakdown.independentEdge && breakdown.independentEdge.decision !== "PASS" && (
        <div className="mt-2 rounded-md border border-ion-blue/30 bg-ion-blue/5 p-2">
          <div className="mb-0.5 flex items-center gap-1.5">
            ...
            <span className="rounded-full bg-titanium px-1.5 py-0.5 text-[9px] text-ink-300">
  ...
  return (
    <div>
      <div className="mb-0.5 flex justify-between text-[10px]">
        ...
        <span className="font-medium text-ink-300">{Math.round(value)}</span>
      </div>
      <div className="h-1 w-full overflow-hidden rounded-full bg-mineral-hi">
        <div
...
function ConfidenceBadge({ confidence }: { confidence: number }) {
  let color = "text-ink-300 bg-titanium";
...
function DataQualityMeter({ score }: { score: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="h-1 w-16 overflow-hidden rounded-full bg-mineral-hi" aria-hidden="true">
        <div
          className={`h-full rounded-full ${color}`}
          style={{ width: `${clamped}%` }}
        />
    ...
    <>
      <span
        className={`h-1.5 w-1.5 rounded-full ${
          ageMinutes < 10 ? "bg-verify" : ageMinutes < 30 ? "bg-ink-400" : "bg-plasma"
        }`}
        aria-hidden="true"
      />
