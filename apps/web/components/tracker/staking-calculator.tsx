  ...
  return (
    <div className="surface-card p-5">
      ...
      <p className="mt-4 rounded-lg border p-3 text-[11px] leading-relaxed" style={{ borderColor: s.hasEdge ? `${BRAND_COLORS.orbitalCyan}33` : `${BRAND_COLORS.ionMagenta}44`, color: s.hasEdge ? "#aeb8c4" : BRAND_COLORS.ionMagenta }}>
        {s.hasEdge
          ? `At ${Math.round(p * 100)}% to win, the market only implies ${Math.round(s.marketProb * 100)}% — a real edge. ${Math.round(frac * 100)}%-Kelly sizes it at ${s.stakeAmount} of ${bk} units. Full Kelly swings hard; most disciplined bettors stay at a quarter or less.`
