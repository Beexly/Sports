      ...
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
        ...
        <div className="mb-6 flex gap-2">
          {["DFS", "Start-Sit", "Draft Board"].map((_, i) => (
            <div
              key={i}
              className={`h-9 animate-pulse rounded-lg bg-white/[0.05] ${i === 0 ? "w-16" : i === 1 ? "w-20" : "w-24"}`}
            />
          ))}
        </div>
        <div className="flex gap-6">
          <div className="flex-1 rounded-xl border border-white/[0.10] bg-obsidian/60 p-5">
            <div className="mb-4 grid grid-cols-4 gap-4 border-b border-white/[0.10]/30 pb-3">
              {["Player", "Pos/Team", "Proj Pts", "Salary"].map((_, i) => (
                <div key={i} className="h-3.5 animate-pulse rounded bg-white/[0.04]" />
              ))}
            </div>
            <div className="space-y-2.5">
              {[0, 1, 2, 3, 4, 5, 6, 7].map((row) => (
                <div key={row} className="grid grid-cols-4 items-center gap-4 rounded-lg px-2 py-2 hover:bg-white/[0.08]/5">
                  <div className="h-4 w-32 animate-pulse rounded bg-white/[0.05]" />
                  <div className="flex items-center gap-2">
                    <div className="h-5 w-8 animate-pulse rounded bg-white/[0.04]" />
                    <div className="h-3.5 w-16 animate-pulse rounded bg-white/[0.03]" />
                  </div>
          ...
          <div className="w-64 rounded-xl border border-white/[0.10] bg-obsidian/60 p-5">
            <div className="mb-4 h-5 w-28 animate-pulse rounded bg-white/[0.08]" />
            <div className="space-y-2">
              {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((slot) => (
                <div key={slot} className="flex items-center gap-3 rounded-lg border border-white/[0.10]/20 px-3 py-2.5">
                  <div className="h-5 w-8 animate-pulse rounded bg-white/[0.04]" />
                  <div className="h-4 flex-1 animate-pulse rounded bg-white/[0.03]" />
                </div>
              ))}
            </div>
            <div className="mt-5 space-y-2 border-t border-white/[0.10]/30 pt-4">
