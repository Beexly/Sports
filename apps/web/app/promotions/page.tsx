      ...
      <main className="flex-1">
        <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          ...
          <form
            action="/promotions"
            method="GET"
            className="mb-8 flex flex-col gap-3 rounded-xl border border-white/[0.10] bg-white/[0.02] p-5 sm:flex-row sm:items-end"
          >
            <label
              htmlFor="state"
              className="flex flex-col gap-1 text-xs uppercase tracking-widest text-ink-500"
            >
              ...
              <input
                ...
                maxLength={2}
                placeholder="e.g. NJ"
                className="rounded-md border border-white/[0.10] bg-obsidian px-3 py-2 text-sm text-white placeholder-ion-3 focus:border-brand-500 focus:outline-none"
              />
            </label>
            <button
              type="submit"
          ...
          <div className="mt-10 space-y-4">
            <p
              ...
              className="rounded-lg border border-white/[0.10] bg-white/[0.02] p-4 text-xs leading-relaxed text-ink-400"
            >
              {PUBLIC_PROMOTIONS_NOTICE}
            </p>
            <RiskDisclosure />
...
function PromotionCard({ promo }: { promo: PublicPromotion }) {
  return (
    <article
      ...
      className="flex h-full flex-col gap-3 rounded-xl border border-white/[0.10] bg-white/[0.02] p-5"
    >
      ...
      <p className="text-sm text-ink-400">{promo.offerSummary}</p>

      <dl className="grid grid-cols-2 gap-2 text-[11px] text-ink-500">
        <div>
          <dt className="uppercase tracking-widest">Operator</dt>
          <dd className="text-ink-300">{promo.operatorName}</dd>
        </div>
        <div>
          <dt className="uppercase tracking-widest">Min age</dt>
          <dd className="text-ink-300">{promo.minimumAge}+</dd>
        </div>
        {promo.promoCode && (
          <div className="col-span-2">
            <dt className="uppercase tracking-widest">Promo code</dt>
            <dd className="text-ink-300">{promo.promoCode}</dd>
          </div>
        )}
        <div className="col-span-2">
      ...
      <div className="mt-auto flex flex-col gap-2 border-t border-white/[0.10] pt-3">
        <a
          href={promo.termsUrl}
          target="_blank"
...
function PromotionsEmptyState({ state }: { state: string | null }) {
  return (
    <div
      ...
      className="rounded-xl border border-white/[0.10] bg-white/[0.02] p-8 text-center"
    >
