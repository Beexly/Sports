...
export default function HouseError({
  ...
  useEffect(() => {
    console.error("[house] error boundary:", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-obsidian px-6 py-16">
      <div className="w-full max-w-md rounded-2xl border border-alert/40 bg-alert/5 p-8 text-center">
        ...
        <h2 className="text-lg font-bold text-white">NFL House failed to load</h2>
        <p className="mt-2 text-sm text-ink-300">The NFL hub hit an error. Board state and picks are unaffected.</p>
        {error.digest && (
