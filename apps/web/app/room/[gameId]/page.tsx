...
export default async function GameRoomPage({
  ...
  return (
    <div className="min-h-screen bg-void text-ion-white">
      <Nav />
      <main className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-10 sm:px-6 lg:px-8">
        <header className="relative border-b border-mineral pb-8">
          <div
            aria-hidden
            className="pointer-events-none absolute inset-x-0 top-0 -z-10"
            style={{ background: "radial-gradient(60% 80% at 50% 0%, rgba(122,92,255,0.10), transparent 70%)" }}
          />
          <Link href="/board" className="text-sm font-semibold text-orbital-cyan hover:text-[#5BEEFF] focus-visible:ring-2 focus-visible:ring-orbital-cyan">
            Today&apos;s Board
          </Link>
        ...
        <section className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr]">
          <Panel title="Evidence Timeline">
            {room.timeline.length === 0 ? (
              <p className="text-sm text-ink-400">No source-aware signals have been attached to this game yet.</p>
            ) : (
              <ol className="divide-y divide-mineral border border-mineral">
                {room.timeline.map((item) => (
                  <li key={item.id} className="grid gap-2 px-4 py-3 text-sm sm:grid-cols-[1fr_auto]">
                    <div>
                      <p className="font-semibold text-white">{item.label}</p>
                      <p className="mt-1 text-xs text-ink-400">{item.source}</p>
                    </div>
                    <div className="text-left sm:text-right">
                      <p className="font-mono text-xs text-orbital-cyan">{item.status}</p>
                      <p className="mt-1 text-xs text-ink-400">{item.fetchedAt.slice(0, 16).replace("T", " ")}</p>
                    </div>
                  </li>
                ))}
              </ol>
          ...
          <Panel title="What Would Change Our Mind">
            {room.premortem ? (
              <div className="text-sm leading-6 text-ink-200">
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_1fr]">
          <Panel title="Lens Switcher">
            <div className="grid gap-3 sm:grid-cols-2">
              {room.lenses.map((lens) => (
                <article key={lens.lens} className="border border-mineral bg-void/55 p-4">
                  <h2 className="font-mono text-xs font-semibold text-orbital-cyan">{lens.lens}</h2>
                  <p className="mt-3 text-sm leading-6 text-ink-300">{lens.visibleSummary}</p>
                </article>
              ))}
            </div>
          </Panel>
          ...
          <Panel title="Galaxy Memory">
            <div className="text-sm leading-6 text-ink-200">
              ...
              {room.memory.settledAt && <p className="mt-3 text-xs text-ink-400">Settled {room.memory.settledAt.slice(0, 10)}</p>}
            </div>
          </Panel>
        </section>
...
function Panel({ title, children }: { title: string; children: React.ReactNode }): JSX.Element {
  return (
    <section className="border border-mineral bg-eclipse/45 p-5">
...
function Metric({ label, value }: { label: string; value: string }): JSX.Element {
  return (
    <div className="min-h-20 border border-mineral bg-eclipse/60 p-4">
...
function NextStep({ href, label, hint }: { href: string; label: string; hint: string }): JSX.Element {
  return (
    <Link
      href={href}
      className="group block border border-mineral bg-void/55 p-4 transition-colors hover:border-orbital-cyan/40 focus-visible:ring-2 focus-visible:ring-orbital-cyan"
    >
      <p className="font-mono text-xs font-semibold text-orbital-cyan group-hover:text-[#5BEEFF]">{label}</p>
      <p className="mt-2 text-sm leading-6 text-ink-300">{hint}</p>
    </Link>
  );
}
