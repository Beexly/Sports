    ...
    <>
      ...
      <main className="min-h-screen bg-void">
        <div className="max-w-3xl mx-auto px-4 py-16 sm:px-6">
          {/* Back link */}
          <Link href="/blog" className="text-ink-400 hover:text-ink-200 text-sm transition-colors mb-8 inline-flex items-center gap-1">
            ← Back to Blog
          </Link>
          ...
          <article className="mt-6">
            ...
            <div className="mb-8">
              <div className="flex items-center gap-2 mb-4">
                ...
                {post.publishedAt && (
                  <span className="text-xs text-ink-400">{formatDate(post.publishedAt)}</span>
                )}
              </div>
              <h1 className="text-3xl font-bold text-white mb-4">{post.title}</h1>
              <div className="flex flex-wrap gap-1">
                {post.tags.map((tag) => (
                  <span key={tag} className="text-xs bg-titanium text-ink-300 px-2 py-0.5 rounded">
                    #{tag}
                  </span>
                ))}
              </div>
            ...
            <div className="prose prose-invert prose-sm max-w-none">
              ...
              {showFullContent ? (
                <div className="text-ink-200 leading-relaxed whitespace-pre-line mt-4">
                  {post.content}
                </div>
              ) : (
                <div className="relative mt-8">
                  {/* Blur overlay for locked content */}
                  <div className="text-ink-300 leading-relaxed whitespace-pre-line blur-sm select-none pointer-events-none line-clamp-3">
                    {post.content.slice(0, 300)}...
                  </div>
                  <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-b from-transparent via-void/80 to-void">
                    <div className="text-center p-6 bg-eclipse border border-mineral rounded-2xl max-w-sm">
                      ...
                      <p className="text-ink-300 text-sm mb-4">Upgrade to Pro or Elite to read full analysis</p>
                      <Link
                        href="/pricing"
