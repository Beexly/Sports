    ...
    <Shell title={p.name} eyebrow={`${p.team} · ${p.position} · ${p.status}`}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="border border-white/[0.08] bg-white/[0.04] p-4">
          <p className="text-xs uppercase tracking-[0.2em] text-ink-400 mb-3">Galaxy Player Index</p>
          <div className="flex justify-center">
            <ScoreRing score={gpiClamped} label="GPI" size={120} />
          </div>
        </div>
        <div className="border border-white/[0.08] bg-white/[0.04] p-4">
          <p className="text-xs uppercase tracking-[0.2em] text-ink-400 mb-3">Efficiency & Usage Scores</p>
          <BarChart items={[
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="border border-white/[0.08] bg-white/[0.04] p-4">
          <h2 className="text-lg font-semibold text-white mb-3">Archetype</h2>
          <p className="text-ink-300">{arch?.archetype}</p>
          <p className="mt-2 text-sm text-ink-300">{arch?.explanation}</p>
        </div>
        <InsightCard
          eyebrow="Source & Data"
        ...
        >
          {hasMissingData && (
            <div className="space-y-2 mt-2">
              ...
              <p className="text-sm text-ink-300 mt-2">Sources: {Array.isArray(p.source_lineage) && p.source_lineage.length > 0 ? p.source_lineage.join(", ") : "—"}</p>
            </div>
          )}
          {!hasMissingData && (
      ...
      <div>
        <h2 className="text-2xl font-semibold text-white mb-4">Weekly Trend</h2>
        {weeks.length === 0 ? (
          <p className="text-sm text-ink-300 py-4 border border-white/[0.08] bg-white/[0.04]/40 px-4">No weekly data in fixture snapshot — will populate when live ingestion is active.</p>
        ) : (
          <DataTable
      ...
      <div>
        <h2 className="text-2xl font-semibold text-white mb-4">Similar Players</h2>
        {comps.length === 0 ? (
          <p className="text-sm text-ink-300 py-4 border border-white/[0.08] bg-white/[0.04]/40 px-4">No weekly data in fixture snapshot — will populate when live ingestion is active.</p>
        ) : (
          <DataTable
