...
export function Shell({ title, eyebrow, children }: { title: string; eyebrow?: string; children: React.ReactNode }) {
  return (
    <main className="min-h-screen bg-white/[0.03] text-ion">
      {/* Cinematic header */}
      <div className="relative isolate overflow-hidden border-b border-white/[0.06] px-6 pb-8 pt-24">
...
export function Cards({ items }: { items: Array<{label:string; value:string|number; note?:string}> }) { return <div className="grid gap-4 md:grid-cols-4">{items.map(i=><div key={i.label} className="border border-white/[0.08] bg-white/[0.04] p-4"><p className="text-xs uppercase tracking-wide text-ink-400">{i.label}</p><p className="mt-2 text-3xl font-semibold text-white">{i.value}</p>{i.note?<p className="mt-2 text-sm text-ink-300">{i.note}</p>:null}</div>)}</div>; }
export function Badge({ children, tone="neutral" }: { children: React.ReactNode; tone?: "good"|"warn"|"bad"|"neutral" }) { const c={good:"border-orbital-cyan text-orbital-cyan",warn:"border-amber-300 text-amber-300",bad:"border-alert text-alert",neutral:"border-white/[0.08] text-ink-300"}[tone]; return <span className={`inline-flex rounded border px-2 py-1 text-xs ${c}`}>{children}</span>; }
export function PlayerTable({ players }: { players: StatKingPlayer[] }) { return <div className="overflow-x-auto border border-white/[0.08]"><table className="w-full text-left text-sm"><thead className="bg-white/[0.04] text-xs uppercase text-ink-400"><tr><th className="p-3">Player</th><th>Team</th><th>Pos</th><th>GPI</th><th>Usage</th><th>Eff</th><th>Fantasy</th><th>Vol</th><th>Confidence</th></tr></thead><tbody>{players.map(p=><tr key={p.player_id} className="border-t border-white/[0.08]"><td className="p-3 text-white"><Link href={`/stats/player/${p.player_id}`}>{p.name}</Link></td><td>{p.team}</td><td>{p.position}</td><td>{p.galaxy_player_index}</td><td>{p.usage_score}</td><td>{p.efficiency_score}</td><td>{p.fantasy_edge}</td><td>{p.volatility_score}</td><td>{p.data_confidence}%</td></tr>)}</tbody></table></div>; }
export function SimpleTable({ rows }: { rows: Array<Record<string, unknown>> }) { const keys=Object.keys(rows[0]??{}).slice(0,8); return <div className="overflow-x-auto border border-white/[0.08]"><table className="w-full text-left text-sm"><thead className="bg-white/[0.04] text-xs uppercase text-ink-400"><tr>{keys.map(k=><th key={k} className="p-3">{k}</th>)}</tr></thead><tbody>{rows.map((r,i)=><tr key={i} className="border-t border-white/[0.08]">{keys.map(k=>{const v=r[k];return <td key={k} className="p-3">{Array.isArray(v)?v.join(", "):String(v)}</td>;})}</tr>)}</tbody></table></div>; }
...
export function HeroStat({ label, value, delta, sublabel, tone = "cyan" }: { label: string; value: string | number; delta?: string; sublabel?: string; tone?: "cyan" | "amber" | "alert"; }) {
  ...
  return (
    <div className={`border bg-white/[0.04] p-6 flex flex-col gap-1 rounded-lg transition-all duration-200 ${styles.border}`}>
      ...
      {sublabel && <p className="mt-1 text-sm text-ink-300">{sublabel}</p>}
    </div>
  );
}
...
export function BarChart({ items }: { items: Array<{ label: string; value: number; max: number; tone?: "cyan" | "amber" | "alert" }> }) {
  return <div className="space-y-3">{items.map((item) => {
    ...
    return <div key={item.label} className="flex items-center gap-3"><span className="w-36 shrink-0 truncate text-xs text-ink-300">{item.label}</span><div className="flex-1 h-1.5 rounded-full overflow-hidden bg-white/[0.04] border border-white/[0.08]"><div className={`h-full rounded-full ${fill}`} style={{ width: `${pct}%` }} /></div><span className="w-12 text-right text-xs font-mono text-white tabular-nums">{item.value}</span></div>;
  })}</div>;
}
...
export function ScoreRing({ score, label, size = 120 }: { score: number; label?: string; size?: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      {label && <p className="text-xs uppercase tracking-[0.2em] text-ink-400 text-center">{label}</p>}
    </div>
  );
}
...
export function InsightCard({ eyebrow, headline, body, tone = "neutral", children }: { eyebrow?: string; headline: string; body: string; tone?: "good" | "warn" | "bad" | "neutral"; children?: React.ReactNode; }) {
  const bdr = { good: "border-orbital-cyan", warn: "border-caution", bad: "border-alert", neutral: "border-white/[0.08]" }[tone];
  const ec = { good: "text-orbital-cyan", warn: "text-caution", bad: "text-alert", neutral: "text-ink-400" }[tone];
  return <div className={`border ${bdr} bg-white/[0.04] p-5 space-y-2`}>{eyebrow && <p className={`text-xs uppercase tracking-[0.2em] ${ec}`}>{eyebrow}</p>}<p className="text-base font-semibold text-white">{headline}</p><p className="text-sm text-ink-300 leading-relaxed">{body}</p>{children && <div className="mt-1">{children}</div>}</div>;
}

export function SectionHeader({ eyebrow, title, action }: { eyebrow?: string; title: string; action?: { label: string; href: string } }) {
  return <div className="flex items-end justify-between border-b border-white/[0.08] pb-3"><div>{eyebrow && <p className="text-xs uppercase tracking-[0.2em] text-ink-400 mb-1">{eyebrow}</p>}<h2 className="text-2xl font-semibold text-white">{title}</h2></div>{action && <a href={action.href} className="text-sm text-orbital-cyan hover:text-white transition-colors">{action.label} →</a>}</div>;
}
...
export function DataTable({ rows, maxRows = 200 }: { rows: Array<Record<string, unknown>>; maxRows?: number }) {
  ...
  if (!rows.length) return <p className="text-sm text-ink-300 py-4">No data.</p>;
  return <div className="border border-white/[0.08] overflow-x-auto" style={{ maxHeight: "60vh", overflowY: "auto" }}><table className="w-full text-left text-sm"><thead className="sticky top-0 z-10 bg-white/[0.04]"><tr>{keys.map((k, i) => <th key={k} className={`p-3 text-xs uppercase tracking-wide text-ink-400 font-medium whitespace-nowrap${i > 0 ? " border-l border-white/[0.08]" : ""}`}>{k.replace(/_/g, " ")}</th>)}</tr></thead><tbody>{rows.slice(0, maxRows).map((r, ri) => <tr key={ri} className="border-t border-white/[0.08] hover:bg-white/[0.04]/60 transition-colors">{keys.map((k, ki) => { const v = r[k]; const num = ki > 0 && isNum(v); return <td key={k} className={`p-3${ki === 0 ? " font-semibold text-white" : num ? " text-right font-mono text-ink-300 tabular-nums" : " text-ink-300"}${ki > 0 ? " border-l border-white/[0.08]" : ""}`} title={Array.isArray(v) ? (v as unknown[]).join(", ") : String(v ?? "")}>{display(v)}</td>; })}</tr>)}</tbody></table></div>;
}
