...
/**
 * Stats segment error boundary.
 *
 * If anything inside /stats/* throws on render, this catches it closer
 * to the surface than the root boundary, keeps the branded chrome, and
 * offers a retry plus a path back to the stats overview.
 */
export default function StatsError({
  ...
  useEffect(() => {
    initObservability();
    console.error("[stats] error boundary caught:", error);
      ...
      <main className="relative z-10 flex flex-1 items-center justify-center px-4 py-22 sm:px-6 lg:px-8">
        <div className="w-full max-w-xl rounded-2xl border border-red-900 bg-red-950/30 p-8 text-center">
          ...
          <h1 className="text-xl font-bold text-white">
            The stats surface hit a snag.
          </h1>
          <p className="mt-2 text-sm text-red-200">
            This page threw a runtime error. Hit retry, or jump back to the
            stats overview — the observatory has the trace either way.
          </p>
