      ...
      <main id="main-content" className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
        <section className="grid gap-8 border-b border-white/[0.08] pb-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
          ...
          <div className="border border-white/[0.08] bg-white/[0.04] p-5">
            ...
            <div className="mt-5 border border-white/[0.08] bg-white/[0.03] p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-ink-300">{pulse.blockReason}</p>
            </div>
          </div>
        </section>
        ...
        {pulse.status === "source-error" ? (
          <section className="border border-white/[0.08] bg-white/[0.04] p-5">
            ...
            <h2 className="mt-2 text-2xl font-semibold text-white">The pulse is intentionally empty.</h2>
            <p className="mt-3 text-sm leading-6 text-ink-300">{pulse.error ?? "UNKNOWN"}</p>
          </section>
        ) : (
          <>
            <section className="border border-white/[0.08] bg-white/[0.04]/80">
              <div className="flex flex-col justify-between gap-3 border-b border-white/[0.08] px-5 py-4 sm:flex-row sm:items-end">
                <div>
                  <p className="font-mono text-[10px] uppercase tracking-[0.18em] text-orbital-cyan">
                    Player usage
                  </p>
                  <h2 className="mt-2 text-2xl font-semibold text-white">Top opportunity rows</h2>
                </div>
                  ...
                  <tbody className="divide-y divide-mineral bg-white/[0.03]">
                    {pulse.playerRows.map((row) => (
                      <tr key={`${row.playerId}-${row.team}-${row.opponent}`}>
                        ...
                        <td className="px-4 py-3 font-mono text-ink-400">{row.opponent}</td>
                        <td className="px-4 py-3 font-mono text-white">{row.opportunities}</td>
                  ...
                  <tbody className="divide-y divide-mineral bg-white/[0.03]">
                    {pulse.qbAgeRows.map((row) => (
                      <tr key={`${row.team}-${row.qbName}`}>
                        <td className="px-4 py-3 font-mono text-orbital-cyan">{row.team}</td>
                        <td className="px-4 py-3">
                          <p className="font-semibold text-white">{row.qbName}</p>
                          <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-400">
                            vs {row.opponent}
                          </p>
                        </td>
                        ...
                        <td className="px-4 py-3 font-mono text-white">{fmtPercent(row.rbTargetShare)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="border border-white/[0.08] bg-white/[0.04] p-5">
                <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                  ...
                  <Link
                    ...
                    className="text-sm font-semibold text-orbital-cyan hover:text-white"
                  >
                    JSON
                  </Link>
                </div>
                {oldQbTrend ? (
                  ...
                  <p className="mt-5 text-sm leading-6 text-ink-300">No cohort cleared the minimum sample gate.</p>
                )}
                ...
                <div className="mt-5 border border-white/[0.08] bg-white/[0.03] p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-ink-300">{qbAgeTrend.boundary}</p>
                </div>
              </div>
            ...
            <section className="border border-white/[0.08] bg-white/[0.04] p-5">
              <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-semibold text-white">Recent 34+ QB team weeks</h2>
                </div>
                <p className="max-w-xl text-sm leading-6 text-ink-300">
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-2 lg:grid-cols-4">
                {qbAgeTrend.examples.map((row) => (
                  <article key={`${row.season}-${row.week}-${row.team}`} className="border border-white/[0.08] bg-white/[0.03] p-4">
                    <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-orbital-cyan">
                      {row.season} week {row.week}
                    </p>
                    <h3 className="mt-2 font-semibold text-white">{row.qbName}</h3>
                    <p className="mt-1 text-xs uppercase tracking-[0.14em] text-ink-400">
                      {row.team} vs {row.opponent} / age {row.qbAge}
                    </p>
                    <p className="mt-4 font-numerals text-2xl font-semibold text-white">
            ...
            <section className="grid gap-6 lg:grid-cols-[0.82fr_1.18fr]">
              <div className="border border-white/[0.08] bg-white/[0.04] p-5">
                ...
                {birthdayResult ? (
                  ...
                  <p className="mt-5 text-sm leading-6 text-ink-300">The birthday-window report is unavailable.</p>
                )}
                <p className="mt-5 text-sm leading-6 text-ink-300">
                ...
                <div className="mt-5 border border-white/[0.08] bg-white/[0.03] p-4">
                  ...
                  <p className="mt-2 text-sm leading-6 text-ink-300">{birthdayTrend.boundary}</p>
                </div>
              </div>
...
function Metric({ label, value }: { label: string; value: string }): JSX.Element {
  return (
    <div className="border border-white/[0.08] bg-white/[0.03] px-3 py-2">
      ...
      <dd className="mt-1 font-numerals text-xl font-semibold tabular-nums text-white">{value}</dd>
    </div>
  );
}
