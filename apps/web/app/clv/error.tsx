...
export default function ClvError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error("[clv] error boundary:", error);
  }, [error]);
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-obsidian px-6 py-16">
      <div className="w-full max-w-md rounded-2xl border border-alert/40 bg-alert/5 p-8 text-center">
        <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full border border-alert/40 bg-alert/10">
          <svg aria-hidden="true" className="h-6 w-6 text-alert" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
          </svg>
        </div>
        <h2 className="text-lg font-bold text-white">CLV page failed to load</h2>
        <p className="mt-2 text-sm text-ink-300">The closing line value report hit an error. Your picks data is safe.</p>
        {error.digest && (
