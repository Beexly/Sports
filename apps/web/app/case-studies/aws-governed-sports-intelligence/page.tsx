      ...
      <main id="main-content" className="flex-1">
        <section className="border-b border-ink-800/60 px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-5xl">
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-ink-300">
        ...
        <section className="px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.9fr_1.1fr]">
            <div>
              ...
              <h2 className="mt-3 font-display text-display-lg text-white">Aggressive architecture, conservative claims.</h2>
            ...
            <div className="grid gap-4">
              {AWS_CASE_STUDY_BOUNDARIES.map((boundary) => (
                <article key={boundary.label} className="surface-card p-5">
                  <h3 className="text-base font-semibold text-white">{boundary.label}</h3>
                  <p className="mt-2 text-sm leading-6 text-ink-300">{boundary.copy}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="border-t border-ink-800/60 px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-6xl">
            ...
            <h2 className="mt-3 font-display text-display-lg text-white">Six pillars translated into GSE controls.</h2>
            <div className="mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {AWS_CASE_STUDY_PILLARS.map((pillar) => (
                <article key={pillar.id} className="surface-card p-5">
                  ...
                  <p className="mt-3 text-sm leading-6 text-ink-300">{pillar.gseControl}</p>
                  <p className="mt-4 text-xs uppercase tracking-widest text-accent-300">{pillar.publicTakeaway}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="border-t border-ink-800/60 px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1fr_1fr]">
            <div>
              ...
              <h2 className="mt-3 font-display text-display-lg text-white">What the repo can show without touching AWS.</h2>
              <p className="mt-4 text-sm leading-7 text-ink-300">
            ...
            <div className="space-y-4">
              {AWS_CASE_STUDY_PROOF_POINTS.map((proof) => (
                <article key={proof.label} className="surface-card p-5">
                  <h3 className="text-base font-semibold text-white">{proof.label}</h3>
                  <p className="mt-2 text-sm leading-6 text-ink-300">{proof.copy}</p>
                  <p className="mt-3 text-xs uppercase tracking-widest text-ink-500">{proof.sourcePaths.join(" / ")}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section className="border-t border-ink-800/60 px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-4xl">
            ...
            <h2 className="mt-3 font-display text-display-lg text-white">What this page does not unlock.</h2>
