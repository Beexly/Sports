          ...
          <section className="mt-20">
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {VALUE_TIERS.filter((t) => t.status === "live").map((t) => (
                <div
                  key={t.id}
                  className="flex h-full flex-col rounded-2xl border border-mineral bg-eclipse/40 p-5"
                >
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {t.status === "waitlist" && (
                      <span className="rounded-full border border-mineral px-2 py-0.5 text-[10px] font-medium text-ink-300">
                        Waitlist
                      </span>
                    )}
                  </div>
                  ...
                  {t.whyNextTier && (
                    <p className="mt-3 border-t border-mineral pt-3 text-xs leading-relaxed text-ink-300">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-mineral">
                    <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-ink-300">
                      Feature
                    </th>
                    {PLANS.map((plan) => (
                      <th
              ...
              {FAQ.map((item) => (
                ...
                >
                  <summary className="flex cursor-pointer items-center justify-between gap-4 text-left text-sm font-semibold text-ion-white">
                    ...
                    <span
                      aria-hidden="true"
                      className="text-ink-300 transition-transform group-open:rotate-45"
                    >
                      +
                    </span>
                  </summary>
      ...
      <ul className="mt-4 space-y-2.5">
        {doors.map((d) => (
          <li key={d.href + d.label}>
            <Link
              href={d.href}
              className="group flex items-center justify-between rounded-lg border border-transparent px-3 py-2 text-sm text-ink-200 transition-colors hover:border-mineral hover:bg-eclipse/50 hover:text-white"
            >
              {d.label}
