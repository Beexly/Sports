...
export default async function Page(){
  ...
  const mappedRows=feedback.map(f=>{const rec=f as Record<string, unknown>; return { id: String(rec.feedback_id ?? rec.id ?? ""), entity: String(rec.entity_id ?? ""), type: String(rec.feedback_type ?? ""), content: String(rec.note ?? rec.feedback_text ?? ""), created_at: String(rec.created_at ?? "") };});
  ...
  <p className="text-ink-300">Inbound user feedback on players and metrics — a review input, not an automatic model signal.</p>
