...
export default async function Page(){
  ...
  return <Shell title="Outreach & Activation ROI"><StatusRibbon status="fixture" label="Admin view — fixture snapshot" /><Cards items={[{label:"Activate now",value:targets.length},{label:"Low-cost wins",value:(roi.top_25_zero_low_cost_wins ?? []).length},{label:"License targets",value:(roi.top_25_license_targets ?? []).length},{label:"Expert/partner",value:(roi.top_25_expert_partner_targets ?? []).length}]}/><SectionHeader title="Top Activation Targets" /><DataTable rows={mappedRows} maxRows={50} /></Shell>;
}
