  ...
  return (
    <div className="min-h-screen bg-obsidian">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-10 sm:px-6">
        <div className="h-6 w-40 animate-pulse rounded bg-titanium" />
        <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
          {Array.from({ length: 3 }).map((_, i) => (
            ...
            >
              {/* Stat value */}
              <div className="h-9 w-24 animate-pulse rounded-lg bg-titanium" />
              {/* Stat label */}
              <div className="mt-3 h-3.5 w-28 animate-pulse rounded bg-titanium/60" />
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6">
        <div className="overflow-hidden rounded-2xl border border-titanium/40">
          {/* Table header */}
          <div className="grid grid-cols-7 gap-4 border-b border-titanium/40 bg-titanium/5 px-5 py-3">
            {[80, 56, 72, 64, 56, 96, 60].map((w, i) => (
              <div
                key={i}
                className="h-3 animate-pulse rounded bg-titanium/60"
                style={{ width: `${w}%` }}
              />
            ))}
          </div>
          ...
          {Array.from({ length: 8 }).map((_, rowIdx) => (
            <div
              key={rowIdx}
              className="grid grid-cols-7 items-center gap-4 border-b border-titanium/20 px-5 py-4 last:border-b-0"
            >
              {/* Matchup name */}
              ...
              <div className="h-4 w-24 animate-pulse rounded bg-titanium/60" />
              {/* Result badge */}
              <div className="h-5 w-12 animate-pulse rounded-full bg-titanium/50" />
              {/* Date */}
              <div className="h-3.5 w-20 animate-pulse rounded bg-titanium/50" />
              {/* Confidence bar */}
              <div className="h-2.5 w-full animate-pulse rounded-full bg-titanium/40" />
              {/* Model version */}
