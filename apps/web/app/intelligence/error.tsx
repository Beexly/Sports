...
export default function IntelligenceError({
  ...
  useEffect(() => {
    ...
    console.error("[intelligence] error boundary:", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-obsidian px-6 py-16">
      <div className="w-full max-w-md rounded-2xl border border-alert/40 bg-alert/5 p-8 text-center">
        ...
        <h2 className="text-lg font-semibold text-white">Intelligence view unavailable</h2>
        <p className="mt-2 text-sm text-ink-300">
          The glass-box engine hit an error during render. This is a display issue only — the
          methodology and underlying records are unaffected.
        </p>
        {error.digest && (
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          ...
          <Link
            href="/"
            className="rounded-xl border border-white/[0.10] px-5 py-2 text-sm text-ink-300 hover:bg-white/[0.04] hover:text-white transition-colors"
          >
            Home
          </Link>
        </div>
