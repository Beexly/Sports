  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <Link href="/cockpit/agents" className="text-xs text-gray-500 hover:text-gray-300">
            ← All agents
          </Link>
          <h1 className="mt-1 text-2xl font-bold text-white">{agent.displayName}</h1>
          <p className="mt-1 max-w-2xl text-sm text-gray-500">{agent.responsibility}</p>
        </div>
        <span className="rounded-full bg-gray-800 px-3 py-1 text-xs font-semibold text-gray-200">
