    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-lg border border-white/[0.06] bg-obsidian/60 p-4">
        ...
        {dashboard.recentErrors.length === 0 ? (
          <p className="mt-3 text-sm text-ink-500">No Claude API errors recorded.</p>
        ) : (
          <div className="mt-3 grid gap-2">
            {dashboard.recentErrors.map((error) => (
              <div key={error.id} className="rounded-md border border-white/[0.06] bg-white/[0.04]/50 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm font-medium text-ink-300">{error.surface}</p>
