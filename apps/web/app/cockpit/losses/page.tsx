    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-2">
        <div className="flex items-center justify-between gap-3">
          ...
          <Link href="/cockpit" className="rounded-lg border border-white/[0.06] px-3 py-1.5 text-xs text-ink-300 hover:bg-white/[0.03]">
            Back to Jarvis
          </Link>
        </div>
      ...
      {rows.length === 0 ? (
        <div className="rounded-lg border border-white/[0.06] bg-obsidian/50 p-6 text-sm text-ink-400">
          No loss autopsies have been authored yet.
        </div>
      ) : (
        <ul className="flex flex-col gap-2">
          {rows.map((row) => (
            <li key={row.id} className="rounded-lg border border-white/[0.06] bg-obsidian/50 p-4">
