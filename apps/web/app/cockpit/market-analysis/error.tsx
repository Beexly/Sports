...
/**
 * Route-level error boundary for the Market Analysis workbench.
 *
 ...
 * than a Next.js stack trace. The Odds API is never called from this surface.
 */
export default function MarketAnalysisError({
  ...
  useEffect(() => {
    ...
    console.error("[cockpit/market-analysis] error boundary caught:", error);
  }, [error]);

  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-red-900 bg-red-950/30 p-6 text-red-200">
      <h2 className="text-lg font-semibold text-white">Market Analysis workbench error</h2>
      <p className="text-sm">
        ...
        and the Odds API is never called from here.
      </p>
