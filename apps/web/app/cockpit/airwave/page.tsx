...
const STATUS_TONE: Record<AirwaveLaneStatus, string> = {
  ...
  "missing-config": "border-white/[0.06] bg-white/[0.04]/60 text-ink-300",
...
const VERDICT_TONE: Record<ClaimVerdict, string> = {
  ...
  UNFALSIFIABLE: "text-ink-400",
  PENDING: "text-yellow-300",
};

const INTAKE_TONE: Record<AirwaveIntakeStatus, string> = {
  "not-configured": "border-white/[0.06] bg-white/[0.04]/60 text-ink-300",
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-2xl border border-white/[0.06] bg-white/[0.04]/40 p-5">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="rounded-xl border border-white/[0.06] bg-obsidian/70 p-4">
            <h3 className="text-sm font-semibold text-white">Gate reason</h3>
            <p className="mt-2 text-sm leading-6 text-ink-400">{intake.source.reason}</p>
          ...
          <div className="overflow-hidden rounded-xl border border-white/[0.06]">
            <table className="w-full text-left text-xs">
              <thead className="bg-obsidian/60 text-[10px] uppercase tracking-widest text-ink-500">
                <tr>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.04]/40 p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <div className="rounded-lg border border-white/[0.06] bg-obsidian/60 px-3 py-2 text-[11px] text-ink-400">
              master: <span className="text-white">{String(control.env.enabled)}</span>
              <span className="mx-2 text-ink-500">/</span>
              legal ack: <span className="text-white">{String(control.env.siriusxmLegalAck)}</span>
            </div>
          </div>
              ...
              <tbody className="divide-y divide-titanium/30">
                {control.spreadsheetContract.map((field) => (
                  <tr key={field.column} className="align-top">
                    ...
                    <td className="px-3 py-2 text-ink-400">{field.purpose}</td>
                    <td className="px-3 py-2 text-ink-500">{field.required ? "yes" : "no"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.04]/40 p-5">
          ...
          <div className="mt-4 grid gap-2 md:grid-cols-2">
            {control.adapters.map((adapter) => (
              <div key={adapter.kind} className="rounded-lg border border-white/[0.06] bg-obsidian/70 p-3">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-semibold text-white">{adapter.label}</p>
                ...
                <p className="mt-2 text-xs leading-5 text-ink-500">{adapter.reason}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-white/[0.06] bg-white/[0.04]/40 p-5">
        ...
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-[920px] w-full text-left text-xs">
            <thead>
              <tr className="border-b border-white/[0.06] text-ink-500">
            ...
            <tbody>
              {DEMO_CLAIMS.map((claim) => (
                <tr key={claim.id} className="border-b border-white/[0.06] align-top">
                  <td className="py-2 pr-3 font-mono text-[11px] text-ink-400">
                    {claim.airedAt.slice(0, 16).replace("T", " ")}
                  </td>
                  <td className="py-2 pr-3 text-ink-300">{punditName(claim.punditId)}</td>
      ...
      {/* ── Claim Review Queue ── */}
      <section data-testid="claim-review-queue" className="rounded-2xl border border-white/[0.06] bg-white/[0.04]/40 p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h2 className="text-xs font-semibold uppercase tracking-widest text-ink-500">
              Claim Review Queue
        ...
        <div className="mt-4 rounded-xl border border-white/[0.06] bg-obsidian/70 p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-xs">
            {(["DRAFT", "REVIEW", "APPROVED", "REJECTED", "SETTLED"] as ClaimCandidateOperatorStatus[]).map((s, i, arr) => (
              <span key={s} className="flex items-center gap-2">
                ...
                {i < arr.length - 1 && i !== 2 && <span className="text-ink-500">→</span>}
                {i === 2 && <span className="text-ink-500 mx-1">or</span>}
              </span>
            ))}
          </div>
      ...
      <section data-testid="intelligence-intake-posture" className="rounded-2xl border border-cyan-900/40 bg-cyan-950/10 p-5">
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-2">
          ...
          <div data-testid="gse-gsn-output-readiness" className="rounded-xl border border-white/[0.06] bg-obsidian/70 p-4">
            <h3 className="text-sm font-semibold text-white">GSE / GSN Output Readiness</h3>
            <div className="mt-3 grid gap-4 sm:grid-cols-2">
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest text-ink-500">GSE</p>
              ...
              <div>
                <p className="text-[10px] font-semibold uppercase tracking-widest text-ink-500">GSN</p>
                <dl className="mt-2 grid gap-2 text-xs">
      ...
      <section data-testid="intelligence-intake-lanes" className="rounded-2xl border border-white/[0.06] bg-white/[0.04]/40 p-5">
        <h2 className="text-xs font-semibold uppercase tracking-widest text-ink-500">
          Intelligence Intake Lanes
        </h2>
        <p className="mt-2 text-sm leading-6 text-ink-400">
      ...
      {intelligence.operatorSurface.nextOperatorActions.length > 0 && (
        <section data-testid="next-operator-actions" className="rounded-2xl border border-white/[0.06] bg-white/[0.04]/40 p-5">
          <h2 className="text-xs font-semibold uppercase tracking-widest text-ink-500">
            Next Operator Actions
          </h2>
          <ul className="mt-4 space-y-2">
            {intelligence.operatorSurface.nextOperatorActions.map((action, i) => (
              <li key={i} className="flex items-start gap-3 rounded-lg border border-white/[0.06] bg-obsidian/50 px-3 py-2 text-sm text-ink-300">
...
function Fact({ label, value }: { label: string; value: string }): JSX.Element {
  return (
    <div className="flex items-center justify-between gap-3 rounded-lg border border-white/[0.06] bg-black/20 px-3 py-2">
      ...
      <dd className="font-mono text-[11px] text-ink-300">{value}</dd>
    </div>
  );
}

const REVIEW_STATUS_TONE: Record<string, string> = {
  DRAFT: "border-white/[0.06] bg-white/[0.04]/60 text-ink-400",
...
const INTAKE_MODE_TONE: Record<string, string> = {
  OFF: "border-white/[0.06] bg-white/[0.04]/60 text-ink-400",
...
function IntakeLaneCard({ lane }: { lane: IntakeLaneState }): JSX.Element {
  ...
  return (
    <article className="rounded-xl border border-white/[0.06] bg-obsidian/70 p-4">
      ...
      <p className="mt-2 text-xs leading-5 text-ink-500">{lane.nextOperatorAction}</p>
    </article>
  );
}
    ...
    <article className="rounded-xl border border-white/[0.06] bg-obsidian/70 p-4">
      ...
      <dl className="mt-3 grid gap-3 text-xs md:grid-cols-2">
        <div>
          ...
          <dd className="mt-1 text-ink-300">{lane.cadence}</dd>
        </div>
        <div>
        ...
        <div>
          <dt className="text-ink-500">Env slots</dt>
          <dd className="mt-1 font-mono text-[11px] text-ink-500">{lane.envVars.join(", ")}</dd>
        </div>
      </dl>
