...
/**
 * Route-level error boundary for the Sports Diagnostics workbench.
 *
 ...
 * than a Next.js stack trace. No external sports API is called from this surface.
 */
export default function SportsDiagnosticsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    ...
    console.error("[cockpit/sports-diagnostics] error boundary caught:", error);
  }, [error]);

  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-red-900 bg-red-950/30 p-6 text-red-200">
      <h2 className="text-lg font-semibold text-white">Sports Diagnostics workbench error</h2>
      <p className="text-sm">
        ...
        and no external sports API is called from here.
      </p>
