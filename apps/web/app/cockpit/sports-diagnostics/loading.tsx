/**
 * Route-level loading fallback for the Sports Diagnostics workbench.
 *
 ...
 * metric grid + per-sport section cards) rather than a full-screen layout. Purely
 * decorative: it asserts no figure and carries no data — it is a placeholder, so
 * it must never read as a real (and therefore fabricated) zero.
 */
export default function SportsDiagnosticsLoading(): JSX.Element {
  return (
    <div
      className="flex flex-col gap-6"
      role="status"
      aria-busy="true"
      aria-label="Loading the Sports Diagnostics workbench"
    >
      <span className="sr-only">Loading the Sports Diagnostics workbench…</span>
        ...
        >
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {Array.from({ length: 4 }).map((_, j) => (
              <div key={j} className="h-12 animate-pulse rounded bg-white/[0.04]" />
            ))}
          </div>
        </div>
      ))}
