...
async function fetchPicks(
  ...
  if (!res.ok) {
    ...
    // a backend outage (reason:"backend_outage" — the API's DB read failed).
    // Each renders as a DESIGNED board state with its own copy — never a raw
    // thrown error. Before the outage branch existed, a backend_outage body
    // fell through to the throw below and customers saw the literal string
    ...
    <div className="flex min-h-screen flex-col bg-obsidian">
      ...
      <main id="main-content" className="flex-1 px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl">
          ...
          {/* Empty state — the three designed dark states (gated / stale / outage).
              The outage variant is amber and says so plainly: an outage rendered
              as a calm "collecting" board would be dressing a failure as intent. */}
              ...
              <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-ion-2">
                {bootstrapState.kind === "outage"
                  ? "We hit a problem reading the board just now. Nothing is " +
                    "wrong with your account and no picks were lost — the board " +
                    "comes back automatically. Try refreshing in a minute."
                  : bootstrapState.kind === "stale"
