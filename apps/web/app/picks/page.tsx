      ...
      <main className="flex-1 px-4 py-10 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl">
              ...
              {SPORTS.map(({ key, label }) => {
                ...
                return (
                  <Link
                    ...
                    className={[
                      "inline-flex min-h-11 items-center rounded-lg border px-3 py-1.5 text-sm font-semibold transition-colors focus-visible:ring-2 focus-visible:ring-orbital-cyan",
                      isActive
                        ? "border-orbital-cyan bg-orbital-cyan text-void shadow-[0_0_18px_rgba(0,229,255,0.35)]"
                        : "border-mineral bg-eclipse text-ion-1 hover:border-orbital-cyan hover:text-white",
                    ].join(" ")}
                  >
                    {label}
                  </Link>
              ...
              {GRADES.map(({ key, label }) => {
                ...
                return (
                  <Link
                    ...
                    href={`/picks${p.toString() ? `?${p}` : ""}`}
                    className={[
                      "inline-flex min-h-11 items-center rounded-full border px-3 py-1 text-xs font-semibold transition-colors focus-visible:ring-2 focus-visible:ring-orbital-cyan",
                      isActive
                        ? "border-ion-magenta bg-ion-magenta text-void shadow-[0_0_18px_rgba(255,45,214,0.35)]"
                        : "border-mineral bg-eclipse text-ion-1 hover:border-ion-magenta hover:text-white",
                    ].join(" ")}
                  >
                    {label}
                  </Link>
          ...
          {!fetchError && bootstrapState && picks.length === 0 && (
            <div className="rounded-xl border border-orbital-cyan/25 bg-orbital-cyan/10 p-8 text-center shadow-[0_0_28px_rgba(0,229,255,0.10)]">
              ...
              <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
                <Link
                  href="/methodology"
                  className="rounded-lg border border-orbital-cyan/30 bg-orbital-cyan/10 px-4 py-2 text-sm font-semibold text-orbital-cyan transition-colors hover:border-orbital-cyan hover:bg-orbital-cyan hover:text-void focus-visible:ring-2 focus-visible:ring-orbital-cyan"
                >
                  Read methodology
                </Link>
                <Link
                  href="/vault"
                  className="rounded-lg border border-mineral bg-eclipse px-4 py-2 text-sm font-semibold text-ion-1 transition-colors hover:border-ion-magenta hover:text-white focus-visible:ring-2 focus-visible:ring-orbital-cyan"
                >
                  View The Vault
                </Link>
              </div>
          ...
          {/* Empty state */}
          {!fetchError && !bootstrapState && picks.length === 0 && (
            <div className="rounded-xl border border-mineral bg-eclipse/60 p-12 text-center">
          ...
          {isFreeTier && picks.length > 0 && (
            <div className="mt-10 rounded-xl border border-soft-ultraviolet/30 bg-soft-ultraviolet/10 p-6 text-center">
              ...
              <Link
                href="/pricing"
                className="mt-4 inline-flex rounded-lg bg-orbital-cyan px-6 py-2.5 text-sm font-semibold text-void shadow-[0_0_24px_rgba(0,229,255,0.25)] transition-colors hover:bg-[#5BEEFF] focus-visible:ring-2 focus-visible:ring-orbital-cyan"
              >
          ...
          {isPro && entitlements.tier === "PRO" && picks.length > 0 && (
            <div className="mt-8 rounded-xl border border-soft-ultraviolet/30 bg-soft-ultraviolet/10 p-4 text-center">
              <p className="text-xs text-soft-ultraviolet">
                Want real-time email and push alerts on every signal?{" "}
                <Link href="/pricing" className="font-semibold underline underline-offset-2 focus-visible:ring-2 focus-visible:ring-orbital-cyan">
...
function SlateBar({ slate }: { slate: DailySlate }) {
  ...
  return (
    <div className="mb-6 rounded-xl border border-orbital-cyan/20 bg-carbon/80 px-5 py-4 shadow-[0_0_28px_rgba(0,229,255,0.12)]">
      <div className="flex flex-wrap items-center gap-3">
        ...
        {/* Recent record */}
        {record && (
          <div className="rounded-lg border border-mineral bg-eclipse px-3 py-2">
            ...
            <span className="mx-1 text-xs text-ink-400">/</span>
        ...
        <div className="mt-3 flex flex-wrap gap-2 border-t border-orbital-cyan/10 pt-3">
          {slate.sportBreakdown.map(({ sport, pickCount }) => (
            <Link
              ...
              className="rounded-full border border-orbital-cyan/25 bg-orbital-cyan/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-orbital-cyan transition-colors hover:border-orbital-cyan hover:bg-orbital-cyan hover:text-void focus-visible:ring-2 focus-visible:ring-orbital-cyan"
            >
              {sport} {pickCount}
            </Link>
          ))}
...
function StatPill({
  ...
  return (
    <div className="min-w-[108px] rounded-lg border border-mineral bg-eclipse px-3 py-2 text-left">
      <p className={`text-lg font-bold ${highlight ? "text-ion-magenta" : "text-white"}`}>
        {value}
      </p>
    ...
    >
      ...
      <div className="flex w-full shrink-0 flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
        {!hasAccount && (
          <Link
            href="/auth/signin"
            className="inline-flex min-h-11 items-center justify-center rounded-lg border border-mineral bg-titanium px-4 py-2 text-xs font-medium text-ink-200 transition-colors hover:bg-eclipse focus-visible:ring-2 focus-visible:ring-orbital-cyan"
          >
            Sign in
          </Link>
        )}
        <Link
          href="/pricing"
          className="inline-flex min-h-11 items-center justify-center rounded-lg bg-orbital-cyan px-4 py-2 text-xs font-semibold text-void shadow-[0_0_24px_rgba(0,229,255,0.25)] transition-colors hover:bg-[#5BEEFF] focus-visible:ring-2 focus-visible:ring-orbital-cyan"
        >
          See plans
        </Link>
      </div>
...
function PicksTrustStrip() {
  return (
    <section
      ...
      className="mb-6 rounded-xl border border-mineral bg-eclipse/50 p-4"
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        ...
        <Link
          href="/methodology"
          className="inline-flex min-h-11 shrink-0 items-center justify-center rounded-lg border border-orbital-cyan/30 bg-orbital-cyan/10 px-4 py-2 text-sm font-semibold text-orbital-cyan transition-colors hover:border-orbital-cyan hover:bg-orbital-cyan hover:text-void focus-visible:ring-2 focus-visible:ring-orbital-cyan"
        >
          Read methodology
        </Link>
      </div>
      <RiskDisclosure
        ...
        className="mt-4 border-mineral bg-void/50"
      />
    </section>
  );
}
...
function DatePickerForm({
  ...
  return (
    <form method="get" action="/picks" className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row sm:items-center">
      ...
      <label htmlFor="date" className="sr-only text-xs text-ink-400">Date</label>
      <input
        id="date"
        type="date"
        name="date"
        defaultValue={currentDate}
        className="min-h-11 rounded-lg border border-mineral bg-titanium px-3 py-1.5 text-sm text-ink-200 focus:border-orbital-cyan focus:outline-none focus:ring-1 focus:ring-orbital-cyan"
      />
      <button
        type="submit"
        className="min-h-11 rounded-lg bg-titanium px-3 py-1.5 text-sm font-medium text-ink-200 transition-colors hover:bg-eclipse hover:text-white focus-visible:ring-2 focus-visible:ring-orbital-cyan"
      >
        Apply date
      </button>
    </form>
