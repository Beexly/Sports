...
export default function PicksError({
  ...
  useEffect(() => {
    ...
    console.error("[picks] error boundary caught:", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-[40vh] items-center justify-center p-6 text-gray-200">
      <div className="max-w-md rounded-2xl border border-red-900 bg-red-950/30 p-6">
        <h2 className="text-lg font-bold text-white">Picks Board couldn't load.</h2>
        <p className="mt-2 text-sm text-red-200">
          A runtime error stopped today's picks from rendering. The underlying
          data pipeline is unaffected — retry to reload.
        </p>
        {error.digest && (
