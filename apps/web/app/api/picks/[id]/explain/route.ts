 ...
 * surfaced this pick, built ONLY from the pick's stored FactorBreakdown and its
 * immutable PickSignalSnapshot. The model never sees anything else, and the
 * output is policy-validated before it is returned (no betting advice, no
 * certainty, must cite its grounding).
 *
 * Gating (fails closed — the launch path is never affected):
 *  - 404 if PICK_EXPLAINER_ENABLED !== "true" (default OFF; surface is invisible)
 *  - 503 if canExposePublicPicks is off
 *  - 401 if not signed in; 403 unless PRO/ELITE (canSeeFactorBreakdown)
 *  - 503 if ANTHROPIC_API_KEY is not configured (inert without the key)
...
export async function POST(
  ...
  // Launch-path isolation: default-OFF feature flag. Even with a key present,
  // the explainer is invisible until this is explicitly turned on.
  if (process.env["PICK_EXPLAINER_ENABLED"] !== "true") {
