...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const spdx = requireSpdx({ licenseSpdx: body.licenseSpdx });
  if (!spdx.ok) {
    return NextResponse.json({ error: "licenseSpdx required", code: spdx.code }, { status: 422 });
  }
  const result = classifyExport({
    bulkRowCount: body.bulkRowCount ?? 0,
  ...
  return NextResponse.json(result, {
    status: result.class === "BLOCKED" ? 403 : 200,
