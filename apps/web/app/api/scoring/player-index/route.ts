/**
 * Player Galaxy Index — the central composite score every player tool reads
 * (single source of truth). Premium-gated. Defaults to the current season;
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const data = await loadPlayerCompositeScores(season);
