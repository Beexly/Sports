...
export default function TrackError({
  ...
  useEffect(() => {
    console.error("[track] error boundary:", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-obsidian px-6 py-16">
      <div className="w-full max-w-md rounded-2xl border border-alert/40 bg-alert/5 p-8 text-center">
        ...
        <h2 className="text-lg font-bold text-white">CLV Tracker failed to load</h2>
        <p className="mt-2 text-sm text-ink-300">Your bet log is stored locally — nothing is lost. Reload to reconnect.</p>
        {error.digest && (
