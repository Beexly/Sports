...
export default function BriefError({
  ...
  useEffect(() => {
    ...
    console.error("[brief] error boundary caught:", error);
  }, [error]);
  ...
  return (
    <div className="flex min-h-[40vh] items-center justify-center p-6 text-gray-200">
      <div className="max-w-md rounded-2xl border border-red-900 bg-red-950/30 p-6">
        <h2 className="text-lg font-bold text-white">Daily Brief couldn't load.</h2>
        <p className="mt-2 text-sm text-red-200">
          A runtime error stopped today's brief from rendering. Retry to reload,
          or check the Picks Board for the full slate.
        </p>
        {error.digest && (
