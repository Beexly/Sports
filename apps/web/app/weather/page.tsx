...
function windClass(mph: number | null): string {
  if (mph === null) return "text-ink-400";
      ...
      <main id="main-content" className="mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
        <section className="grid gap-8 border-b border-white/[0.08] pb-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-end">
          ...
          <div className="border border-white/[0.08] bg-white/[0.04] p-5">
            <div className="flex items-start justify-between gap-4">
              ...
              <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-ink-400">NWS · public domain</p>
            </div>
            <div className="mt-5 border border-white/[0.08] bg-white/[0.03] p-4">
              ...
              <p className="mt-2 text-sm leading-6 text-ink-300">{wx.note}</p>
            </div>
          </div>
        </section>
        ...
        {wx.status === "source-error" ? (
          <section className="border border-white/[0.08] bg-white/[0.04] p-5">
            ...
            <h2 className="mt-2 text-2xl font-semibold text-white">No conditions to show right now.</h2>
            <p className="mt-3 text-sm leading-6 text-ink-300">{wx.error ?? "UNKNOWN"}</p>
          </section>
        ) : (
          <section className="border border-white/[0.08] bg-white/[0.04]/80">
            <div className="border-b border-white/[0.08] px-5 py-4">
              ...
              <h2 className="mt-2 text-2xl font-semibold text-white">Current conditions</h2>
            </div>
                ...
                <tbody className="divide-y divide-mineral bg-white/[0.03]">
                  {wx.venues.map((v: VenueWeather) => (
                    <tr key={v.team + v.stadium}>
                      ...
                      <td className="px-4 py-3 text-white">{v.stadium}</td>
                      ...
                      <td className="px-4 py-3 text-ink-400">{v.status === "ok" ? v.shortForecast : "unavailable"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
