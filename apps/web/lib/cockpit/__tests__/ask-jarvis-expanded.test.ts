/**
 * Tests for the Ask Jarvis OS-layer expansion: every OS_LAYER intent is
 * registered, answers without throwing, and stays honest about what is
 * and is not wired (voice, external actions, memory).
 */
...
describe("Ask Jarvis — OS layer expansion (structural)", () => {
  it("registers all 17 OS intents in the group, order, and questions", () => {
    expect(OS_INTENTS.length).toBe(17);
