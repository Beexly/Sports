 ...
 *
 * Serializes ONLY the real, stored signals for a pick (its FactorBreakdown,
 * including the independent-edge rationale from #10, plus the immutable
 * PickSignalSnapshot) into a deterministic context block. Nothing else is ever
 * put in front of the model. The explanation agent and the loss-autopsy drafter
 * both build their prompt from this and nothing more — so the model can only
 * describe what the engine actually used, never invent a reason.
 *
...
export interface GroundingPick {
  ...
  // CLV (optional — only after settlement grading)
...
export function buildGroundedContext(input: GroundingInput): GroundedContext {
  ...
  if (fb) {
    ...
    // Independent-edge layer (#10) — surfaced, not yet priced.
    if (fb.independentEdge) {
      ...
      lines.push(`    rationale: ${ie.rationale}`);
    }
  } else {
