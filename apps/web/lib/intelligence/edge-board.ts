/**
 * Edge Board â€” the cross-dataset divergence scan, at scale.
 *
 ...
 * caught up â€” it is the entire product thesis, surfaced as one ranked list.
 *
 ...
 *   â€¢ player-model        â†’ GSE Rating (process) â‰« production  â†’ buy-low
 *                            production â‰« process               â†’ sell-high
 *   â€¢ expected-points     â†’ expected PPR â‰« actual               â†’ buy-low (xFP)
 *                            actual â‰« expected                  â†’ sell-high (xFP)
 *   â€¢ edge-signals (NGS)  â†’ tracking (separation/YAC+/-/air-share) â‰« box score
 *   â€¢ opportunity-transferâ†’ a ruled-OUT starter vacates volume   â†’ role inheritor
 *   â€¢ snap-share          â†’ an elevated workload share the box score lags
 *
 * Each edge carries: player, team, position, a typed label, a 0â€“100 MAGNITUDE
 ...
 * number. The board is sorted by magnitude â€” the strongest divergences on top.
 *
 * INTEGRITY (non-negotiable, inherited from every source loader):
 *   â€¢ No fabrication. Each edge is built only from a value a loader actually
 *     returned. A missing dataset contributes zero rows, never invented ones.
 *   â€¢ No forward projections. These are reads on settled data, exactly like the
 *     loaders they fuse; canPublishProjections stays false.
 *   â€¢ The graded-pool MODEL is untouched â€” we consume processGrade/signal as-is.
 *   â€¢ Honest empty + source-error states: if every source fails we say so; if
 ...
 * is exercisable offline with fixture loader-results â€” no network in the unit.
 */

import { loadPlayerModel, type PlayerModel } from "./player-model";
import {
  loadExpectedPoints,
  FF_OPPORTUNITY_ATTRIBUTION,
  type ExpectedPoints,
...
export type EdgeType =
  | "process-buy" // GSE Rating (process) â‰« production
  | "process-sell" // production â‰« GSE Rating (process)
  | "xfp-buy" // expected points â‰« actual production
  | "xfp-sell" // actual production â‰« expected points
  | "tracking-buy" // NGS tracking signal â‰« box score
  | "tracking-sell" // box score â‰« NGS tracking signal
...
/** The directional read an edge carries â€” drives tone + the buy/sell framing. */
...
export interface EdgeRow {
  ...
  /** Short human label for the SignalChip, e.g. "Buy-low Â· Rating". */
  readonly label: string;
  /**
   * 0â€“100 magnitude, normalized ACROSS edge types so heterogeneous gaps share one
  ...
  /** One-line, real-number driver. Never generic â€” always quotes the gap. */
  readonly reason: string;
  /** Which loader this edge came from â€” surfaced as a provenance tag. */
...
export interface EdgeBoard {
  ...
  /** Count of edges per type â€” drives the legend / summary chips. */
...
/** Inputs to the pure builder â€” the resolved results of each source loader. */
...
/** Human label for each edge type â€” used by the legend and the chip. */
export const EDGE_TYPE_LABEL: Record<EdgeType, string> = {
  "process-buy": "Buy-low Â· Rating",
  "process-sell": "Sell-high Â· Rating",
  "xfp-buy": "Buy-low Â· xFP",
  "xfp-sell": "Sell-high Â· xFP",
  "tracking-buy": "Buy-low Â· Tracking",
  "tracking-sell": "Sell-high Â· Tracking",
 ...
 *
 * Magnitude is normalized PER edge type to 0â€“100 against that type's expected
...
export function buildEdgeBoard(inputs: EdgeBoardInputs): {
  ...
  // â”€â”€ 1) player-model: GSE Rating (process) vs production â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // The loader already computed signal + the process/production percentiles; the
  // gap is processGrade âˆ’ productionPct. We rank by |gap| (the divergence size).
  if (playerModel.status === "live") {
    for (const p of playerModel.profiles) {
      if (p.signal === "in-line") continue;
      const gap = p.processGrade - p.productionPct; // +buy / âˆ’sell, in percentile pts
      ...
      edges.push({
        ...
        signedDomain: 60,
        reason: buy
          ? `GSE Rating ${p.processGrade} but production only ${p.productionPct}th pct â€” a ${Math.abs(gap)}-point process edge the box score hasn't paid yet.`
          : `Production ${p.productionPct}th pct on a ${p.processGrade} GSE Rating â€” ${Math.abs(gap)} points of output the underlying process doesn't support.`,
        source: "GSE Rating",
  ...
  // â”€â”€ 2) expected-points: expected PPR vs actual â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // The loader emits diff (actual âˆ’ expected) and a buy/sell signal. Expected â‰«
  // actual is a buy (positive edge for the player); actual â‰« expected is a sell.
  if (expectedPoints.status === "live") {
    for (const r of expectedPoints.rows) {
      ...
      const gapPct = r.xfpPct - r.prodPct; // +buy / âˆ’sell
      ...
      edges.push({
        ...
        signedDomain: 40,
        reason: buy
          ? `Earned ${round(r.xfpPerGame)} expected PPR/game but the box score paid less (${round(r.diff)} pts vs expected) â€” volume the market is under-pricing.`
          : `Outscored his expected points by ${round(r.diff)} â€” production running ahead of the opportunity that drives it.`,
  ...
  // â”€â”€ 3) edge-signals (NGS tracking) vs box score â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Already split into buyLow / sellHigh with a standardized z gap (underlying âˆ’
  // production). We map the z gap onto the shared 0â€“100 axis (z of ~3 = max).
    ...
    const pushTracking = (rows: readonly NflverseEdgeSignals["buyLow"][number][], buy: boolean): void => {
      for (const r of rows) {
        ...
        edges.push({
          ...
          signedDomain: 3,
          reason: buy
            ? `Tracking runs hot (${r.avgSeparation} yds separation, ${r.yacAboveExpectation} YAC+/-) but only ${r.pprPerGame} PPR/game â€” the underlying outpaces the box score.`
            : `${r.pprPerGame} PPR/game on cooler tracking (${r.avgSeparation} yds separation, ${r.yacAboveExpectation} YAC+/-) â€” production ahead of the underlying signal.`,
          source: "NGS Tracking",
          tone: toneFor(buy ? "buy" : "sell"),
        });
      }
  ...
  // â”€â”€ 4) opportunity-transfer: a ruled-OUT starter's vacated volume â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  if (opportunityTransfer.status === "live") {
    for (const r of opportunityTransfer.rows) {
      ...
      if (vacated <= 0 || !r.beneficiary) continue; // honest: no measured volume or no inheritor â†’ no edge
      ...
      edges.push({
        ...
        signedDomain: 20,
        reason: `${r.outPlayer} is OUT, vacating ${round(vacated)} targets+carries/game; ${r.beneficiary} is the next ${r.position} up on ${r.team} â€” opportunity the market hasn't repriced.`,
  ...
  // â”€â”€ 5) snap-share: an elevated every-down workload the box score lags â”€â”€â”€â”€â”€â”€â”€
  ...
  if (snapShare.status === "live") {
    ...
    for (const l of skillLeaders) {
      ...
      edges.push({
        ...
        signedDomain: 1,
        reason: `On ${pct}% of ${l.team}'s offensive snaps (${l.snapsPerGame}/game) â€” an every-down role that anchors floor and target volume.`,
        source: "Snap Share",
        tone: "neutral",
      });
    }
 ...
 * source-error when EVERY source failed â€” partial data still yields a real board.
 */
export async function loadEdgeBoard({
  ...
  return {
    ...
    note: allFailed
      ? "Every underlying source is unavailable right now, so the Edge Board is intentionally empty â€” we never fabricate divergences."
      : "Where our datasets disagree the loudest â€” ranked. Each edge fuses real, settled signals across the data we ingest; nothing here is a projection or a pick.",
    error: allFailed ? "all sources unavailable" : null,
  };
}

// â”€â”€ Synthetic source-error results (used only when a loader rejects outright) â”€â”€
// Each mirrors that loader's own source-error shape so the builder treats it as a
// dataset that simply contributed nothing â€” the same honest-empty path the
...
function expectedPointsError(e: unknown): ExpectedPoints {
  // NOTE (rescue 2026-09-25): record + attribution became required on the
  // ExpectedPoints interface after this module was written; values mirror the
  // loader's own source-error fallback in ./expected-points.
  return {
    generatedAt: new Date().toISOString(), status: "source-error", season: 0, throughWeek: null,
