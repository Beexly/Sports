/**
 * Play Design engine â€” the play-call DNA behind every snap (FTN charting, 2022+).
 *
 ...
 * FTN charting carries every one of these as a real per-play flag â€” but the FTN
 ...
 * (`nflverse_game_id` â†” pbp `game_id`, `nflverse_play_id` â†” pbp `play_id`), then
 * aggregates:
 *
 *   â€¢ per QB  â€” playActionRate, rpoRate, screenRate, motionRate, noHuddleRate,
 *               avgBlitzersFaced (mean n_blitzers on that QB's charted plays)
 *   â€¢ per team â€” the same six, over every charted play the team ran on offense
 *
 ...
 *
 * SCOPE â€” regular season only. FTN charting covers the postseason too, but the
 ...
 * a status / source-error path) â€” no functions cross the RSC boundary. The CLIENT
 * (engine-view.tsx) owns all render()/column functions. Pattern mirrors
 * team-environment.ts. canPublishProjections false â€” this is design context, not a
...
/**
 * Play identity â†’ offense/QB attribution, harvested from play-by-play. Keyed by
 ...
 *   scope guard: season_type (REG/POST) â€” only REG identities are indexed so
 ...
 *
 * SCOPE â€” regular season only: FTN charting covers the postseason too, and a
...
export function buildPbpIdentity(records: readonly CsvRecord[]): Map<string, PbpPlayIdentity> {
  ...
  for (const r of records) {
    if (r["season_type"] !== "REG") continue; // drop POST (and any non-REG) â€” scope is regular season
 ...
 * no offense team) is skipped â€” never guessed. QB attribution requires a non-empty
...
export function buildPlayDesign(
  ...
  for (const ftn of ftnRows) {
    ...
    if (!id) continue; // unjoinable charted play â€” drop, never invent identity
    joined += 1;
  ...
  // Most play-action-reliant QB first â€” the headline of the play-design read.
...
export async function loadPlayDesign({
  ...
  // FTN is the gating source â€” without it there are no design flags to attribute.
  ...
  // we cannot honestly attribute anything â€” empty state rather than orphan rows.
  ...
  return {
    ...
    teams,
    canPublishProjections: false,
    note: "The play-call DNA behind every snap â€” how each QB and offense is actually designed to attack, across the regular season. Design context, not a projection or a pick.",
    sourceUrl: ftn.sourceUrl,
    error: null,
  };
}
