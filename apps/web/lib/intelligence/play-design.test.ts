...
describe("buildPbpIdentity â€” regular-season scope", () => {
...
describe("buildPlayDesign â€” POST charted plays drop, REG-only rates", () => {
  ...
  const ftnRows: FtnChartingRow[] = [
    ...
    ftn({ gameId: "G_POST", playId: "9", isPlayAction: true }), // playoff snap â€” must drop
  ];
  ...
  it("computes per-QB rates over the regular-season denominator only", () => {
    ...
    const qb = qbs[0]!;
    expect(qb.plays).toBe(2); // not 3 â€” POST excluded
