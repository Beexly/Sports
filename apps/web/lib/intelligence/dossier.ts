/**
 * Player Dossier â€” the full-spectrum synthesis for ONE player.
 *
 ...
 * Sleeper market) â€” but nothing JOINED it for a single player. Every surface was
 ...
 * model (loadPlayerModel) â€” we never recompute the grade; the number and its
 * buy/sell signal are owned by player-model.ts. ratingWhy() narrates it. Then we
 * hang each domain section off that identity.
 *
 * JOIN KEYS â€” honest about the two id namespaces in the stack:
 *   â€¢ GSIS id (the player model's playerId): Next Gen Stats, scoring-zone,
 *     injury report all key on the same gsis id, so those join directly.
 *   â€¢ PFR id / name-only sources: PFR pressure-coverage + snap-share are keyed by
 ...
 * state), not a zero or an invented value. canPublishProjections stays false â€”
...
// â”€â”€ Serializable section shapes â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/** The GSE Rating headline â€” the spine. Always present when the player resolves. */
...
export interface DossierEfficiency {
  ...
  readonly avgSeparation: number | null; // receivers â€” yards of separation
  readonly avgYacAboveExpectation: number | null; // receivers â€” YAC over expected
  readonly catchPct: number | null; // receivers
  readonly cpoe: number | null; // QBs â€” completion % over expected
  readonly avgTimeToThrow: number | null; // QBs â€” seconds
  readonly ryoePerAtt: number | null; // RBs â€” rush yards over expected / attempt
  readonly pctStackedBox: number | null; // RBs â€” % vs 8+ box
  // PFR receiver charting (name+team join):
  readonly adot: number | null; // average depth of target
  readonly dropPct: number | null; // 0..1
}

/** Role / scheme â€” how the offense uses this player (QB-centric, via play-design). */
export interface DossierRole {
  ...
  // QB pressure environment (name+team join â†’ PFR pass charting):
...
/** Situational â€” scoring-zone (inside the 20) equity, the TD-driver context. */
...
/** Athletic prior â€” the combine measurements (name+position join; no id in source). */
...
/** Availability â€” the official injury designation (gsis id join). */
...
/** Market â€” live Sleeper crowd sentiment (name+team join). */
...
/** A nullable section paired with WHY it is null â€” so the page can distinguish
...
// â”€â”€ Join helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
/** Wrap a loader result as a section: a thrown/rejected load â†’ source-error,
 *  a resolved-but-empty match â†’ missing, a real row â†’ ok. */
...
// They NEVER fabricate: an unreachable source â†’ source-error; a reached source
// with no matching row â†’ missing; a real match â†’ ok.
...
function buildUsage(
  ...
  // Snap share lives on PFR snap counts (pfr id) â†’ name+team join.
...
/**
 * loadPlayerDossier â€” assemble the full dossier for one player id.
 *
 ...
 * in parallel (allSettled â€” one slow/broken source can't take the page down) and
...
export async function loadPlayerDossier(
  ...
  if (profile == null) {
    return {
      ...
      canPublishProjections: false,
      note: "This player isn't in the current graded pool â€” we only carry a GSE Rating for qualified skill players this season.",
      error: null,
    };
  }
  ...
  return {
    ...
    canPublishProjections: false,
    note: "Every signal we hold on this player, joined onto the GSE Rating. A synthesis of what has happened â€” not a projection or a pick.",
    error: null,
  };
}
