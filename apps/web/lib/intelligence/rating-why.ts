/**
 * Rating "why" â€” the human read behind the GSE Rating.
 *
 ...
 *
 * Hard constraints:
 *   â€¢ Pure mapping of EXISTING fields â†’ language. We do NOT recompute the grade,
 *     re-rank, or publish any forward projection. The number is owned by
 *     player-model.ts; this only narrates it.
 *   â€¢ NO fabricated data. If a driver field is null we omit that clause â€” we
 *     never invent a target share, an EPA, a WOPR. Honest gaps over filler.
 *   â€¢ Server-safe AND client-safe: zero node/Next imports so it can be called
 *     from a loader, a server page, or a client render fn alike.
 *
 * Branches are organized by position Ã— signal (buy-low / sell-high / in-line),
 ...
 *  these fields are in hand â€” and makes the "omit when null" contract explicit. */
...
function whyReceiver(p: RatingWhyInput): string {
  ...
  if (p.signal === "buy-low") {
    return usage
      ? `Elite usage â€” ${usage} â€” that the box score hasn't paid out yet; the role says the production is coming.`
      : "The process is outrunning the production here â€” a buy-low before the points catch up.";
  }
  if (p.signal === "sell-high") {
    return usage
      ? `The points are running ahead of the role (${usage}); sell into the production before it regresses.`
      : "Production has outrun the underlying role â€” sell-high while the value is rich.";
  }
  // in-line
  return usage
    ? `Role and production line up â€” ${usage} earning a ${tierWord(p.processGrade)} grade, priced about right.`
    : `Role and production are in step at a ${tierWord(p.processGrade)} grade â€” fairly priced.`;
}
...
function whyRunningBack(p: RatingWhyInput): string {
  ...
  if (p.signal === "buy-low") {
    return `${volume.charAt(0).toUpperCase()}${volume.slice(1)} anchoring the role â€” the usage is there ahead of the production; buy before it pays out.`;
  }
  if (p.signal === "sell-high") {
    return efficient
      ? `Production is ahead of the workload on hot efficiency â€” sell-high before the touchdowns and the EPA come back to earth.`
      : `The box score is outrunning the workload behind it (${volume}); sell into the production.`;
  }
  return `${volume.charAt(0).toUpperCase()}${volume.slice(1)} and the production agree â€” a ${tierWord(p.processGrade)} grade that's fairly priced.`;
}
...
function whyQuarterback(p: RatingWhyInput): string {
  ...
  if (p.signal === "buy-low") {
    return efficient
      ? "Efficient, high-EPA process the fantasy points haven't rewarded yet â€” the underlying play says it's coming."
      : "The process grade is ahead of the production â€” a buy-low if the efficiency holds.";
  }
  if (p.signal === "sell-high") {
    return "Fantasy production is outrunning the efficiency under it â€” sell-high before the play regresses to the process.";
  }
  if (inefficient) {
    return `Process and production agree on a ${tierWord(p.processGrade)} grade â€” the efficiency is the ceiling here.`;
  }
  return `Efficiency and production are in step at a ${tierWord(p.processGrade)} grade â€” priced about right.`;
}

/**
 * ratingWhy â€” the public entry point. Given a player profile (or the relevant
 ...
 * driver field is populated). Branches are position Ã— signal.
 */
...
/** Signal-only read for the (typed-impossible) default case â€” keeps the function
...
export function signalFallback(signal: ProcessSignal): string {
  switch (signal) {
    case "buy-low":
      return "The process is ahead of the production â€” a buy-low before the market reprices it.";
    case "sell-high":
      return "Production has outrun the process behind it â€” sell-high into the value.";
    default:
      return "Process and production agree â€” fairly priced.";
  }
}
