...
export type ConsensusRow = {
  ...
  /** The ranking score: points ARE weighted (accuracy-weighted or equal, per `mode`). */
  readonly points: number;
  readonly rank: number; // 1-indexed position within this consensus board
  // best/worst/sourcesCounted/avgRank are RAW TRANSPARENCY stats — "what did the
  // sources actually say" — deliberately unweighted so a caller can see the raw
  // spread even when `points` down-weights a source's opinion to near zero. They
  // count every source that ranked this player, including ones weighted to 0.
  ...
  /** Plain (unweighted) average rank across every source that ranked this player. */
...
function buildBoard(pos: Pos, sources: readonly RankSource[], weights: ReadonlyMap<string, number>, mode: ConsensusBoard["mode"]): ConsensusBoard {
  ...
  const rows: ConsensusRow[] = [...points.entries()]
    .map(([playerId, pts]) => {
      ...
      const plainAvgRank = ranks.reduce((s, r) => s + r, 0) / ranks.length;
      return {
        playerId,
        points: round1(pts),
