 ...
 * them head-to-head against the incumbent heuristic and a LineStar-style
 ...
 *     lineup. LineStar's randomized-column heuristic can only approximate this.
...
export type Benchmark = {
  readonly cash: {
    ...
    readonly heuristicBest: number; // best Σproj a random-restart heuristic found
    readonly heuristicWorst: number; // worst of the sampled runs — the risk it leaves on the table
    readonly optimalityGap: number; // exact − heuristicWorst (points the heuristic can miss)
  };
  readonly gpp: {
...
/**
 * Run the comparison. Deterministic for the exact + simulated parts; the
 * heuristic is sampled over several random restarts to expose its variance.
 */
export function benchmark(slate: readonly DfsPlayer[] = activeDfsSlate(), seed = 7): Benchmark {
  // ── CASH: provable optimum vs random-restart heuristic ──
  const exactCash = optimizeExact({ mode: "cash", locks: new Set(), excludes: new Set() }, slate);
  const heurObjs: number[] = [];
  // Low restart counts on purpose: shows the guarantee the heuristic can't make.
  for (const restarts of [3, 3, 5, 5, 8]) {
    const lu = optimizeOne({ mode: "cash", stack: false, locks: new Set(), excludes: new Set() }, undefined, restarts, slate);
    if (lu) heurObjs.push(objOf(lu, "cash"));
  }
  const heuristicBest = heurObjs.length ? Math.max(...heurObjs) : 0;
  const heuristicWorst = heurObjs.length ? Math.min(...heurObjs) : 0;
  ...
  return {
    cash: {
      ...
      heuristicBest: round1(heuristicBest),
      heuristicWorst: round1(heuristicWorst),
      optimalityGap: round1(exactCash.objective - heuristicWorst),
    },
    gpp: {
