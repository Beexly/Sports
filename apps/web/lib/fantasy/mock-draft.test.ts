...
describe("userPick", () => {
  ...
  it("marks draft finished after all picks", () => {
    ...
    expect(s1.endedEarly).toBe(false);
  });

  it("ends honestly instead of stalling when the pool runs out before every round fills", () => {
    // The smallest Setup-screen config (8 teams x 10 rounds = 80 picks) exceeds
    // the 30-player illustrative demo pool — this reproduces that exact shape
    // (8 teams x 4 rounds = 32 picks against a 30-player pool) to prove the
    // draft terminates honestly rather than looping forever with
    // finished: false once the pool is exhausted.
    const cfg: MockDraftConfig = { teams: 8, rounds: 4, userSlot: 1 };
    const smallPool = PLAYERS.slice(0, 30);
    let state = advanceAI(initMockDraft(cfg, smallPool), smallPool);
    let guard = 0;
    while (!state.finished && guard < 40) {
      const nextAvailable = [...state.available][0];
      if (!nextAvailable) break;
      state = userPick(state, nextAvailable, smallPool);
      guard += 1;
    }
    expect(state.finished).toBe(true);
    expect(state.endedEarly).toBe(true);
    expect(state.available.size).toBe(0);
    // 30 players available, 32 picks configured — the draft must stop at 30,
    // never claim to have completed all 32.
    expect(state.picks.length).toBe(30);
  });
});
