...
export type MockDraftState = {
  ...
  /**
   * True when the draft finished because the player pool ran out before every
   * round could be filled (config asks for more picks than the injected pool
   * has players) rather than by completing all configured rounds. The
   * illustrative demo pool is small enough that this is reachable with the
   * Setup screen's smaller team/round combinations — an honest end-state, not
   * a stuck board, beats silently hanging on `finished: false` forever.
   */
...
function recordPick(state: MockDraftState, playerId: string): MockDraftState {
  ...
  // The pool can run out before every round fills (small illustrative demo
  // pool + a full-size config) — end honestly there too, not just at the
  // configured round count.
  const finished = newNextOverall > totalPicks(config) || newAvailable.size === 0;
  const endedEarly = finished && newNextOverall <= totalPicks(config);
...
export function advanceAI(state: MockDraftState, pool: readonly Player[]): MockDraftState {
  ...
  while (!s.finished && !isUserPick(s.nextOverall, s.config)) {
    if (s.available.size === 0) return { ...s, finished: true, endedEarly: true };
    const id = aiPickId(s.available, pool, s.nextOverall);
    if (!id) return { ...s, finished: true, endedEarly: true };
    s = recordPick(s, id);
  }
  // It's the user's turn (or the draft just finished), but nothing is left to
  // draft — end honestly instead of presenting an empty board as "your pick!".
  if (!s.finished && s.available.size === 0) return { ...s, finished: true, endedEarly: true };
  return s;
}
