...
export const GSE_SCORING_SYSTEMS: readonly ScoringSystemSpec[] = [
  ...
  // ── Sprint 2 additions: research/autonomy/self-learning scores ─────────────
  {
    id: "drift_risk",
    name: "Drift Risk Score",
    purpose: "Detect distribution shift in data/features before it corrupts outputs.",
    inputs: ["population stability index (PSI)", "expected vs actual distribution"],
    orientation: "higher_is_riskier",
    outputScale: "0–100",
    surface: "internal",
    misuseRisk: "Trusting model outputs through significant drift; PSI bands are advisory, not absolute.",
    v1: "scoreDriftRisk() in self-learning.ts over a PSI value (credit-risk transfer).",
    fn: "scoreDriftRisk",
  },
  {
    id: "model_promotion",
    name: "Model Promotion Readiness Score",
    purpose: "Whether a challenger model may replace the champion.",
    inputs: ["settled sample", "challenger vs champion Brier", "shadow period", "input drift"],
    orientation: "higher_is_better",
    outputScale: "0–100",
    surface: "internal",
    misuseRisk: "Promoting on a small sample or through a calibration regression — both are hard gates.",
    v1: "scoreModelPromotionReadiness() in self-learning.ts (champion/challenger gate).",
    fn: "scoreModelPromotionReadiness",
  },
  {
    id: "active_learning",
    name: "Active-Learning Priority Score",
    purpose: "Where to spend the next unit of attention/labeling for maximum learning.",
    inputs: ["predictive uncertainty", "decision impact", "data staleness", "coverage gap"],
    orientation: "higher_is_better",
    outputScale: "0–100",
    surface: "internal",
    misuseRisk: "Chasing uncertainty alone — impact and coverage decide where learning actually helps.",
    v1: "scoreActiveLearningPriority() in self-learning.ts.",
    fn: "scoreActiveLearningPriority",
  },
  {
    id: "adoption_value",
    name: "External Adoption Value Score",
    purpose: "How worth adopting an open-source repo / dataset / tool is.",
    inputs: ["value to GSE", "license class", "commercial-use verdict", "integration cost"],
    orientation: "higher_is_better",
    outputScale: "0–100",
    surface: "internal",
    misuseRisk: "Adopting a non-commercial or unverified-rights resource; license is a hard gate.",
    v1: "scoreAdoptionValue() in open-source-ledger.ts.",
    fn: "scoreAdoptionValue",
  },
  {
    id: "feature_gap",
    name: "Competitor Feature-Gap Score",
    purpose: "Rank competitor features by build opportunity (copy / build-better / ignore).",
    inputs: ["value to GSE", "copyability", "GSE status (have/partial/gap)", "trust impact"],
    orientation: "higher_is_better",
    outputScale: "0–100",
    surface: "internal",
    misuseRisk: "Copying a trust-eroding mechanic (e.g. fake confidence UX); trust impact is a hard gate.",
    v1: "scoreFeatureGap() in competitor-intelligence.ts.",
    fn: "scoreFeatureGap",
  },
] as const;
