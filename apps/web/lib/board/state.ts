...
export interface BoardStatePayload {
  ...
  meta: {
    ...
    dataStatus: "live" | "sample" | "degraded";
...
function degradedBoardState(now: Date): BoardStatePayload {
  return {
    data: {
      sportsWatched: 0,
      booksPolled: 0,
...
export async function loadBoardState(now = new Date()): Promise<BoardStatePayload> {
  ...
  if (demoActive) {
    ...
    return {
      ...
      meta: { isSampleData: true, dataStatus: "sample" },
    };
  }
  ...
  try {
    ...
    if (decisions.length > 0) {
      ...
      return {
        ...
        meta: { isSampleData: false, dataStatus: "live" },
      };
    }
    ...
    console.warn(
      "[board/state] live board data unavailable; returning degraded public payload.",
      err instanceof Error ? err.message : "unknown error"
    );
    captureError(err, { surface: "board:state", degraded: "board_data_unavailable" });
    return degradedBoardState(now);
  }
}
