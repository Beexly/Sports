...
export function loadKingScorecard() { return readJson<{generated_at:string; overall_score:number; dimensions:Record<string, number>}>("data/statking/crown/king_standard_scorecard.json", {generated_at:"", overall_score:0, dimensions:{}}); }
...
export function loadDepthChart() { return readJson<{depth:Array<{team:string; position:string; player_id:string; role:string; source_lineage:string[]}>}>("data/statking/snapshots/depth_chart_sample.json", {depth:[]}).depth; }
export function playerNameMap(): Map<string,string> { return new Map(loadPlayers().map((p)=>[p.player_id, p.name])); }
