...
const COPY: Record<ExplainRegister, AcademyCopy> = {
  teach: {
    ...
    beatTheCloseBody:
      "A line (the price, expressed as a spread) opens. Market intel lands tick by tick — each piece can move the number. Take the current number any time, or pass. Your score is pure CLV: did the number you took beat where the market finally closed? There are no win/loss results here — only price.",
  },
