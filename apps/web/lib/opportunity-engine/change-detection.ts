/**
 * Deterministic snapshot-to-snapshot change detection.
 *
 * S3 split unit of the frozen #146 reference branch (fbc3cfe), extracted
 * verbatim per the NOVA convergence freeze (§4 S3). Pure and side-effect
 * free: given two observation snapshots it derives NEW / UPDATED / REMOVED /
 * UNCHANGED material changes with graded materiality. It performs no network
 * IO and never promotes an observation on its own — promotion authority lives
 * with the failed-closed runtime receipts (`scripts/nova/source-runtime-core.mjs`).
 */
