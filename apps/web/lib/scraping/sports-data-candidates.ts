...
export const SPORTS_DATA_CANDIDATES: readonly SportsDataCandidate[] = [
  ...
  // ── Free-data census additions (2026-07-02, handoff/claude/overnight-2026-07-01/FREE-DATA-CENSUS.md) ──
  {
    id: "mlb-statsapi",
    name: "MLB Stats API (statsapi.mlb.com)",
    homepage: "https://statsapi.mlb.com",
    coverage: "Official MLB feed: schedules, PROBABLE PITCHERS, confirmed lineups, live scores, rosters, player/team stats, venue metadata. The sharpest free MLB signal for totals/moneyline is probables + lineup confirmation.",
    accessModel: "free_no_key",
    freeTier: "Public JSON endpoints, no key. Rate limits undocumented; verify politely and cache.",
    keyRequired: false,
    apiKeyEnvVar: null,
    priority: "high",
    oddsOnly: false,
    ...GATE,
    inMainRegistry: true,
    registrySourceId: "mlb-stats-api",
    verificationSteps: [
      "Read MLBAM's current terms; the defensible pattern is FACTS AS MODEL INPUTS (probables, lineups, scores), never re-serving the feed itself.",
      "Verify live endpoint schemas (schedule?sportId=1, /people, /game feeds) before building the adapter.",
      "On clearance: registry entry with the facts-only note, then wire probables/lineups into MLB game enrichment as shadow evidence first.",
    ],
    notes: "Census priority #2 (after NWS weather, already cleared in the registry). Facts are not copyrightable (Feist); the terms read decides HOW we may fetch them. H-S probe 2026-08-29: 2/2 OK (schedule 168ms, teams 38ms). Registry entry added as vendor_candidate pending terms review.",
  },
  {
    id: "baseball-savant",
    name: "Baseball Savant / Statcast CSV",
    homepage: "https://baseballsavant.mlb.com",
    coverage: "Pitch-level Statcast: barrel rate, xwOBA, exit velocity, sprint speed, arm strength. The public sabermetric backbone for MLB props and totals context.",
    accessModel: "free_no_key",
    freeTier: "Public CSV export endpoints (the ones the MIT-licensed pybaseball library documents). No key.",
    keyRequired: false,
    apiKeyEnvVar: null,
    priority: "high",
    oddsOnly: false,
    ...
    registrySourceId: null,
    verificationSteps: [
      "Confirm the CSV endpoints' terms (MLBAM property; same facts-as-inputs pattern as mlb-statsapi).",
      "Prefer season-aggregate pulls over per-pitch firehose; verify schema live.",
      "On clearance: registry entry, then feed barrel%/xwOBA into player-lab style MLB context (shadow first).",
    ],
    notes: "Already identified in the intelligence-program mandate (Statcast barrel%/K%). This is the formal intake entry.",
  },
  {
    id: "nhl-api-web",
    name: "NHL API (api-web.nhle.com)",
    homepage: "https://api-web.nhle.com",
    coverage: "Official NHL: schedules, rosters, live boxscores, standings, shift data. Complements MoneyPuck (already cleared) with same-day facts.",
    accessModel: "free_no_key",
    freeTier: "Public JSON, no key. Community-documented; limits unpublished.",
    keyRequired: false,
    apiKeyEnvVar: null,
    priority: "medium",
    oddsOnly: false,
    ...
    registrySourceId: null,
    verificationSteps: STANDARD_STEPS,
    notes: "Wire before NHL season. Facts-as-inputs pattern; the hockey analytics community has used these endpoints openly for years.",
  },
  {
    id: "chadwick-register",
    name: "Chadwick Bureau Register",
    homepage: "https://github.com/chadwickbureau/register",
    coverage: "The player-ID crosswalk (MLBAM, Retrosheet, FanGraphs, Baseball-Reference ids). Pure plumbing: required the moment two MLB sources need joining.",
    accessModel: "free_no_key",
    freeTier: "Open data on GitHub; bulk CSV.",
    keyRequired: false,
    apiKeyEnvVar: null,
    priority: "medium",
    oddsOnly: false,
    ...
    registrySourceId: null,
    verificationSteps: [
      "Confirm the repo's current license file (historically open).",
      "On clearance: registry entry; used internally for joins only, never displayed.",
    ],
    notes: "Zero user-facing surface; unlocks mlb-statsapi + baseball-savant + retrosheet joins.",
  },
  {
    id: "sportsdataverse-archives",
    name: "sportsdataverse data releases (hoopR/cfbfastR/wehoop)",
    homepage: "https://sportsdataverse.org",
    coverage: "College basketball / football / WNBA play-by-play and box archives published as parquet/RDS releases. The biggest free unlock for NCAAB and CFB seasons.",
    accessModel: "free_no_key",
    freeTier: "GitHub release assets; MIT-licensed code, per-dataset provenance varies.",
    keyRequired: false,
    apiKeyEnvVar: null,
    priority: "high",
    oddsOnly: false,
    ...
    registrySourceId: null,
    verificationSteps: [
      "Per-dataset provenance check: the CODE is MIT, but each data release mirrors an upstream feed whose terms govern (same lesson as espn-hidden-api).",
      "Verify parquet schemas live; pick the minimal datasets (team box, schedules, efficiency) before PBP volume.",
      "On clearance: registry entries PER DATASET, not one blanket entry.",
    ],
    notes: "Do this before the college seasons. The nflverse pattern, applied to college.",
  },
  {
    id: "thesportsdb",
    name: "TheSportsDB",
    homepage: "https://www.thesportsdb.com",
    coverage: "Multi-sport metadata: teams, schedules, venues, artwork/logos. Presentation-layer enrichment more than signal.",
    accessModel: "free_key",
    freeTier: "Free/dev key tier (Patreon supporters get higher limits); verify current numbers live.",
    keyRequired: true,
    apiKeyEnvVar: "THESPORTSDB_API_KEY",
    priority: "low",
    oddsOnly: false,
    ...GATE,
    inMainRegistry: true,
    registrySourceId: "thesportsdb",
    verificationSteps: STANDARD_STEPS,
    notes: "Community-maintained DB; useful for logos/venue metadata where licensing of official marks must ALSO be considered (team logos are trademarks; display context matters). H-S probe 2026-08-29: 2/2 OK (team search 248ms, NFL season events 170ms). Registry entry added as vendor_candidate pending terms clearance.",
  },
  {
    id: "football-data-org",
    name: "football-data.org",
    homepage: "https://www.football-data.org",
    coverage: "Soccer competitions: fixtures, results, standings, scorers for major leagues.",
    accessModel: "free_key",
    freeTier: "Free tier with modest per-minute limits and a restricted competition set; verify current terms for commercial display.",
    keyRequired: true,
    apiKeyEnvVar: "FOOTBALL_DATA_ORG_KEY",
    priority: "low",
    oddsOnly: false,
    ...GATE,
    inMainRegistry: true,
    registrySourceId: "football-data-org",
    verificationSteps: STANDARD_STEPS,
    notes: "MLS is already covered by openfootball (cleared); this adds European leagues if/when GSE expands soccer. H-S probe 2026-08-29: 1/2 OK (competitions list 200 in 308ms, PL matches 403 without key). Free tier requires API key registration (10 req/min). Soccer only. NOT an odds source.",
  },
];
