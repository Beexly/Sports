 ...
 *
 * When DEV_FAKE_ADMIN=true AND NODE_ENV !== "production", every auth() call
 * returns a synthetic ADMIN session. This is the launch-night escape hatch —
 * it lets the operator open /dashboard and /cockpit without OAuth or a real
 * Postgres session table. The NODE_ENV guard prevents accidental activation
 * in production even if the env var is mistakenly set.
 */
