...
export function detectStaleSource(input: SourceFreshnessInput, thresholdHours = 4): SourceFreshnessStatus {
  ...
  // Unparsable timestamps yield NaN — never present that as FRESH (a fake-green).
  if (!Number.isFinite(ageHours)) return { sourceId: input.sourceId, status: "UNKNOWN", ageHours: null, critical: input.critical === true };
