/**
 * Performance segments â€” units, win%, ROI and CLV broken down by sport and by market.
 *
 ...
 * lean into the sportÃ—market segments that print â€” and show the receipts per segment.
 *
 * Built on the tested `portfolio()` engine (no duplicate math). Pure + deterministic.
 *
 * Honesty guardrail: `edgeBoard()` will not crown a tiny-sample segment â€” a 3-0 run is
...
export type Segment = {
  ...
  readonly key: string;
  /** human label, e.g. "NBA" or "NBA Â· Total" */
...
/** Per MARKET (Spread / Total / Moneyline / propsâ€¦) across all sports. */
export function byMarket(bets: readonly Bet[]): Segment[] {
  return segmentBy(bets, (b) => b.market, (k) => k);
}

/** Per SPORT Ã— MARKET â€” the finest cut, where soft markets (props) reveal themselves. */
export function bySportMarket(bets: readonly Bet[]): Segment[] {
  return segmentBy(
    bets,
    (b) => `${b.sport}|${b.market}`,
    (k) => `${titleCase(k.split("|")[0]!)} Â· ${k.split("|")[1]}`,
  );
}

export type EdgeBoard = {
  /** segments with a real sample (>= minSettled), ranked by ROI desc â€” the honest leaders. */
  readonly leaders: readonly Segment[];
  /** promising but under-sampled â€” watch, don't bank on. */
  readonly lowSample: readonly Segment[];
  readonly minSettled: number;
};

/**
 * Rank sportÃ—market segments by ROI, but only those with enough settled bets to mean
