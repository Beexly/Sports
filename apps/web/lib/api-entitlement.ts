...
const FANTASY_MESSAGE = "This fantasy tool requires a Fantasy, Pro, or Elite subscription.";
 ...
 * {@link requirePremiumApiRateLimited} exactly; only the predicate and message
 * differ.
 *
 * @param bucketId stable per-endpoint limiter name (e.g. "tools/lineup")
 */
export async function requireFantasyApiRateLimited(
  bucketId: string
): Promise<NextResponse | null> {
  const gate = await evaluateGate((e) => e.canUseFantasyFull, FANTASY_MESSAGE);
  if (gate.denied) return gate.denied;
  if (!gate.userId) return null;
