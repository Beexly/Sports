...
const ARTIFACT_TO_CHECK_ID: Readonly<Record<string, string>> = {
  ...
  "/api/board/state?check=book-depth": "CHECK-E2",
  "/api/board/state?check=edge-index": "CHECK-E3",
  "/api/calibration": "CHECK-A5",
  "/api/picks?check=public-picks-gate": "CHECK-T1",
  "/api/performance?check=performance-gate": "CHECK-T2",
  "/api/cockpit/readiness?check=public-blog-gate": "CHECK-T3",
  "/api/cockpit/bot-outbox/preview?surface=twitter": "CHECK-B1",
  "/api/cockpit/bot-outbox/preview?surface=discord": "CHECK-B2",
  "/journal/rss.xml": "CHECK-B3",
  "local://build-size-budget": "CHECK-C1",
};
