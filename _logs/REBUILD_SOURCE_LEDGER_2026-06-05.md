...
Agent audit found the core convolution: nav has **41 desktop / 37 mobile links**, **three** competing optimizers (`/fantasy/dfs`, `/fantasy/lineup`, `/players/dfs`), the player pool is buried in a cramped 44vh sidebar with 9px text, and FantasyShell ceremony pushes tools below the fold. Recommended fix: ONE canonical `/optimizer` (contest switch), **pool-as-hero** sortable table, sticky salary-cap + lineup rail, progressive-disclosure advanced settings, and a **5-item top nav** (Board · Optimizer · Players ▾ · Fantasy ▾ · Intelligence ▾) with the meta-pages (`/fantasy` gates list, `/fantasy/baseline`) demoted. Quick wins: add `/optimizer` entry, flip pool to wide column + bump text, add salary meter, promote DK CSV import, tab the pool (All/Locked/Faded), un-truncate exposure, group mobile nav.

## Optimizer/nav consolidation — WAVE 1 shipped (Claude, 2026-06-06)
First redesign wave (the "one door / stop being convoluted" fix):
- Nav restructured from 4 primary + two 16-20-item dropdowns into **5 groups**: Board · Optimizer · Players ▾ · Fantasy ▾ · Intelligence ▾ (+ Methodology/Pricing tail). Players split out of the old mega-Intelligence menu; dropdowns now 2-column for scannability. Every route still reachable (no destinations dropped).
- New canonical **`/optimizer` hub** — one door to DFS Optimizer, Start/Sit, Draft Assistant, DFS Salaries, with "what feeds these tools" + the gated-data honesty note.
- Mobile nav regrouped from a flat 37-link wall into 5 labeled sections (Start here / Players / Fantasy / Intelligence / Account), scrollable; CSS added (`.mobile-nav-section`/`.mobile-nav-heading`).
...
- **NHL goalies (`7c4abc9`):** extended MoneyPuck lib with goalies.csv → GSAx (xGA − GA, min 15 GP) on `/nhl`. Live: Logan Thompson +29.3.
- **Combine (`/players/combine`):** nflverse `combine.csv` (8,968 rows) → latest-class fastest-40 table + all-time fastest-40 board (40/vert/broad/cone/shuttle/bench). On-theme with Human-Performance "athletic traits" priors. Live: Xavier Worthy 4.21 all-time (2024), Maxwell Hairston 4.28 (2025). Tests (3). canPublishProjections=false.
- **Total QBR (`/players/qbr`):** nflverse `espn_data/qbr_week_level.csv` (10,709 rows) → play-weighted season QBR leaders, min 6 g, an independent QB estimate triangulating CPOE+pressure. Live: 2025 leader Drake Maye 71.8 (matches his +9.1% CPOE — lenses agree). Tests (3). canPublishProjections=false.
- Aborted this session (recon friction, queued): MLB via Lahman/baseballdatabank (GitHub path 404'd + api.github.com blocked in this env — need correct raw path/branch); CFBD (needs a free API key → gated); nflverse depth_charts (2025 file is 554k rows — heavy; build with a size-guard later).
