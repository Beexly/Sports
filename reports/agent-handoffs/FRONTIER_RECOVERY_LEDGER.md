...
| P0-7 public fantasy fixtures | P0 | `FIX_VALIDATED` | Every public `/fantasy/*` tool and `/optimizer` now redirects at middleware before illustrative pools can render or serialize; the internal Studio retains its ADMIN boundary. The hub and OpenGraph image expose a four-part data-clearance contract instead of selling sample projections. Twenty-seven focused assertions and desktop/mobile browser receipts pass. |
...
| 80 | `claude/hotfix-slate-freeze-frontrun` -> `main` | 3 / 1 commit | Closed with #112 supersession receipt | The 07:00 settlement freeze could seal today's partial population before the 10:00 mint; current external and Vercel schedules preserve that race | `EXTRACTED_REBUILT_CLOSED`; pre-mint deferral with early-kickoff exception | 13 freeze tests + pipeline typecheck | Revert extraction commit |
| 81 | `claude/guardrail-hardening` -> `main` | 10 / 4 commits | Closed with #112 supersession receipt | Current CI checked only PRs targeting `main`; public copy scanners covered a narrow route subset; staged secret scanning read worktree bytes; tracked artifact directories and multi-commit deploy diffs remained blind | `EXTRACTED_REBUILT_CLOSED`; all PR targets run, fork-safe branch concurrency deduplicates runs, scanners normalize and sweep every rendered public route, staged secrets come from index blobs, and Vercel uses the last successful deployment SHA with fail-safe build behavior | 13 behavioral guardrail + 10 deploy-gate + 23 integration assertions; full guardrails, web typecheck, changed-test lint | Revert extraction commit |
...
| 83 | `claude/stress-property-suite` -> `claude/hotfix-clv-settlement-billing-integrity` | 11 / 2 commits | Closed with #112 supersession receipt | Prefix-overlapping team names reproduced side inversion; stale branch left free settlement and historical replay on the unsafe optional path | `EXTRACTED_REBUILT_CLOSED`; mandatory two-team identity across settlement, CLV, replay, and free settlement plus compact seeded properties | 75 engine + 26 pipeline + 17 web assertions; all three typechecks | Revert extraction commit |
| 84 | `claude/hotfix-clv-regrade-orphans` -> `main` | 2 / 1 commit | Closed with #112 supersession receipt | Current settlement already carried the observed-market receipt contract; missing grade-once orphan replay was reproduced | `EXTRACTED_REBUILT_CLOSED` | 21 settlement tests + pipeline typecheck | Revert extraction commit |
| 85 | `claude/hotfix-fabricated-record` -> `main` | 3 / 1 commit | Closed with #112 supersession receipt | Reproduced hard-coded public 0-0-0; rebuilt on shared canonical settled population with empty/error withholding | `EXTRACTED_REBUILT_CLOSED` | 26 slate/gate assertions + web typecheck | Revert extraction commit |
| 86 | `claude/hotfix-void-stale-picks` -> `main` | 8 / 2 commits | Closed with #112 supersession receipt | Three-day feed horizon, recorded-FINAL replay, stale VOID, outage-independent sweep, and no-action downstream semantics remained missing | `EXTRACTED_REBUILT_CLOSED` | 26 settlement + 35 ROI/outbox tests; web/pipeline typechecks | Revert extraction commit |
| 87 | `claude/hotfix-picks-outage-state` -> `main` | 9 / 3 commits | Closed with #112 supersession receipt | Picks and CLV primary-read failures still masqueraded as deliberate bootstrap gating; current picks page already had a designed generic outage state | `EXTRACTED_REBUILT_CLOSED`; shared honest `backend_outage` response and picks-scoped probe diagnostics | 41 focused web assertions + web typecheck | Revert extraction commit |
...
| 89 | `claude/hotfix-outage-sweep` -> `claude/hotfix-picks-outage-state` | 10 / 1 commit | Closed with #112 supersession receipt | Calibration, daily slate, promotions, and game-room reads still converted database failure into deliberate empty/missing states; proof-of-record already had the stronger `ledgerUnreachable` contract | `EXTRACTED_REBUILT_CLOSED`; four missing outage boundaries plus status-aware calibration probe | 134 focused outage/surface assertions + web typecheck | Revert extraction commit |
...
| 92 | `claude/hotfix-settle-refresh-races` -> `main` | 5 / 2 commits | Closed with #112 supersession receipt | Current create-once policy superseded refresh mutation, but concurrent upsert losers could still mint a mismatched immutable sidecar | `EXTRACTED_REBUILT_CLOSED`; create-only P2002 adoption, no loser sidecars | 25 process-sport tests + pipeline typecheck | Revert extraction commit |
| 93 | `claude/hotfix-cockpit-page-auth` -> `main` | 34 / 1 commit | Closed with #112 supersession receipt | All 32 current Cockpit pages lacked a page-boundary ADMIN check and relied on the parent layout | `EXTRACTED_REBUILT_CLOSED`; shared guard runs before page data loads and recursive CI scan covers the complete tree | 36/36 guard and complete-tree assertions; zero missing pages | Revert extraction commit |
| 94 | `claude/hotfix-proof-count-utc-bounds` -> `main` | 8 / 2 commits | Closed with #112 supersession receipt | Local-midnight helpers ignored injected time; bare Merkle root retained duplicate-last ambiguity | `EXTRACTED_REBUILT_CLOSED`; shared request time + UTC bounds + count-bound digest | 22 proof/board tests; web/engine typechecks | Revert extraction commit |
| 95 | `claude/hotfix-vacuous-stub-tests` -> `main` | 3 / 2 commits | Closed with #112 supersession receipt | Strict populated demo mock reproduced mixed DB/sample totals; board query predicates lacked exact UTC filter pins | `EXTRACTED_REBUILT_CLOSED`; non-vacuous fixtures retained | 16 populated/slate/board tests + web typecheck | Revert extraction commit |
...
| GitHub PR #76-#101 final reconciliation | 0 | All stale non-merged PRs in the range are closed with disposition receipts; extracted PRs #80/#81/#83-#87/#89/#92-#95 point to replacement draft PR #112; #101 remains open under an owner-gated migration hold | No PR was merged by this lane, no branch was deleted, and no migration was applied. |
| Public fantasy truth-gate matrix | 0 | 27/27 middleware, redirect, loop-prevention, source-truth, and protected-Studio assertions passed. Playwright verified `/fantasy`, `/fantasy/draft`, `/fantasy/props`, and `/optimizer` at 1440x900 and 390x844: 200 gate, correct 307 destinations, one H1, main landmark, no framework overlay, no console/page errors, and no horizontal overflow. | Local dev used a deliberate stub DB and a non-secret local auth placeholder; the gate performs no database read. Production deployment remains unverified. |
| Pick Evidence Envelope / Intelligence Playback focused matrix | 0 | 54/54 assertions passed across deterministic digesting, exact proof/gate binding, evidence-ID collision and temporal/boundary consistency, post-decision evidence exclusion, proof/factor eligibility, stale active-factor blocking, PUBLIC/PAID/COCKPIT projection, granular capability isolation, stored gate-status consistency, complete PASS lifecycle, stored published/PASS adaptation, Game Room entitlement redaction, outage behavior, route wiring, keyboard controls, scrubber, transcript/table, epistemic deltas, cited decision certificates, and Twin/Brain/autopsy/Studio consumers. Web TypeScript passed. | Persistence/providers are mocked in focused tests. Current `GateDecision` cannot capture a complete PASS market/threshold and has no discovered writer; stored incomplete PASS rows withhold. A real persisted route remains pending for browser proof. |
| Intelligence Playback renderer browser matrix | 0 | Playwright passed at 1440x900 and 390x844 with reduced motion: next/keyboard/scrubber/stop state changes, keyboard focus, transcript/table, both evidence directions, delta/certificate/citations, main landmark, zero raw marker, no horizontal page overflow, no framework overlay, and no console/page errors. Axe-core reported zero WCAG A/AA violations and 200% text zoom retained reflow. Screenshots are stored under `reports/visual/frontier-recovery/intelligence-playback-renderer-qa-*.png`. | Used an explicitly labeled local renderer fixture because no production database or eligible persisted room was available. The temporary route was deleted after capture. This proves rendering and interaction, not live data or deployment state. |
...
| `docs/architecture/INTELLIGENCE_DECISION_PERSISTENCE_PROPOSAL.md` | Owner-gated schema lane | Additive decision/evidence/calibration persistence proposal plus mandatory writer and no-guess rollout | PROPOSAL_ONLY |
...
- Git transport, branch push permission, and authenticated connector-backed GitHub PR reads/writes are verified. Draft PR #112 is the replacement review surface. Merge remains owner/repository-policy controlled.
...
5. Persist one immutable decision snapshot and derive every renderer from it; do not store a second event truth for Game Room, Twin, autopsy, or Studio.
6. Keep legacy PASS rows unavailable when their offered quote, boundary threshold, or source bindings cannot be proven. No guessed backfill is permitted.
7. Use draft PR #112 as the only replacement review surface. Closed extracted PRs remain provenance, not merge candidates.
