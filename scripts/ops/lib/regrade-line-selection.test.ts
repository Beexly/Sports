...
describe("gradingLineOf — the no-drift rule the settlement lanes use", () => {
  it("prefers the locked line and honours a genuine zero", () => {
    expect(gradingLineOf({ clvLockLine: -3.5, line: -3.25 })).toBe(-3.5);
    expect(gradingLineOf({ clvLockLine: 0, line: -3.25 })).toBe(0);
    expect(gradingLineOf({ clvLockLine: null, line: -3.25 })).toBe(-3.25);
  });
});
