/**
 * Completions-air-yards-diff covariate bind: couples the covariate bus
 * (PR 1) to the completions model (props-hb-comp).
 *
 * Honesty header
 * ──────────────
 * This is a BIND, not a model. It does not invent completions or air yards.
 *
 * `props-hb-comp.ts` models completions | attempts as Beta-Binomial (bounded
 * by attempts). Its `CompSample` carries only the *realized* `attempts` and
 * `completions` — the historical samples the prior fits on. Nothing upstream
 * fed it a leak-safe, honest NGS signal for how far downfield the passer is
 * throwing vs. how far the receiver is getting.
 *
 ...
 *      yards minus completed air yards per attempt. A positive differential
 *      → passes thrown downfield that aren't being completed → lower
 *      completion probability for the same attempt volume. This is a
 *      process/scheme arm-strength + accuracy signal, not a per-play frame
 *      measurement.
 *
 ...
 * seam where the p-path completions covariates meet the completions model.
 *
...
export const COMP_AIR_YARDS_DIFF_BIND_METHOD_TAG = "comp_air_yards_diff_bind_v1" as const;
 ...
 */
export interface CompAirYardsDiffBindRequest {
...
/**
 * `CompSample` enriched with leak-safe NGS covariates. The `attempts` /
 * `completions` fields are the model's existing realized inputs (unchanged);
...
/**
 * Result of binding completions-air-yards-diff covariates to a request.
 ...
 */
export type CompAirYardsDiffBindResult =
  | {
      readonly ok: true;
      readonly methodTag: typeof COMP_AIR_YARDS_DIFF_BIND_METHOD_TAG;
 ...
 * into a batch of completion samples, producing leak-safe `BoundCompSample`s.
 *
 * For each request:
...
export function bindCompAirYardsDiffSamples(
  ...
  for (const req of requests) {
    ...
    if (row === null) {
      out.push({
        ok: false,
        methodTag: COMP_AIR_YARDS_DIFF_BIND_METHOD_TAG,
        priced: false,
        refuse: "no_prior_row",
      });
      continue;
...
export function boundCompAirYardsDiffSamples(
  rows: readonly CovariateRow[],
  requests: readonly CompAirYardsDiffBindRequest[],
): BoundCompSample[] {
  return bindCompAirYardsDiffSamples(rows, requests)
    .filter((r): r is Extract<CompAirYardsDiffBindResult, { ok: true }> => r.ok)
    .map((r) => r.sample);
}
