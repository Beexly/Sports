...
export default async function Page(){
  ...
  const suggestionRows=suggestions.map(s=>{const rec=s as Record<string, unknown>; return { source: String(rec.source_name ?? rec.source ?? rec.source_type ?? ""), url: String(rec.submitted_url ?? rec.url ?? ""), status: String(rec.reviewed_status ?? rec.status ?? ""), suggested_by: String(rec.suggested_by ?? ""), created_at: String(rec.created_at ?? "") };});
